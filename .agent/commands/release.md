...
### 0.x での minor と patch の使い分け

semver は 0.x について規定を持たない（破壊的変更を minor で公開してよいとされる）。
本プロジェクトでは次の基準を採る。

| 変更内容                           | 上げる桁         | 例                     |
| ---------------------------------- | ---------------- | ---------------------- |
| 破壊的変更、機能追加               | minor（`0.X.0`） | モードの集約（v0.3.0） |
| 不具合修正、ビルドと CI のみの変更 | patch（`0.3.X`） | proxy の修正（v0.2.1） |

利用者にとっての意味は、minor が上がった場合は使い方が変わる可能性があり、
patch が上がった場合は同じ使い方のまま修正されている、というものである。

### 1.0.0 に上げる条件

**private リポジトリの外へ配布を始める時点で `1.0.0` にする。** Marketplace への
公開、または組織外への `.vsix` の配布がこれに当たる。内部利用が続く間は 0.x を
維持する。中身が安定していることは条件に含めない。

1.0.0 以降は通常の semver に従う（破壊的変更は major、機能追加は minor、
不具合修正は patch）。
