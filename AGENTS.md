...
- **カバレッジ基準: 触ったファイルは C0（命令網羅）100% ではなく C1（分岐網羅）100% に到達させる。**
    ...
    - **模したものが本物より寛容になっていないか。** `vi.fn()` はクラスのメソッドと違い
      `this` を辿らない。実例: host の戻し方を束縛せずに渡していた不具合が、`vi.fn()` を
      挿した試験では緑のままだった（ツールを呼ぶたびに例外になる不具合である）。本物と同じ形
      （クラス）で確かめるか、モックの約束を本物に対して固定する試験を置く
      （`webview-ui/src/components/settings/__tests__/settingsMocks.contract.spec.tsx`）。
    - **反例は、規則をぎりぎり満たさないものを選ぶ。** 「番地を要求する」規則の反例に数字を
      含まない文を置いても、規則が無くても通る。確かめたつもりになるだけである。
    - **上限や区切りは、届く大きさの入力で確かめる。** 分岐の被覆は「その行を通ったか」しか
      見ない。`if (held > 上限)` は毎回 false で通っても 100% になり、true 側は一度も
      動いていない。実例: 判定の記憶を捨てる処理が、捨てたばかりの結果まで消していた
      （上限 400 万字に対し、試験の文は数十字）。まとめて処理する数も同じで、1 件しか
      渡さない試験では 2 つ目以降の扱いを見ていない。上限は試験から差し替えられるように
      するか、届く入力を意図して作る。
    - **模したものに、本物の制限を写す。** 寛容さだけでなく、本物が持つ上限も写す。実例:
      判定のモデルは 512 断片までしか見ず超えた分を落とすが、偽物は無制限だったため、
      長い本文の後ろが黙って落ちることに気づけなかった。渡された量を確かめ、超えたら
      投げる偽物にする。
    - **状態そのものではなく、移り変わりを確かめる。** 「モデルが無い」と「ある」を別々に
      試験しても、「無い → ある」で起きることは分からない。実例: 第 2 層が効かないときに
      伏せた結果が記憶へ残り、あとでモデルを置いても第 1 層だけの結果を返し続けていた。
      切替・失敗からの回復・設定の変更は、1 つの試験の中で前後をまたいで確かめる。
    - **規則が 2 つ当たる入力を作る。** 規則ごとに試験しても、重なったときの挙動は分からない。
      実例: 肩書き（`営業部長`）と敬称（`森さん`）は別々に確かめていたが、両方が付いた
      `部長さん` を人名として伏せていた。
    - **継ぎ目は、両側を 1 つの試験で繋いで見る。** 送る側と受ける側を別々に確かめると、
      間の解釈のずれが誰にも見えない。実例: 画面が空の文字列を送ることと、拡張が空の
      文字列をどう読むかを別の試験で確かめており、「空欄＝既定の場所」という意図が
      途中で落ちていた。
- **同じ処理を挟む呼び出し箇所が複数あるなら、その数を固定する。**
    - カバレッジも単体の試験も「ほかにも呼び出し箇所がある」ことは教えない。直した箇所の
      試験は通り、残った箇所の試験も（新しい関心事を知らないので）通る。実例: 伏せ字の機能で、
      LLM へ送る
      箇所 3 つとツールの引数を解釈する箇所 4 つに対し、取りこぼしを 3 回繰り返した。
    - 直す前に `grep` で数え上げる。1 つ直して終わりにしない。
    - ソースを読んで呼び出し箇所の一覧を固定する試験を置く。箇所を足した人が赤で気づく。例:
      `src/services/pii/__tests__/maskingSeams.invariants.spec.ts`、
      `src/activate/__tests__/registerCommands.invariants.spec.ts`。
    - 一覧の照合は部分一致にしない。語の境界で当て、同じファイルに複数あるなら数を突き合わせる。
      部分一致だと、名前を変えただけの変異を捕まえられない。
    - 根拠の全文と不変条件ファーストの考え方: `.agent/rules/rules.md`。
- **配布物のビルド:** `pnpm bundle:internal`（esbuild）→ `pnpm vsix:internal`
  （vsce → `bin/openai-compatible-agent-<version>.vsix`）。
  **リリースは platform を指して作る**（`pnpm vsix:linux-x64` / `pnpm vsix:win32-x64`）。
  第 2 層が native を含むため、指さずには作れない。指さないと、作った機械のぶんだけが
  入った配布物ができ、ほかの platform では第 2 層が黙って動かない。
...
- **削除済みの mode slug が保存されている場合に注意する。** `isToolAllowedForMode` は解決
  ...
  も通ったまま拡張が動作しなくなる**不具合であるため、`?? defaultModeSlug` のように検証せずに
  通す実装へ戻してはならない。
