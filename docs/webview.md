...
- 入口（ホスト側の司令塔）: `src/core/webview/ClineProvider.ts`
...
VSCode がサイドバーを開くと、ホスト側の司令塔にある**画面初期化の担当**が呼ばれる。
...
どれも「具体的な司令塔クラスを直接は知らず、必要な操作だけを列挙した細い窓口
...
司令塔には「今の状態を UI へ送る」入口があり、送信用の状態オブジェクトを組み立てて
...
状態は重いので、司令塔は用途に応じて**軽量版の push**も持っている。チャットが
...
チャットが streaming 中の 1 メッセージ更新に至っては、Task 実行エンジンが司令塔への
...
司令塔が**同名イベントとして中継**する仕組みで拾っている。listener は配列から機械的に
...
UI のルートは、タブ（チャット/設定/履歴）を切り替えるだけの薄い司令塔で、
...
## 司令塔の内部構造 — 18 人の担当への分割

ホスト側の司令塔（ClineProvider）は、外から見ると「webview を持ち、タスクの山を管理し、
状態を push する 1 個のクラス」に見える。しかし中身は**薄い取次ぎ役（facade）**で、
実際の仕事は**18 の専門担当（collaborator）**に振り分けている。司令塔のコンストラクタは
これらを自分で `new` せず、**配線専用の組み立て役から受け取って代入するだけ**にしてある。
こうすると、依存関係が 1 か所で読め、テスト時は担当一式を丸ごと差し替えられる。

- 司令塔本体: `src/core/webview/ClineProvider.ts`
...
- **MCP ハブ / スキル** — 起動後に非同期で初期化され、解決後に司令塔が保持する（`startBackgroundInitialization`）。**ワークスペース追跡**は factory 内で同期的に生成され即保持される（ファイルパスの初期化のみ `webviewDidLaunch` 内で非同期）。

司令塔に残った公開メソッドの多くは「対応する担当へそのまま渡すだけ」の 1 行取次ぎで、
状態 push まわりのように複数担当をまたぐ調整だけが司令塔自身に残っている。
...
- 司令塔は薄い取次ぎ役で、実務は 18 の専門担当に分割され、配線は 1 か所に集約されている。
