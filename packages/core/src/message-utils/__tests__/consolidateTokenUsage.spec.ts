...
describe("consolidateTokenUsage", () => {
	...
	describe("context tokens calculation", () => {
		...
		it("condense_context に contextCondense が無ければ cost/contextTokens は 0 に落ちる", () => {
			// contextCondense 未設定 → cost は `?? 0`、newContextTokens も `?? 0` の各フォールバックを通る
			const messages: ClineMessage[] = [{ ts: 1000, type: "say", say: "condense_context" } as ClineMessage]

			const result = consolidateTokenUsage(messages)

			expect(result.totalCost).toBe(0)
			// contextTokens は 0 のまま（break せず逆順ループを走り切る）
			expect(result.contextTokens).toBe(0)
		})
		...
		it("疎な配列（穴）を含んでも逆順ループの undefined ガードで落ちない", () => {
			...
			const messages: ClineMessage[] = []
			messages[0] = createApiReqMessage(1000, { tokensIn: 100, tokensOut: 50 })
			messages.length = 3
...
describe("hasToolUsageChanged", () => {
	...
	it("キー数が同じでもスナップショットに無いツールがあれば変化ありとみなす", () => {
		// current と snapshot はキー数が等しい（各 1）が、キー自体が食い違う。
		// current のキーを引くと snapshotTool は undefined → `!snapshotTool` で true。
		const current = {
			read_file: { attempts: 1, failures: 0 },
		}
		const snapshot = {
