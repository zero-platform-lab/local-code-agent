...
/**
 * git のブランチ名として渡してよい形か。
 *
 * `execFile` なのでシェルは介さないが、git 自身が `-` 始まりをオプションとして読む。
 * `.` や `..` は pathspec として解釈されうる。git check-ref-format の完全な再実装は
 * しないが、破壊につながる形だけは確実に落とす。
 */
export function isPlausibleBranchName(branch: unknown): branch is string {
	if (typeof branch !== "string" || branch.length === 0) return false
	if (branch.startsWith("-")) return false
	// `.` / `..` そのもの、およびパス要素として `..` を含むもの
	if (branch === "." || branch.split("/").some((seg) => seg === "." || seg === "..")) return false
	// 制御文字・空白・git が禁じている文字（check-ref-format より抜粋）
...
export class WorktreeService {
	...
	async deleteWorktree(cwd: string, worktreePath: string, force = false): Promise<WorktreeResult> {
		try {
			...
			// worktreePath は webview 由来で、これまで正規化も検証もせず git に渡していた。
			// git 側の歯止めは「main working tree か」の判定だけなので、ワークスペース自身が
			// linked worktree のとき（この機能で switch した後の通常状態）は、利用者が今
			// 開いているディレクトリが --force で未コミット変更ごと消える。
			// git が知っている worktree のいずれかであることを、消す前に確かめる。
			if (!worktreeToDelete) {
				return {
					success: false,
					message: `Refusing to remove a path git does not report as a worktree: ${worktreePath}`,
				}
			}

			// 今開いているディレクトリは消さない。UI 側の disabled だけに頼らない。
			if (this.normalizePath(worktreeToDelete.path) === this.normalizePath(cwd)) {
				return {
					success: false,
					message: `Refusing to remove the worktree currently open: ${worktreePath}`,
				}
			}
	...
	async checkoutBranch(cwd: string, branch: string): Promise<WorktreeResult> {
		// branch は webview 由来で、cwd は本体のワークスペース。
		// `git checkout <branch>` は end-of-options 区切りが無いと pathspec とも解釈されるため、
		// `"."` を渡されると `git checkout .` になって未コミット変更が全部消える。
		// `-f` や `--orphan` も git のオプションとして解釈される。
		// 末尾の `--` で「pathspec は無い」と明示し、あわせて `-` 始まりを弾く。
