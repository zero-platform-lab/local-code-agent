import {
	agentCliControlEventSchema,
	agentCliFinalOutputSchema,
	agentCliInputCommandSchema,
	agentCliStreamEventSchema,
} from "../cli.js"

describe("CLI types", () => {
	describe("agentCliInputCommandSchema", () => {
		it("validates a start command", () => {
			const result = agentCliInputCommandSchema.safeParse({
				command: "start",
				requestId: "req-1",
				prompt: "hello",
	...
	describe("agentCliControlEventSchema", () => {
		it("validates a control done event", () => {
			const result = agentCliControlEventSchema.safeParse({
				type: "control",
				subtype: "done",
				requestId: "req-3",
				command: "start",
	...
	describe("agentCliStreamEventSchema", () => {
		it("accepts passthrough fields for forward compatibility", () => {
			const result = agentCliStreamEventSchema.safeParse({
				type: "assistant",
				id: 42,
				content: "partial",
				customField: "future",
	...
	describe("agentCliFinalOutputSchema", () => {
		it("validates final json output shape", () => {
			const result = agentCliFinalOutputSchema.safeParse({
				type: "result",
				success: true,
				content: "done",
				events: [],
