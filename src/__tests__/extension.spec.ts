...
vi.mock("../activate", () => ({
	...
	registerFileVaultCommands: vi.fn(),
...
describe("extension.ts", () => {
	...
	test("dotenvx.config が投げても activation を止めず警告に落とす", async () => {
		vi.resetModules()
		vi.clearAllMocks()
		...
		const dotenvx = await import("@dotenvx/dotenvx")
		vi.mocked(dotenvx.config).mockImplementation(() => {
			throw new Error("bad env")
		})

		const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {})

		// import 時（トップレベル）に config が呼ばれて例外を握りつぶす
		const { activate } = await import("../extension")

		expect(dotenvx.config).toHaveBeenCalledTimes(1)
		expect(warnSpy).toHaveBeenCalledWith("Failed to load environment variables:", expect.any(Error))
