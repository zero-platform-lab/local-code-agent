...
describe("ClineProvider.delegateParentAndOpenChild()", () => {
	it("persists parent delegation metadata and emits TaskDelegated", async () => {
		...
		const child = await makeDelegationController(provider).delegateParentAndOpenChild(params)
		...
		expect(providerEmit).toHaveBeenCalledWith(AgentEventName.TaskDelegated, "parent-1", "child-1")
	...
	it("calls child.start() only after parent metadata is persisted (no race condition)", async () => {
		...
		await makeDelegationController(provider).delegateParentAndOpenChild({
			parentTaskId: "parent-1",
			message: "Do something",
			initialTodos: [],
			mode: "code",
