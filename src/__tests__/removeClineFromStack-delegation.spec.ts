...
import { TaskStackController } from "../core/webview/TaskStackController"
...
describe("ClineProvider.removeClineFromStack() delegation awareness", () => {
	...
	function buildMockProvider(opts: {
		...
		const taskStack = new TaskStackController(
			{ log, getState: async () => ({ mode: "code" }), getTaskWithId, updateTaskHistory },
			[childTask as any],
		)
	...
	it("handles empty stack gracefully", async () => {
		...
		const taskStack = new TaskStackController(
			{ log: vi.fn(), getState: async () => ({ mode: "code" }), getTaskWithId, updateTaskHistory },
			[],
		)
		const provider = {
			taskStack,
