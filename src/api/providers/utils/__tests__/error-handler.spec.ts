...
describe("handleProviderError", () => {
	...
	describe("error message formatting", () => {
		it("should fall back to an empty message when the Error has no message and no metadata.raw", () => {
			// error.message が空文字 → `anyErr?.error?.metadata?.raw || error.message || ""` の
			// 最後の `|| ""` に落ちる分岐を踏む。
			...
			expect(result).toBeInstanceOf(Error)
			// prefix だけが残り、後続のメッセージは空。
			expect(result.message).toBe("TestProvider completion error: ")
		})
