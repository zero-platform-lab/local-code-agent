...
describe("presentAssistantMessage - Image Handling in Native Tool Calling", () => {
	...
	beforeEach(() => {
		...
		mockTask.pushToolResultToUserContent = vi.fn().mockImplementation((toolResult: any) => {
			const existingResult = mockTask.stream.userMessageContent.find(
				(block: any) => block.type === "tool_result" && block.tool_use_id === toolResult.tool_use_id,
			)
			if (existingResult) {
				return false
			}
			mockTask.stream.userMessageContent.push(toolResult)
			return true
		})
	})
	...
	it("should preserve images in tool_result for native tool calling", async () => {
		...
		mockTask.stream.assistantMessageContent = [
			{
				type: "tool_use",
		...
		expect(mockTask.stream.userMessageContent.length).toBeGreaterThan(0)

		// Find the tool_result block
		const toolResult = mockTask.stream.userMessageContent.find(
		...
		const imageBlocks = mockTask.stream.userMessageContent.filter((item: any) => item.type === "image")
	...
	it("should fail fast when tool_use is missing id (legacy/XML-style tool call)", async () => {
		...
		const textBlocks = mockTask.stream.userMessageContent.filter((item: any) => item.type === "text")
		...
		expect(mockTask.stream.userMessageContent.some((item: any) => item.type === "image")).toBe(false)
	})
	...
	describe("Multiple tool calls handling", () => {
		it("should send tool_result with is_error for skipped tools in native tool calling when didRejectTool is true", async () => {
			...
			mockTask.stream.didRejectTool = true

			// Process the second tool (should be skipped)
			mockTask.stream.currentStreamingContentIndex = 1
			await presentAssistantMessage(mockTask)
			...
			const textBlocks = mockTask.stream.userMessageContent.filter(
		...
		it("should reject subsequent tool calls when a legacy/XML-style tool call is encountered", async () => {
			...
			expect(mockTask.stream.userMessageContent.some((item: any) => item.type === "tool_result")).toBe(false)
		})
