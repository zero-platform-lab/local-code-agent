...
describe("importExport", () => {
	...
	describe("importSettings", () => {
		...
		it("should not clobber existing api configs", async () => {
			...
			if (result.success && "providerProfiles" in result) {
				...
				expect(result.providerProfiles?.apiConfigs["default"].apiProvider).toBe("openai")
			}
		})
		...
		describe("lenient import with invalid providers", () => {
			...
			it("should handle multiple profiles with mixed valid and invalid providers", async () => {
				...
				const importedProfiles = mockProviderSettingsManager.import.mock.calls[0][0]
				expect(importedProfiles.apiConfigs["anthropic-profile"].apiProvider).toBe("openai")
	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			;(vscode.window.showSaveDialog as Mock).mockResolvedValue({
				fsPath: "/mock/path/agent-settings.json",
			})
			...
			expect(safeWriteJson).toHaveBeenCalledWith("/mock/path/agent-settings.json", {
		...
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(path.join("/mock/home", "Downloads", "agent-settings.json"))
		})
		...
		describe("codebase indexing export", () => {
			...
			it("should maintain backward compatibility with existing exports", async () => {
				...
				const mockGlobalSettings = {
					...
					codebaseIndexConfig: {
						codebaseIndexEnabled: true,
						codebaseIndexEmbedderProvider: "openai-compatible" as const, // Not openai-compatible
		...
		it.each([
			...
			async ({ providerName, modelId, providerId }) => {
				...
				// スキーマから外れた古いフィールドを持つ設定を保存する（cast で無理やり載せる）。
				// export 時にスキーマ parse で落ちることを検証する。
	...
	describe("branch coverage gaps", () => {
		it("sanitizes null and primitive apiConfig entries via the non-object early return", async () => {
			// sanitizeProviderConfig の `typeof apiConfig !== "object" || apiConfig === null` を
			// 両サイド踏む: string は左（typeof）で、null は右（=== null）で早期 return する。
			;(vscode.window.showOpenDialog as Mock).mockResolvedValue([{ fsPath: "/mock/path/settings.json" }])

			const content = JSON.stringify({
				providerProfiles: {
					currentApiConfigName: "good",
					apiConfigs: {
						good: { apiProvider: "openai" as ProviderName, openAiApiKey: "k", id: "good-id" },
						nullEntry: null,
			...
			expect(result.success).toBe(true)
			// 壊れないエントリだけが取り込まれ、null/primitive は落ちる（設定を壊さない不変条件）。
			const imported = (mockProviderSettingsManager.import as Mock).mock.calls[0][0]
			expect(imported.apiConfigs.good).toBeDefined()
			expect(imported.apiConfigs.nullEntry).toBeUndefined()
			expect(imported.apiConfigs.stringEntry).toBeUndefined()
			expect(result.warnings).toEqual(
				expect.arrayContaining([expect.stringContaining("nullEntry"), expect.stringContaining("stringEntry")]),
			)
		})

		it("does not write anything when there are no provider profiles to export", async () => {
			;(vscode.window.showSaveDialog as Mock).mockResolvedValue({ fsPath: "/mock/path/agent-settings.json" })
			// export() が undefined を返す＝プロファイル未設定。書き込みへ進まず黙って返る。
			mockProviderSettingsManager.export.mockResolvedValue(undefined as never)
			mockContextProxy.export.mockResolvedValue({ mode: "code" })
		...
		it("reports failure without touching the file when access rejects with a non-Error value", async () => {
			// catch の `error instanceof Error ? error.message : "Unknown error"` の非 Error 側を踏む。
			const filePath = "/nope/settings.json"
			;(fs.access as Mock).mockRejectedValue("not-an-error-string")
			...
			expect(fs.access).toHaveBeenCalledWith(filePath, fs.constants.F_OK | fs.constants.R_OK)
			// アクセス失敗時は読み込みも状態更新もしない（失敗したら何もしない）。
			expect(fs.readFile).not.toHaveBeenCalled()
		...
		it("falls back to the file dialog and shows an info toast on a clean import when no filePath is given", async () => {
			// filePath なし → else 分岐で importSettings を使う。かつ警告なしの成功で
			// showInformationMessage 側（警告分岐の else）を踏む。
			;(vscode.window.showOpenDialog as Mock).mockResolvedValue([{ fsPath: "/mock/path/settings.json" }])
			...
			const content = JSON.stringify({
				providerProfiles: {
					currentApiConfigName: "test",
					apiConfigs: {
						test: { apiProvider: "openai" as ProviderName, openAiApiKey: "k", id: "test-id" },
					},
				},
				globalSettings: {},
			})
			...
			const showInfoSpy = vi.spyOn(vscode.window, "showInformationMessage").mockResolvedValue(undefined)
			const showWarnSpy = vi.spyOn(vscode.window, "showWarningMessage").mockResolvedValue(undefined)
			...
			// filePath 経路を通っていないので access は呼ばれない。
