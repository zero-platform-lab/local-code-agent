...
import { itemRole } from "../../task-persistence/agentMessageUtils"
...
describe("Context Management", () => {
	...
	describe("estimateTokenCount", () => {
		...
		it("should estimate tokens for text blocks", async () => {
			...
			const longerContent: Array<ContentBlockParam> = [
				{
					type: "text",
		...
		it("should estimate tokens for mixed content blocks", async () => {
			...
			const imageOnlyContent: Array<ContentBlockParam> = [
				{ type: "image", source: { type: "base64", media_type: "image/jpeg", data: "dummy_data" } },
			]
		...
		it("should handle empty text blocks", async () => {
			const content: Array<ContentBlockParam> = [{ type: "text", text: "" }]
	...
	describe("manageContext", () => {
		...
		it("should consider incoming content when deciding to truncate", async () => {
			...
			// 履歴の content は Responses 形式の input_text パート列になったが、
			// トークン概算 API は従来どおり ContentBlockParam（text ブロック）を受け取る。
			// 同じ文字列から両方を組み立てて、期待値と実装の食い違いを防ぐ。

			// Test case 1: Small content that won't push us over the threshold
			const smallText = "Small content"
			const smallContentTokens = await estimateTokenCount([{ type: "text", text: smallText }], mockApiHandler)
			const messagesWithSmallContent: ApiMessage[] = [
				...messages.slice(0, -1),
				{ type: "message", role: "user", content: [{ type: "input_text", text: smallText }] },
			]
			...
			const largeText =
				"A very large incoming message that would consume a significant number of tokens and push us over the threshold"
			const largeContentTokens = await estimateTokenCount([{ type: "text", text: largeText }], mockApiHandler)
			...
			const veryLargeText = "X".repeat(1000)
			const veryLargeContentTokens = await estimateTokenCount(
				[{ type: "text", text: veryLargeText }],
				mockApiHandler,
			)
			const messagesWithVeryLargeContent: ApiMessage[] = [
				...messages.slice(0, -1),
				{ type: "message", role: "user", content: [{ type: "input_text", text: veryLargeText }] },
			]
		...
		it("should use summarizeConversation when autoCondenseContext is true and tokens exceed threshold", async () => {
			...
			const mockSummarizeResponse: condenseModule.SummarizeResponse = {
				messages: [
					{ type: "message", role: "user", content: "First message" },
					{ type: "message", role: "user", content: mockSummary, isSummary: true },
					{ type: "message", role: "assistant", content: "Last message" },
				],
				summary: mockSummary,
				cost: mockCost,
	...
	/**
	 * Tests for filesReadByAgent being passed to summarizeConversation
	 */
	describe("filesReadByAgent parameters", () => {
		const createModelInfo = (contextWindow: number, maxTokens?: number): ModelInfo => ({
			contextWindow,
			supportsPromptCache: true,
			maxTokens,
		...
		it("should pass filesReadByAgent, cwd, and rooIgnoreController to summarizeConversation when provided", async () => {
			// Mock the summarizeConversation function
			...
			const mockSummarizeResponse: condenseModule.SummarizeResponse = {
				messages: [
					{ type: "message", role: "user", content: "First message" },
					{ type: "message", role: "assistant", content: mockSummary, isSummary: true },
					{ type: "message", role: "user", content: "Last message" },
				],
				summary: mockSummary,
				cost: mockCost,
				newContextTokens: 100,
			...
			// Verify summarizeConversation was called with filesReadByAgent, cwd, and rooIgnoreController
			expect(summarizeSpy).toHaveBeenCalledWith({
				messages: messagesWithSmallContent,
				apiHandler: mockApiHandler,
				systemPrompt: "System prompt",
		...
		it("should pass undefined filesReadByAgent parameters when not provided", async () => {
			// Mock the summarizeConversation function
			...
			const result = await manageContext({
				...
				profileThresholds: {},
				currentProfileId: "default",
				// filesReadByAgent, cwd, rooIgnoreController are NOT provided
			})
		...
		it("should pass empty array filesReadByAgent when provided as empty", async () => {
			// Mock the summarizeConversation function
	...
	describe("profile-specific thresholds", () => {
		...
		/**
		 * 範囲外（-1 でも [MIN,MAX] でもない）のプロファイル閾値は無効として扱い、
		 * console.warn を出してグローバル設定へフォールバックする（else 分岐）。
		 */
		it("should warn and fall back to global when profile threshold is out of range", async () => {
			const modelInfo = createModelInfo(100000, 30000)
			const profileThresholds = {
				"test-profile": 200, // 範囲外（MAX_CONDENSE_THRESHOLD=100 を超える）
			}
			const currentProfileId = "test-profile"

			// 50% はグローバル閾値 80% 未満かつ allowedTokens(60000) 未満 → 何も起きない
			...
			expect(warnSpy).toHaveBeenCalledWith(
				expect.stringContaining('Invalid profile threshold 200 for profile "test-profile"'),
			)
			// グローバル 80% を使うので 50% では要約しない
			expect(summarizeSpy).not.toHaveBeenCalled()
			expect(result.prevContextTokens).toBe(totalTokens)
	...
	describe("willManageContext", () => {
		...
		it("maxTokens が falsy のとき ANTHROPIC_DEFAULT_MAX_TOKENS を予約する（auto-condense 無効側）", () => {
			// reservedTokens = maxTokens || ANTHROPIC_DEFAULT_MAX_TOKENS。
			// maxTokens=null なので既定(8192)が使われ、allowedTokens = 100000*0.9 - 8192 = 81808。
			// totalTokens 81809 は超過 → true。
			const over = willManageContext({
				totalTokens: 81809,
				contextWindow: 100000,
				maxTokens: null,
				autoCondenseContext: false,
		...
		it("maxTokens が falsy のとき ANTHROPIC_DEFAULT_MAX_TOKENS を予約する（auto-condense 有効側）", () => {
			// auto-condense 有効。閾値 50% には届かないが、既定予約での allowedTokens 超過で true。
			const result = willManageContext({
				totalTokens: 90000, // 90% > 50% 閾値
				contextWindow: 100000,
				maxTokens: undefined,
				autoCondenseContext: true,
				autoCondenseContextPercent: 50,
	...
	describe("newContextTokensAfterTruncation", () => {
		...
		it("should include system prompt tokens in newContextTokensAfterTruncation", async () => {
			...
			const messages: ApiMessage[] = [
				...
				{ type: "message", role: "user", content: "Third message" },
				{ type: "message", role: "assistant", content: "Fourth message" },
				{ type: "message", role: "user", content: "" }, // Small content in last message
			]
