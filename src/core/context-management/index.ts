 ...
 *
 * @param {Array<ContentBlockParam>} content - The content to count tokens for
 * @param {ApiHandler} apiHandler - The API handler to use for token counting
 * @returns {Promise<number>} A promise resolving to the token count
 */
export async function estimateTokenCount(content: Array<ContentBlockParam>, apiHandler: ApiHandler): Promise<number> {
...
export function truncateConversation(messages: ApiMessage[], fracToRemove: number, _taskId: string): TruncationResult {
	...
	// function_call とその function_call_output のペアが割れる。片方だけ隠すと
	// 相手のいない tool 応答（orphan）が送信され、互換サーバが 400 を返す。
	// 隠す集合を call_id 単位で閉じるまで広げる。
	expandToWholeToolPairs(messages, indicesToTruncate)
...
/**
 * 隠す index の集合を、function_call / function_call_output のペアが割れないように広げる。
 *
 * 片方だけが隠れる call_id が無くなるまで、相方の index を集合に足す。
 */
function expandToWholeToolPairs(messages: ApiMessage[], indicesToTruncate: Set<number>): void {
	const byCallId = new Map<string, number[]>()
	messages.forEach((msg, index) => {
		if (msg.type !== "function_call" && msg.type !== "function_call_output") return
		const list = byCallId.get(msg.call_id) ?? []
		list.push(index)
		byCallId.set(msg.call_id, list)
	})

	for (const indices of byCallId.values()) {
		if (indices.some((i) => indicesToTruncate.has(i))) {
			for (const i of indices) indicesToTruncate.add(i)
		}
	}
}
...
export async function manageContext({
	...
	const lastMessageText = itemText(messages[messages.length - 1])
	const lastMessageTokens = lastMessageText
		? await estimateTokenCount([{ type: "text", text: lastMessageText }], apiHandler)
		: 0
	...
	if (prevContextTokens > allowedTokens) {
		...
		for (const msg of effectiveMessages) {
			const text = itemText(msg)
			if (text) {
				newContextTokensAfterTruncation += await estimateTokenCount([{ type: "text", text }], apiHandler)
			}
		}
