...
describe("checkContextWindowExceededError", () => {
	...
	describe("throwing getters (catch coverage / 例外を漏らさない不変条件)", () => {
		it("OpenAI 判定中に name ゲッタが投げても false を返す（checkIsOpenAI の catch）", () => {
			// `"name" in error` は true（アクセサが存在する）→ error.name 参照で throw → catch → false。
			// OpenRouter/Anthropic 判定は status も error も持たないので false のまま。
			const error = {
				get name() {
					throw new Error("boom in name getter")
				},
			}
			expect(checkContextWindowExceededError(error)).toBe(false)
		})

		it("Anthropic 判定中に error ゲッタが投げても false を返す（checkIsAnthropic の catch）", () => {
			// OpenAI 判定は name を持たず APIError でもないので素直に false。
			// OpenRouter 判定は err.error ゲッタで throw するが自前 catch で false。
			// Anthropic 判定でも res.error ゲッタで throw → catch → false。
			const error = {
				get error() {
					throw new Error("boom in error getter")
				},
			}
			expect(checkContextWindowExceededError(error)).toBe(false)
		})

		it('Anthropic の message が欠落していても型のみで判定し false（message || "" の既定側）', () => {
			// type は一致するが message が undefined → String(undefined || "") = "" → どのパターンにも一致せず false。
			const error = {
				error: {
					error: {
						type: "invalid_request_error",
