...
export function computeUnifiedDiffStats(diff?: string): DiffStats | null {
	...
	try {
		...
		/* v8 ignore next -- 到達不能: parsePatch は常に長さ>=1 の配列を返し、null/[] を返さない。ライブラリ契約が将来崩れたときに null を返す防御既定 */
		...
		for (const p of patches) {
			/* v8 ignore next -- 到達不能: parsePatch のパッチは常に hunks 配列を持つ。hunks 欠落時の防御既定 */
			for (const h of (p as any).hunks ?? []) {
				/* v8 ignore next -- 到達不能: hunk は常に lines 配列を持つ。lines 欠落時の防御既定 */
