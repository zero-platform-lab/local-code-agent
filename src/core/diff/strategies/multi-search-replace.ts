...
import { ToolProgressStatus } from "@openai-agent/types"
		...
		for (const replacement of replacements) {
			...
			if (matchIndex === -1 || bestMatchScore < this.fuzzyThreshold) {
				...
				if (bestMatchIndex !== -1 && bestScore >= this.fuzzyThreshold) {
					...
					// startLine / endLine はこの時点で常に number（上流で数値化済み）なので、
					// 旧 `startLine !== undefined && endLine !== undefined` ガードは常に真。
					// 到達不能なフォールバックは置かず、範囲付きの Original Content を常に組み立てる。
			...
			const originalIndents = matchedLines.map((line) => {
				...
				/* v8 ignore next 3 -- 到達不能: 先頭空白の正規表現は空文字にもマッチするため match は null にならない。防御既定の空文字は残す */
				if (!match) {
					return ""
				}
				return match[0]
			...
			const indentedReplaceLines = replaceLines.map((line) => {
				...
				/* v8 ignore next -- 到達不能: 先頭空白の正規表現は空文字にもマッチするため currentIndentMatch は null にならない。防御既定の空文字は残す */
