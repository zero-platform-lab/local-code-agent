...
import type { ClineMessage, ExtensionState, TodoItem } from "@openai-agent/types"
...
/**
 * ワークスペースのファイル一覧取得（best-effort）の最大待ち時間。これを超えたら空一覧で
 * 続行し、リクエスト全体をブロックしない。ripgrep 不在で throw する場合は即 catch されるので
 * 通常このタイムアウトには達しない（巨大/遅い/ネットワーク FS の保険）。
 */
const LIST_FILES_MAX_WAIT_MS = 15_000

/**
 * Provider state read by {@link getEnvironmentDetails}. Mirrors the return type
 * of `ClineProvider.getState()` without importing the concrete provider, keeping
 * the environment layer decoupled from the webview layer.
 */
type EnvironmentDetailsProviderState = Omit<
	ExtensionState,
	"clineMessages" | "renderContext" | "hasOpenedModeSelector" | "version"
>

/**
 * Narrow structural view of {@link Task} required by {@link getEnvironmentDetails}.
 * Declared as an interface (rather than importing the concrete `Task`) so this
 * module does not depend on the task layer, avoiding a module cycle. Any object
 * exposing these members (notably `Task`) satisfies it structurally.
 */
export interface EnvironmentDetailsHost {
	readonly api: { getModel(): { id: string } }
	readonly messageStore: { clineMessages: ClineMessage[] }
	readonly cwd: string
	didEditFile: boolean
	readonly fileContextTracker: { getAndClearRecentlyModifiedFiles(): string[] }
	readonly providerRef: {
		deref(): { getState(): Promise<EnvironmentDetailsProviderState> } | undefined
	}
...
export async function getEnvironmentDetails(cline: EnvironmentDetailsHost, includeFileDetails: boolean = false) {
	let details = ""
	...
	if (includeCurrentCost) {
		const { totalCost } = getApiMetrics(cline.messageStore.clineMessages)
	...
	if (includeFileDetails) {
		...
		if (isDesktop) {
			...
			/* v8 ignore next -- 到達不能: maxWorkspaceFiles は 58 行の分割代入で undefined を 200 にデフォルト済み・型も number のため、?? 200 の既定側（右辺）には到達しない。作者が書いた防御既定・安全のため残す */
			...
			if (maxFiles === 0) {
				...
				// ワークスペースのファイル一覧は **best-effort**。ripgrep(rg) が見つからない(throw)や、
				// 巨大/遅い/ネットワーク FS で応答が返らない(hang)場合でも、**リクエスト全体を止めない**：
				// 失敗・タイムアウト時は空一覧で続行する。以前はここで throw/hang が起き、
				// attemptApiRequest に到達できず、UI が「APIリクエスト…」のまま無音で固まっていた
				// （ripgrep が同梱されない Windows 環境で発生）。
				const empty: [string[], boolean] = [[], false]
				let timer: ReturnType<typeof setTimeout> | undefined
				const [files, didHitLimit] = await Promise.race([
					listFiles(cline.cwd, true, maxFiles).catch(() => empty),
					new Promise<[string[], boolean]>((resolve) => {
						timer = setTimeout(() => resolve(empty), LIST_FILES_MAX_WAIT_MS)
					}),
				])
				clearTimeout(timer)
