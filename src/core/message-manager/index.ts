import { turnRole } from "../task-persistence/agentMessageUtils"
...
/**
 * Narrow structural view of {@link Task} required by {@link MessageManager}.
 * Declared as an interface (rather than importing the concrete `Task`) so this
 * module does not depend on the task layer, breaking the Task ↔ MessageManager
 * module cycle. `Task` satisfies it structurally.
 *
 * NOTE: `taskId`/`globalStoragePath` are read via an `as any` cast in the body
 * (globalStoragePath is `private` on Task, so it cannot be part of a structural
 * interface), and are intentionally omitted here.
 */
export interface MessageManagerHost {
...
export class MessageManager {
	constructor(private task: MessageManagerHost) {}
	...
	async rewindToTimestamp(ts: number, options: RewindOptions = {}): Promise<void> {
		...
		const clineIndex = this.task.messageStore.clineMessages.findIndex((m) => m.ts === ts)
		if (clineIndex === -1) {
	...
	async rewindToIndex(toIndex: number, options: RewindOptions = {}): Promise<void> {
		const cutoffTs = this.task.messageStore.clineMessages[toIndex]?.ts ?? Date.now()
	...
	private collectRemovedContextEventIds(fromIndex: number): ContextEventIds {
		...
		for (let i = fromIndex; i < this.task.messageStore.clineMessages.length; i++) {
			const msg = this.task.messageStore.clineMessages[i]
	...
	private async truncateClineMessages(toIndex: number): Promise<void> {
		await this.task.overwriteClineMessages(this.task.messageStore.clineMessages.slice(0, toIndex))
	}
	...
	private async truncateApiHistoryWithCleanup(
		...
		const originalHistory = this.task.messageStore.apiConversationHistory
		...
		if (!hasExactMatch && hasMessageBeforeCutoff) {
			...
			const firstUserMsgIndexToRemove = apiHistory.findIndex(
				// item 列では tool 結果は function_call_output という別 item になる。
				// これも user ターンに属するので turnRole で判定する（itemRole だと拾えない）。
				(m) => m.ts !== undefined && m.ts >= cutoffTs && turnRole(m) === "user",
			)
		...
		if (!skipCleanup) {
			...
			for (const msg of this.task.messageStore.clineMessages) {
				if (msg.ts) {
