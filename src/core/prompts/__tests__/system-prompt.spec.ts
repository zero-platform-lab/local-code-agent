...
describe("SYSTEM_PROMPT", () => {
	...
	it("未知モードでも既定モードにフォールバックして生成する", async () => {
		const prompt = await SYSTEM_PROMPT(
			mockContext,
			"/test/path",
			false,
		...
		// 既定モード (modes[0]) の roleDefinition が使われる
		expect(prompt).toContain(modes[0].roleDefinition)
	})

	it("skillsManager がスキルを返すと AVAILABLE SKILLS セクションを含める", async () => {
		const skillsManager = {
			getSkillsForMode: vi.fn().mockReturnValue([
				{
					name: "pdf-processing",
					description: "Extract text from PDFs",
					path: "/abs/pdf/SKILL.md",
