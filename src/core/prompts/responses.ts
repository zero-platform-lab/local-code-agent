...
(This is an automated message, so do not respond to it conversationally.)`
	...
	toolResult: (text: string, images?: string[]): string | Array<TextBlockParam | ImageBlockParam> => {
		if (images && images.length > 0) {
			const textBlock: TextBlockParam = { type: "text", text }
			const imageBlocks: ImageBlockParam[] = formatImagesIntoBlocks(images)
	...
	imageBlocks: (images?: string[]): ImageBlockParam[] => {
...
const formatImagesIntoBlocks = (images?: string[]): ImageBlockParam[] => {
	return images
		? images.map((dataUrl) => {
