...
function formatDirectoryContent(files: Array<{ filename: string; content: string }>, cwd: string): string {
	/* v8 ignore next -- 到達不能: 呼び出し元 (loadRuleFiles / addCustomInstructions) が files.length>0 を確認してから呼ぶため、空配列は渡らない */
...
async function loadAgentRulesFileFromDirectory(
	...
	/* v8 ignore next -- 到達不能: 両呼び出し元 (loadAllAgentRulesFiles) が常に cwd を渡すため、cwd falsy の分岐には到達しない */
...
export async function addCustomInstructions(
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		// NOTE: usedRuleFile は ".agent/rules-<mode>" を含まない文字列
		// （"rules-<mode> directories" / ".agentrules-<mode>" / ".clinerules-<mode>"）
		// にしか設定されないため、この includes は常に false（下の else が唯一の実経路）。
		/* v8 ignore next 2 -- 到達不能: 上記理由により true 分岐へは到達しない */
