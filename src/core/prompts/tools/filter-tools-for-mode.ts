...
import type { ModeConfig, ToolName, ToolGroup, ModelInfo } from "@openai-agent/types"
...
function getOrCreateRenamedTool(
	...
	/* v8 ignore next 3 -- 到達不能: 唯一の呼び出し元 filterNativeToolsForMode は `"function" in tool && tool.function` を確認済みのツールだけを渡すため、この防御ガードには到達しない */
...
export function filterNativeToolsForMode(
	...
	// web_fetch は既定オフ（opt-in）。有効化されていなければモデルへ送るツールから外す。
	// 0.8.1 で read グループに常時含め既定 on にしたところ、proxy 制限網でモデルが
	// web_fetch を呼んで失敗しタスクが脱線する退行が出たため、0.8.2 で既定オフに戻す。
	if (!settings?.webFetchEnabled) {
		allowedToolNames.delete("web_fetch")
	}
...
export function isToolAllowedInMode(
	...
	if (ALWAYS_AVAILABLE_TOOLS.includes(toolName)) {
		...
		/* v8 ignore start -- 到達不能: codebase_search は ALWAYS_AVAILABLE_TOOLS に含まれないため、この if ブロックには入らない（許可判定は下部の isToolAllowedForMode 経路が担う） */
