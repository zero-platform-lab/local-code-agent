...
describe("getMcpServerTools", () => {
	...
	it("should deduplicate tools with identical name from the same server", () => {
		// 同一サーバ内に同名ツールが 2 つある場合、2 個目は seenToolNames で弾かれる。
		// buildMcpToolName(server, tool) が同一になるため重複判定に入る。
		const server = createMockServer("dupServer", [
			createMockTool("sameName", "first wins"),
			createMockTool("sameName", "second ignored"),
		])
		const mockHub = createMockMcpHub([server])

		const result = getMcpServerTools(mockHub as McpHub)

		// 重複は 1 つに折り畳まれ、最初の出現が採用される
		expect(result).toHaveLength(1)
		expect(getFunction(result[0]).name).toBe("mcp--dupServer--sameName")
		expect(getFunction(result[0]).description).toBe("first wins")
	})

	it("should build a minimal schema when tool has no inputSchema", () => {
		// inputSchema が無いツールは { type: object, additionalProperties: false } を得る
		const toolWithoutSchema: McpTool = {
			name: "noSchema",
			description: "Tool without schema",
			inputSchema: undefined as unknown as McpTool["inputSchema"],
		}
		const server = createMockServer("testServer", [toolWithoutSchema])
		const mockHub = createMockMcpHub([server])
