...
describe("createReadFileTool", () => {
	...
	describe("tool structure", () => {
		...
		it("must NOT set strict (strict:true + required:['path'] のみ は endpoint に 400 で弾かれる)", () => {
			const tool = createReadFileTool()

			// 以前は strict:true だったが、プロパティ5個中 required は path だけで strict 要件
			// （全 property が required）を満たさず、tools 付きリクエストが 400 になっていた。
			expect(getFunctionDef(tool).strict).toBeUndefined()
		})
