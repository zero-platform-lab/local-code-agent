...
export function createReadFileTool(options: ReadFileToolOptions = {}): OpenAI.Chat.ChatCompletionTool {
	...
	return {
		type: "function",
		function: {
			// strict は付けない（非 strict 方針）。以前ここだけ strict:true を直書きしており、
			// required:["path"] のみ（プロパティ5個）で strict 要件（全 property が required）を
			// 満たさず、endpoint に 400 (no body) で弾かれてツール付きリクエストが全滅していた。
			name: "read_file",
			description,
			parameters: {
				type: "object",
