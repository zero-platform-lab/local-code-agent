...
describe("Task persistence", () => {
	...
	describe("saveApiConversationHistory", () => {
		it("returns true on success", async () => {
			...
			task.messageStore.apiConversationHistory.push({
				type: "message",
				role: "user",
		...
		it("snapshots the array before passing to saveApiMessages", async () => {
			...
			const originalMsg = {
				...
				role: "user" as const,
				content: [{ type: "input_text" as const, text: "snapshot test" }],
			}
			task.messageStore.apiConversationHistory.push(originalMsg)
			...
			expect(callArgs.messages).not.toBe(task.messageStore.apiConversationHistory)
			// But the content should be the same
			expect(callArgs.messages).toEqual(task.messageStore.apiConversationHistory)
		})
	})
	...
	describe("saveClineMessages", () => {
		...
		it("snapshots the array before passing to saveTaskMessages", async () => {
			...
			task.messageStore.clineMessages.push({
				type: "say",
				say: "text",
				text: "snapshot test",
				ts: Date.now(),
			...
			// The messages passed should be a COPY, not the live reference
			expect(callArgs.messages).not.toBe(task.messageStore.clineMessages)
			// But the content should be the same
			expect(callArgs.messages).toEqual(task.messageStore.clineMessages)
		})
	})
	...
	describe("flushPendingToolResultsToHistory persistence", () => {
		it("retains userMessageContent on save failure", async () => {
			...
			expect(task.stream.userMessageContent.length).toBeGreaterThan(0)
			expect(task.stream.userMessageContent[0]).toMatchObject({
				type: "tool_result",
				tool_use_id: "tool-fail",
			})
		})
		...
		it("clears userMessageContent on save success", async () => {
			...
			expect(task.stream.userMessageContent).toEqual([])
		})
	})
})
