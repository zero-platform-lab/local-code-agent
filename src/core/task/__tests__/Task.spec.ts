...
import type { AgentMessage, ToolResultBlockParam, TextBlockParam } from "@openai-agent/types"
import * as os from "os"
import * as path from "path"

import * as vscode from "vscode"

import type { GlobalState, ProviderSettings, ModelInfo } from "@openai-agent/types"
					...
					JSON.stringify([
						{
							type: "message",
							role: "user",
							content: [{ type: "input_text", text: "historical task" }],
							ts: Date.now(),
						},
						{
							type: "message",
							role: "assistant",
							content: [{ type: "input_text", text: "I'll help you with that task." }],
							ts: Date.now(),
						},
					]),
				)
...
describe("Cline", () => {
	...
	describe("constructor", () => {
		...
		it("should use default consecutiveMistakeLimit when not provided", () => {
			...
			expect(cline.mistakeTracker.limit).toBe(3)
		})
		...
		it("should respect provided consecutiveMistakeLimit", () => {
			...
			expect(cline.mistakeTracker.limit).toBe(5)
		})
		...
		it("should keep consecutiveMistakeLimit of 0 as 0 for unlimited", () => {
			...
			expect(cline.mistakeTracker.limit).toBe(0)
		})
		...
		it("initialTodos が渡されれば todoList に載せ、onCreated を呼ぶ", () => {
			const onCreated = vi.fn()
			const initialTodos = [{ id: "1", content: "do it", status: "pending" }] as never
			...
			expect(cline.todoList).toEqual(initialTodos)
			expect(onCreated).toHaveBeenCalledWith(cline)
		})

		it("空の initialTodos は todoList を設定しない", () => {
			const cline = new Task({
				provider: mockProvider,
				apiConfiguration: mockApiConfig,
				task: "test task",
			...
			expect(cline.todoList).toBeUndefined()
		})

		it("historyItem から構築すると mode 状態を履歴項目から取る", async () => {
			const cline = new Task({
				provider: mockProvider,
				apiConfiguration: mockApiConfig,
				historyItem: {
					...
					totalCost: 0,
					mode: "architect",
					apiConfigName: "profile-x",
				} as never,
				startTask: false,
			})

			expect(cline.modeState.mode).toBe("architect")
			expect(cline.modeState.apiConfigName).toBe("profile-x")
		})
	})
	...
	describe("launcher / attemptApiRequest wiring", () => {
		it("history 起動なら launcher.start() が resumeTaskFromHistory を呼ぶ", async () => {
			...
			await (cline as never as { launcher: { start(): Promise<void> } }).launcher.start()

			expect(resumeSpy).toHaveBeenCalled()
			resumeSpy.mockRestore()
		})

		it("attemptApiRequest は runAttemptApiRequest に委譲し、stream を最後まで流し切る", async () => {
			const cline = new Task({
				provider: mockProvider,
				apiConfiguration: mockApiConfig,
				task: "test task",
			...
			// 完了する async iterable を返す stream
			...
			// buildApiRequestDeps を差し替えて、重い依存を通さず generator の完走だけを見る
			const fakeDeps = {
				host: {
					api: {
						getModel: () => ({ id: "m", info: { contextWindow: 100000 } }),
						countTokens: vi.fn(async () => 1),
						createMessage: vi.fn(() => makeStream([{ type: "text", text: "hello" }])),
					},
					apiConfiguration: {},
					taskId: "t1",
					instanceId: "i1",
					...
					stream: { isWaitingForFirstChunk: false, currentRequestAbortController: undefined },
				},
				getProviderState: vi.fn(async () => ({ autoApprovalEnabled: true, mode: "code" })),
	...
	describe("getEnvironmentDetails", () => {
		describe("API conversation handling", () => {
			it.skip("should clean conversation history before sending to API", async () => {
				...
				const mockStreamForClean = (async function* () {
					yield { type: "input_text", text: "test response" }
				})()
				...
				cline.messageStore.apiConversationHistory = [
					{
						type: "message",
						role: "user" as const,
						content: [{ type: "input_text" as const, text: "test message" }],
						ts: Date.now(),
					},
				]
				...
				cline.messageStore.apiConversationHistory = [messageWithExtra]

				// Trigger an API request
				// userContent は依然として Anthropic 互換のブロック列（ContentBlockParam）で受ける。
				// item 形式になったのは保存側の会話履歴だけ。
			...
			it.skip("should handle image blocks based on model capabilities", async () => {
				...
				// 会話履歴は Responses item 形式。画像は data URL 化した input_image で持つ。
				...
				const taskWithImages = clineWithImages.start()
				...
				clineWithImages.messageStore.apiConversationHistory = conversationHistory
				...
				const taskWithoutImages = clineWithoutImages.start()
				...
				clineWithoutImages.messageStore.apiConversationHistory = conversationHistory
				...
				clineWithImages.messageStore.apiConversationHistory = [
					{
						type: "message",
						role: "user",
						content: [
				...
				if (imagesCalls[0]?.[1]?.[0]?.content) {
					...
					expect(imagesCalls[0][1][0].content[0]).toEqual({ type: "input_text", text: "Here is an image" })
				...
				if (noImagesCalls[0]?.[1]?.[0]?.content) {
					...
					expect(noImagesCalls[0][1][0].content[0]).toEqual({ type: "input_text", text: "Here is an image" })
			...
			it.skip("should handle API retry with countdown", async () => {
				...
				const mockSuccessStream = {
					async *[Symbol.asyncIterator]() {
						yield { type: "input_text", text: "Success" }
					},
					async next() {
						return { done: true, value: { type: "input_text", text: "Success" } }
					},
					async return() {
						return { done: true, value: undefined }
					},
				...
				cline.messageStore.clineMessages = [
					{
						ts: Date.now(),
						type: "say",
						say: "api_req_started",
			...
			describe("processUserContentMentions", () => {
				it("should process mentions in user_message tags", async () => {
					...
					// processUserContentMentions は保存前の userContent（ContentBlockParam 列）を扱うので
					// item 形式ではなく従来どおりの text / tool_result ブロックのまま。
					const userContent = [
						{
							type: "text",
					...
					expect((processedContent[0] as TextBlockParam).text).toBe(
						"Regular text with 'some/path' (see below for file content)",
					)

					// Text within user_message tags should be processed
					expect((processedContent[1] as TextBlockParam).text).toContain("processed:")
					expect((processedContent[1] as TextBlockParam).text).toContain(
					...
					const toolResult1 = processedContent[2] as ToolResultBlockParam
					const content1 = Array.isArray(toolResult1.content) ? toolResult1.content[0] : toolResult1.content
					expect((content1 as TextBlockParam).text).toContain("processed:")
					expect((content1 as TextBlockParam).text).toContain(
						"<user_message>Check 'some/path' (see below for file content)</user_message>",
					)

					// Regular tool result should not be processed
					const toolResult2 = processedContent[3] as ToolResultBlockParam
					const content2 = Array.isArray(toolResult2.content) ? toolResult2.content[0] : toolResult2.content
					expect((content2 as TextBlockParam).text).toBe(
		...
		describe("Subtask Rate Limiting", () => {
			...
			it("should enforce rate limiting across parent and subtask", async () => {
				...
				const mockStream = {
					async *[Symbol.asyncIterator]() {
						yield { type: "input_text", text: "parent response" }
					},
					async next() {
						return { done: true, value: { type: "input_text", text: "parent response" } }
					},
					async return() {
						return { done: true, value: undefined }
					},
				...
				const childMockStream = {
					async *[Symbol.asyncIterator]() {
						yield { type: "input_text", text: "child response" }
					},
					async next() {
						return { done: true, value: { type: "input_text", text: "child response" } }
					},
					async return() {
						return { done: true, value: undefined }
					},
			...
			it("should not apply rate limiting if enough time has passed", async () => {
				...
				const mockStream = {
					async *[Symbol.asyncIterator]() {
						yield { type: "input_text", text: "response" }
					},
					async next() {
						return { done: true, value: { type: "input_text", text: "response" } }
					},
					async return() {
						return { done: true, value: undefined }
					},
		...
		describe("getApiProtocol", () => {
			// Note: this build supports only the OpenAI Compatible provider, so the
			// former multi-provider protocol-detection cases (anthropic/openrouter and
			// undefined-provider, which buildApiHandler now rejects) no longer apply.
			it("builds a task with the OpenAI Compatible provider", async () => {
				const config = {
	...
	describe("abortTask", () => {
		...
		describe("cancelCurrentRequest", () => {
			it("should cancel the current HTTP request via AbortController", () => {
				...
				task.stream.currentRequestAbortController = mockAbortController
				...
				expect(task.stream.currentRequestAbortController).toBeUndefined()
			...
			it("should handle missing AbortController gracefully", () => {
				...
				task.stream.currentRequestAbortController = undefined
	...
	describe("委譲メソッドと getter", () => {
		...
		it("getSavedApiConversationHistory は messageStore.readApiConversationHistory へ委譲する", async () => {
			const task = makeTask()
			const hist = [{ type: "message", role: "user", content: [] }]
			;(task as any).messageStore.readApiConversationHistory = vi.fn().mockResolvedValue(hist)
			await expect(task.getSavedApiConversationHistory()).resolves.toBe(hist)
		})

		it("getSavedClineMessages は messageStore.readClineMessages へ委譲する", async () => {
			const task = makeTask()
			const msgs = [{ ts: 1 }]
			;(task as any).messageStore.readClineMessages = vi.fn().mockResolvedValue(msgs)
			await expect(task.getSavedClineMessages()).resolves.toBe(msgs)
		})

		it("overwriteClineMessages は clineMessages を差し替えて保存する", async () => {
			const task = makeTask()
			const saveSpy = vi.spyOn(task, "saveClineMessages").mockResolvedValue(true)
			const newMessages = [{ ts: 1, type: "say", say: "text", text: "a" }] as any
			await task.overwriteClineMessages(newMessages)
			expect((task as any).messageStore.clineMessages).toBe(newMessages)
			expect(saveSpy).toHaveBeenCalledTimes(1)
		})

		it("findMessageByTimestamp は messageStore へ委譲する", () => {
			const task = makeTask()
			const m = { ts: 42 }
			;(task as any).messageStore.findMessageByTimestamp = vi.fn().mockReturnValue(m)
			expect((task as any).findMessageByTimestamp(42)).toBe(m)
		...
		it("denyAsk は noButtonClicked で handleWebviewAskResponse を呼ぶ", () => {
			...
			task.denyAsk({ text: "no", images: ["i"] })
			expect(spy).toHaveBeenCalledWith("noButtonClicked", "no", ["i"])
		})

		it("supersedePendingAsk は askState.lastMessageTs を更新する", () => {
			const task = makeTask()
			;(task as any).askState.lastMessageTs = 0
			task.supersedePendingAsk()
			expect((task as any).askState.lastMessageTs).toBeGreaterThan(0)
		})

		it("updateApiConfiguration は config を差し替え api を再構築する", () => {
			const task = makeTask()
			const before = task.api
			const newCfg = { ...mockApiConfig, apiModelId: "different-model" }
			task.updateApiConfiguration(newCfg as any)
			expect(task.apiConfiguration).toBe(newCfg)
			expect(task.api).not.toBe(before)
		})

		it("handleTerminalOperation は continue/abort を terminalProcess へ委譲する", async () => {
			const task = makeTask()
			const cont = vi.fn()
			const abort = vi.fn()
			;(task as any).terminalProcess = { continue: cont, abort }
		...
		it("handleTerminalOperation は terminalProcess 不在でも throw しない", async () => {
			const task = makeTask()
			;(task as any).terminalProcess = undefined
			await expect(task.handleTerminalOperation("continue")).resolves.toBeUndefined()
		})

		it("sayAndCreateMissingParamError は say して欠落パラメータのエラー文字列を返す", async () => {
			const task = makeTask()
			const saySpy = vi.spyOn(task, "say").mockResolvedValue(undefined)
			const withPath = await task.sayAndCreateMissingParamError("read_file" as any, "path", "src/a.ts")
			expect(saySpy).toHaveBeenCalled()
			expect(String(withPath)).toContain("path")
			const noPath = await task.sayAndCreateMissingParamError("read_file" as any, "path")
			expect(String(noPath)).toContain("path")
		})

		it("postStateToWebviewWithoutTaskHistory は provider へ委譲する", () => {
			const task = makeTask()
			task.postStateToWebviewWithoutTaskHistory()
			expect(mockProvider.postStateToWebviewWithoutTaskHistory).toHaveBeenCalled()
		})

		it("maybeWaitForProviderRateLimit は apiRequestTiming へ委譲する", async () => {
			const task = makeTask()
			;(task as any).apiRequestTiming.maybeWaitForRateLimit = vi.fn().mockResolvedValue(undefined)
			await task.maybeWaitForProviderRateLimit(2)
			expect((task as any).apiRequestTiming.maybeWaitForRateLimit).toHaveBeenCalledWith(2)
		})

		it("backoffAndAnnounce は apiRequestTiming へ委譲する", async () => {
			const task = makeTask()
			;(task as any).apiRequestTiming.backoffAndAnnounce = vi.fn().mockResolvedValue(undefined)
			const err = new Error("x")
			await task.backoffAndAnnounce(1, err)
			expect((task as any).apiRequestTiming.backoffAndAnnounce).toHaveBeenCalledWith(1, err)
		})

		it("getCurrentProfileId は現在の profile id を返し、無ければ default", () => {
			const task = makeTask()
			const state = { currentApiConfigName: "prod", listApiConfigMeta: [{ name: "prod", id: "p1" }] }
			expect((task as any).getCurrentProfileId(state)).toBe("p1")
			expect((task as any).getCurrentProfileId({})).toBe("default")
			expect((task as any).getCurrentProfileId(undefined)).toBe("default")
		})

		it("combineMessages は command/api シーケンスを結合して返す", () => {
			const task = makeTask()
			const result = task.combineMessages([{ ts: 1, type: "say", say: "text", text: "hello" } as any])
			expect(Array.isArray(result)).toBe(true)
			expect(result[0]).toMatchObject({ text: "hello" })
		})

		it("taskStatus は interactive>resumable>idle>Running の優先順位で異なる状態を返す", () => {
			const task = makeTask()
			;(task as any).askState.interactiveAsk = undefined
			;(task as any).askState.resumableAsk = undefined
			;(task as any).askState.idleAsk = undefined
			const running = task.taskStatus
			;(task as any).askState.idleAsk = { ts: 1 }
			const idle = task.taskStatus
			;(task as any).askState.resumableAsk = { ts: 1 }
			const resumable = task.taskStatus
			;(task as any).askState.interactiveAsk = { ts: 1 }
			const interactive = task.taskStatus
			expect(new Set([running, idle, resumable, interactive]).size).toBe(4)
		})

		it("taskAsk / queuedMessages / tokenUsage getter が collaborator の値を返す", () => {
			const task = makeTask()
			// currentAsk は getter（idle>resumable>interactive）なので interactiveAsk 経由で設定
			;(task as any).askState.interactiveAsk = { ts: 9 }
			expect(task.taskAsk).toEqual({ ts: 9 })
			// queuedMessages は messageQueueService.messages（getter）を透過。空キューでは空配列
			expect(task.queuedMessages).toEqual([])
			;(task as any).tokenUsageTracker.getCachedTokenUsage = vi.fn().mockReturnValue({ totalTokensIn: 3 })
			expect(task.tokenUsage).toEqual({ totalTokensIn: 3 })
		})

		it("messageManager は lazy 初期化してキャッシュする（同一インスタンスを返す）", () => {
			const task = makeTask()
			const first = task.messageManager
			const second = task.messageManager
			expect(first).toBe(second)
		})

		it("initCheckpointService / processQueuedMessages / dispose は例外なく委譲する", () => {
			const task = makeTask()
			expect(() => task.initCheckpointService()).not.toThrow()
			expect(() => task.processQueuedMessages()).not.toThrow()
...
describe("pushToolResultToUserContent", () => {
	...
	it("should add tool_result when not a duplicate", () => {
		...
		expect(task.stream.userMessageContent).toHaveLength(1)
		expect(task.stream.userMessageContent[0]).toEqual(toolResult)
	})
	...
	it("should prevent duplicate tool_result with same tool_use_id", () => {
		...
		const toolResult1: ToolResultBlockParam = {
			type: "tool_result",
			tool_use_id: "duplicate-id",
			content: "First result",
		}

		const toolResult2: ToolResultBlockParam = {
			type: "tool_result",
			tool_use_id: "duplicate-id",
		...
		expect(task.stream.userMessageContent[0]).toEqual(toolResult1)
	...
	it("should allow different tool_use_ids to be added", () => {
		...
		expect(task.stream.userMessageContent).toHaveLength(2)
		expect(task.stream.userMessageContent[0]).toEqual(toolResult1)
		expect(task.stream.userMessageContent[1]).toEqual(toolResult2)
	})
	...
	it("should handle tool_result with is_error flag", () => {
		...
		expect(added).toBe(true)
		expect(task.stream.userMessageContent).toHaveLength(1)
		expect(task.stream.userMessageContent[0]).toEqual(errorResult)
	})
	...
	it("should not interfere with other content types in userMessageContent", () => {
		...
		task.stream.userMessageContent.push(
		...
		expect(added).toBe(true)
		expect(task.stream.userMessageContent).toHaveLength(3)
		expect(task.stream.userMessageContent[0].type).toBe("text")
		expect(task.stream.userMessageContent[1].type).toBe("image")
		expect(task.stream.userMessageContent[2]).toEqual(toolResult)
	})
})
