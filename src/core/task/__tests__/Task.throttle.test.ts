import { AgentEventName, ProviderSettings, TokenUsage, ToolUsage } from "@openai-agent/types"
...
describe("Task token usage throttling", () => {
	...
	test("should throttle subsequent emissions within 2 seconds", async () => {
		...
		const firstEmitCount = emitSpy.mock.calls.filter(
			(call) => call[0] === AgentEventName.TaskTokenUsageUpdated,
		).length
	...
	test("should include toolUsage in emission payload", async () => {
		...
		expect(emitSpy).toHaveBeenCalledWith(
			...
			task.tokenUsageTracker.toolUsage, // toolUsage
		)
	})
	...
	test("should force final emission on task abort", async () => {
		...
		const tokenUsageUpdateIndex = calls.findIndex((call) => call[0] === AgentEventName.TaskTokenUsageUpdated)
		const taskAbortedIndex = calls.findIndex((call) => call[0] === AgentEventName.TaskAborted)
	...
	test("should update tokenUsageSnapshot when throttled emission occurs", async () => {
		...
		const initialSnapshot = (task as any).tokenUsageTracker.tokenUsageSnapshot
		...
		expect((task as any).tokenUsageTracker.tokenUsageSnapshot).toBe(initialSnapshot)
		...
		expect((task as any).tokenUsageTracker.tokenUsageSnapshot).not.toBe(initialSnapshot)
		// Values should be different
		expect((task as any).tokenUsageTracker.tokenUsageSnapshot.totalTokensIn).toBeGreaterThan(
	...
	test("should update toolUsageSnapshot when emission occurs", async () => {
		...
		const initialSnapshot = (task as any).tokenUsageTracker.toolUsageSnapshot
		...
		const newSnapshot = (task as any).tokenUsageTracker.toolUsageSnapshot
	...
	test("emitFinalTokenUsageUpdate should emit on tool usage change alone", async () => {
		...
		;(task as any).tokenUsageTracker.tokenUsageSnapshot = task.tokenUsageTracker.getTokenUsage()
		;(task as any).tokenUsageTracker.toolUsageSnapshot = {}
