...
describe("Grace Retry Error Handling", () => {
	...
	describe("consecutiveNoAssistantMessagesCount", () => {
		it("should initialize to 0", () => {
			...
			expect(task.graceRetry.noAssistantMessages).toBe(0)
		})
		...
		it("should reset to 0 when abortTask is called", async () => {
			...
			task.graceRetry.noAssistantMessages = 5
		...
		it("should reset consecutiveNoToolUseCount when abortTask is called", async () => {
			...
			task.graceRetry.noAssistantMessages = 3
			task.graceRetry.noToolUse = 4
			...
			expect(task.graceRetry.noAssistantMessages).toBe(0)
			expect(task.graceRetry.noToolUse).toBe(0)
		})
	})
	...
	describe("Grace Retry Pattern", () => {
		it("should not show error on first failure (grace retry)", async () => {
			...
			task.graceRetry.noAssistantMessages = 0
			task.graceRetry.noAssistantMessages++
			expect(task.graceRetry.noAssistantMessages).toBe(1)

			// First failure: grace retry (silent)
			if (task.graceRetry.noAssistantMessages >= 2) {
		...
		it("should show error after 2 consecutive failures", async () => {
			...
			task.graceRetry.noAssistantMessages = 1
			task.graceRetry.noAssistantMessages++
			expect(task.graceRetry.noAssistantMessages).toBe(2)
		...
		it("should show error on third consecutive failure", async () => {
			...
			task.graceRetry.noAssistantMessages = 2
			task.graceRetry.noAssistantMessages++
			expect(task.graceRetry.noAssistantMessages).toBe(3)
	...
	describe("Parallel with noToolsUsed error handling", () => {
		it("should have separate counters for noToolsUsed and noAssistantMessages", () => {
			...
			task.graceRetry.noToolUse = 3
			expect(task.graceRetry.noAssistantMessages).toBe(0)

			task.graceRetry.noAssistantMessages = 2
			expect(task.graceRetry.noToolUse).toBe(3)
		})
	})
})
