...
Sources: [1](https://example.com), [2](https://another.com)
		...
		// addToApiConversationHistory は今も「1 ターン = {role, content}」で受け取り、
		// item 列への分割は内部で行う。
		...
		// 会話履歴は item 列なので、content を見る前に message item であることを確かめる。
		// テキストだけのターンは content を文字列に畳んで持つ。
		const stored = task.messageStore.apiConversationHistory[0]
		if (!isMessageItem(stored)) throw new Error(`expected a message item, got ${stored?.type}`)
		expect(stored.role).toBe("assistant")
		expect(stored.content).toBe("This is the main response content.")
	})
	...
	it("should not modify assistant message when no grounding sources are present", async () => {
		...
		const stored = task.messageStore.apiConversationHistory[0]
		if (!isMessageItem(stored)) throw new Error(`expected a message item, got ${stored?.type}`)
		expect(stored.content).toBe("This is a regular response without any sources.")
	})
