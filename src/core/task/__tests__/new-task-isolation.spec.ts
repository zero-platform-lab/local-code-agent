import type { ToolResultBlockParam, TextBlockParam, ToolUseBlockParam } from "@openai-agent/types"
/**
...
describe("new_task Tool Isolation Enforcement", () => {
	...
	const enforceNewTaskIsolation = (
		assistantContent: Array<TextBlockParam | ToolUseBlockParam>,
	): {
		truncatedContent: Array<TextBlockParam | ToolUseBlockParam>
		injectedToolResults: ToolResultBlockParam[]
	} => {
		const injectedToolResults: ToolResultBlockParam[] = []
	...
	describe("new_task as last tool (no truncation needed)", () => {
		it("should not truncate when new_task is the only tool", () => {
			const assistantContent: ToolUseBlockParam[] = [
				{
					type: "tool_use",
					id: "toolu_new_task_1",
					name: "new_task",
	...
	describe("new_task followed by other tools (truncation required)", () => {
		it("should truncate tools after new_task", () => {
			...
			expect((result.truncatedContent[0] as ToolUseBlockParam).name).toBe("new_task")
		})
		...
		it("should preserve tools before new_task", () => {
			...
			expect(result.truncatedContent).toHaveLength(2)
			expect((result.truncatedContent[0] as ToolUseBlockParam).name).toBe("read_file")
			expect((result.truncatedContent[1] as ToolUseBlockParam).name).toBe("new_task")
	...
	describe("Edge cases", () => {
		...
		it("should only consider the first new_task if multiple exist", () => {
			...
			expect(result.truncatedContent).toHaveLength(2)
			expect((result.truncatedContent[0] as ToolUseBlockParam).name).toBe("read_file")
			expect((result.truncatedContent[1] as ToolUseBlockParam).id).toBe("toolu_new_task_1")
