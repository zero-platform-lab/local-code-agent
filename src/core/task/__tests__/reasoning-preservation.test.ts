...
describe("Task reasoning preservation", () => {
	...
	it("should append reasoning to assistant message when preserveReasoning is true", async () => {
		...
		// 履歴は AgentMessage の item 列になり、text ブロックだけの本文は文字列 content に畳まれる。
		expect(task.messageStore.apiConversationHistory).toHaveLength(1)
		expect(itemText(task.messageStore.apiConversationHistory[0])).toContain("<think>")
		expect(itemText(task.messageStore.apiConversationHistory[0])).toContain("</think>")
		expect(itemText(task.messageStore.apiConversationHistory[0])).toContain("Here is my response to your question.")
		expect(itemText(task.messageStore.apiConversationHistory[0])).toContain(
	...
	it("should NOT append reasoning to assistant message when preserveReasoning is false", async () => {
		...
		expect(task.messageStore.apiConversationHistory).toHaveLength(1)
		expect(itemText(task.messageStore.apiConversationHistory[0])).toBe("Here is my response to your question.")
		expect(itemText(task.messageStore.apiConversationHistory[0])).not.toContain("<think>")
	})
	...
	it("should handle empty reasoning message gracefully when preserveReasoning is true", async () => {
		...
		expect(itemText(task.messageStore.apiConversationHistory[0])).toBe("Here is my response.")
		expect(itemText(task.messageStore.apiConversationHistory[0])).not.toContain("<think>")
	})
	...
	it("should store encrypted reasoning as a standalone item before the assistant message", async () => {
		const task = new Task({
			provider: mockProvider as ClineProvider,
			apiConfiguration: mockApiConfiguration,
			task: "Test task",
		...
		// Mock API handler to provide encrypted reasoning data
		task.api = {
		...
		// 暗号化 reasoning は assistant の content に埋め込むのではなく、Responses が要求する
		// 順序どおり「本文より前の独立 item」として積まれる。
		expect(task.messageStore.apiConversationHistory).toHaveLength(2)
		const [reasoningItem, messageItem] = task.messageStore.apiConversationHistory
	...
	it("should fold plain text reasoning into the start of the message body", async () => {
		const task = new Task({
			provider: mockProvider as ClineProvider,
			apiConfiguration: mockApiConfiguration,
			task: "Test task",
		...
		// 平文 reasoning は Responses の item にできないので、独立 item ではなく
		// 本文の先頭にテキストとして畳まれる（reasoning item は作られない）。
		expect(task.messageStore.apiConversationHistory).toHaveLength(1)
		const stored = task.messageStore.apiConversationHistory[0]
		...
		expect(stored.type).not.toBe("reasoning")
	})
})
