import type { MessageParam, ToolResultBlockParam, TextBlockParam } from "@openai-agent/types"
...
describe("validateAndFixToolResultIds", () => {
	...
	describe("直近 assistant ターンの手前に非 assistant item があるとき", () => {
		it("ターン境界で走査を止め、直近ターンの function_call だけと突き合わせる", () => {
			// 履歴末尾が assistant ターンだが、その手前に user メッセージ item がある。
			// lastAssistantTurnCalls は user item に当たった時点で break する（ターンをまたがない）。
			const history = [
				...messageToAgentItems("user", [{ type: "text", text: "前のターン" }]),
				...messageToAgentItems("assistant", [
			...
			const result = validateAndFixToolResultIds(userMessage, history)

			// call-1 の tool_result は既に正しいので、変更されずそのまま返る
			expect(result).toEqual(userMessage)
		})
	})
	...
	describe("when tool_result IDs match tool_use IDs", () => {
		it("should return the user message unchanged for single tool", () => {
			...
			const result = validateAndFixToolResultIds(
				userMessage,
				messageToAgentItems("assistant", assistantMessage.content),
			)
	...
	describe("when tool_result IDs do not match tool_use IDs", () => {
		it("should fix single mismatched tool_use_id by position", () => {
			...
			const resultContent = result.content as ToolResultBlockParam[]
	...
	describe("when user message has non-tool_result content", () => {
		it("should preserve text blocks alongside tool_result blocks", () => {
			...
			expect(Array.isArray(result.content)).toBe(true)
			const resultContent = result.content as Array<ToolResultBlockParam | TextBlockParam>
			expect(resultContent[0].type).toBe("tool_result")
			expect((resultContent[0] as ToolResultBlockParam).tool_use_id).toBe("tool-123")
			expect(resultContent[1].type).toBe("text")
			expect((resultContent[1] as TextBlockParam).text).toBe("Additional context")
		})
	})
	...
	describe("when there are more tool_results than tool_uses", () => {
		...
		it("should preserve text blocks while filtering orphaned tool_results", () => {
			...
			expect(resultContent[0].type).toBe("tool_result")
			expect((resultContent[0] as ToolResultBlockParam).tool_use_id).toBe("tool-1")
			expect(resultContent[1].type).toBe("text")
			expect((resultContent[1] as TextBlockParam).text).toBe("Some additional context")
		})
		...
		it("should preserve text blocks while deduplicating tool_results with same valid ID", () => {
			...
			expect(resultContent[0].type).toBe("tool_result")
			expect((resultContent[0] as ToolResultBlockParam).tool_use_id).toBe("tool-123")
			expect((resultContent[0] as ToolResultBlockParam).content).toBe("First result")
			expect(resultContent[1].type).toBe("text")
			expect((resultContent[1] as TextBlockParam).text).toBe("Environment details here")
		})
	})
	...
	describe("when tool_results are completely missing", () => {
		it("should add missing tool_result for single tool_use", () => {
			...
			expect(resultContent[0].type).toBe("tool_result")
			expect((resultContent[0] as ToolResultBlockParam).tool_use_id).toBe("tool-123")
			expect((resultContent[0] as ToolResultBlockParam).content).toBe(
		...
		it("should add missing tool_results for multiple tool_uses", () => {
			...
			expect((resultContent[1] as ToolResultBlockParam).tool_use_id).toBe("tool-2")
