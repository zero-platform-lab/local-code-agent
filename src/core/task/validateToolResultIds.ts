import type { AgentMessage, ContentBlockParam, MessageParam, ToolResultBlockParam } from "@openai-agent/types"
 ...
 * @param apiConversationHistory - 履歴（item 列）。末尾の assistant ターンにある
 *   function_call と突き合わせる
...
export function validateAndFixToolResultIds(
	...
	apiConversationHistory: AgentMessage[],
): MessageParam {
	...
	const toolUseBlocks = lastAssistantTurnCalls(apiConversationHistory)

	// No tool call to match against - no validation needed
	if (toolUseBlocks.length === 0) {
		return userMessage
	}

	// Find tool_result blocks in the user message
	let toolResults = userMessage.content.filter((block): block is ToolResultBlockParam => block.type === "tool_result")
	...
	toolResults = deduplicatedContent.filter((block): block is ToolResultBlockParam => block.type === "tool_result")
	...
	const contentArray = userMessage.content as ContentBlockParam[]
	...
	const correctedContent = contentArray
		.map((block: ContentBlockParam) => {
			...
			const toolResultIndex = toolResults.indexOf(block as ToolResultBlockParam)
	...
	const coveredToolUseIds = new Set(
		correctedContent
			.filter((b: ContentBlockParam): b is ToolResultBlockParam => b.type === "tool_result")
			.map((r: ToolResultBlockParam) => r.tool_use_id),
	)
	...
	const missingToolResults: ToolResultBlockParam[] = stillMissingToolUseIds.map((toolUse) => ({
...
/**
 * 履歴末尾の assistant ターンが出した function_call を、発行順に取り出す。
 *
 * item 列では 1 ターンが複数要素に割れるので、末尾から assistant ターンが続く間だけ
 * さかのぼる。ターンをまたいだ古い呼び出しとは突き合わせない（旧実装が
 * 「直前の assistant メッセージ」だけを見ていたのと同じ範囲）。
 */
function lastAssistantTurnCalls(history: AgentMessage[]): Array<{ id: string }> {
	const calls: Array<{ id: string }> = []
	for (let i = history.length - 1; i >= 0; i--) {
		const item = history[i]
		if (!isAssistantTurn(item)) break
		if (item.type === "function_call") calls.unshift({ id: item.call_id })
	}
	return calls
}
