...
import type { ToolUse, DiffStrategy } from "../../shared/tools"

import { BaseTool, ToolCallbacks } from "./BaseTool"
import type { ToolEditContext } from "./toolHost"

/** {@link ToolEditContext} plus ApplyDiff's extra members (diffStrategy / api / counters). */
interface ApplyDiffHost extends ToolEditContext {
	mistakeTracker: { count: number; applyDiffMisses: Map<string, number> }
	stream: {
		didToolFailInCurrentTurn: boolean
	}
	readonly api: { getModel(): { id: string } }
	readonly diffStrategy?: DiffStrategy
}
...
export class ApplyDiffTool extends BaseTool<"apply_diff"> {
	...
	async execute(params: ApplyDiffParams, task: ApplyDiffHost, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			if (!relPath) {
				...
				task.tokenUsageTracker.recordToolError("apply_diff")
			...
			if (!diffResult.success) {
				task.mistakeTracker.count++
				const currentCount = (task.mistakeTracker.applyDiffMisses.get(relPath) || 0) + 1
				task.mistakeTracker.applyDiffMisses.set(relPath, currentCount)
				let formattedError = ""
				...
				task.tokenUsageTracker.recordToolError("apply_diff", formattedError)
			...
			task.mistakeTracker.applyDiffMisses.delete(relPath)
	...
	override async handlePartial(task: ApplyDiffHost, block: ToolUse<"apply_diff">): Promise<void> {
