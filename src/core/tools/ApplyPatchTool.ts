...
import { type ClineSayTool, DEFAULT_WRITE_DELAY_MS, type Experiments } from "@openai-agent/types"
...
import type { ToolTaskContext, ToolTaskSay, ToolFileContextTracker, ToolDiffViewProvider } from "./toolHost"
...
interface ApplyPatchProviderLike {
	getState(): Promise<{ diagnosticsEnabled?: boolean; writeDelayMs?: number; experiments?: Experiments }>
}

/** Narrow structural view of `Task` required by {@link ApplyPatchTool}. */
interface ApplyPatchHost extends ToolTaskContext, ToolTaskSay, ToolFileContextTracker, ToolDiffViewProvider {
	didEditFile: boolean
	readonly cwd: string
	processQueuedMessages(): void
	readonly providerRef: WeakRef<ApplyPatchProviderLike>
...
export class ApplyPatchTool extends BaseTool<"apply_patch"> {
	...
	async execute(params: ApplyPatchParams, task: ApplyPatchHost, callbacks: ToolCallbacks): Promise<void> {
		const { patch } = params
		...
		try {
			...
			if (!patch) {
				...
				task.tokenUsageTracker.recordToolError("apply_patch")
			...
			task.tokenUsageTracker.recordToolUsage("apply_patch")
		} catch (error) {
	...
	override async handlePartial(task: ApplyPatchHost, block: ToolUse<"apply_patch">): Promise<void> {
