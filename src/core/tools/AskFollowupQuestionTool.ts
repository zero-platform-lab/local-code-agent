...
/** Narrow structural view of `Task` required by {@link AskFollowupQuestionTool}. */
interface AskFollowupQuestionHost extends ToolTaskContext, ToolTaskSay {
	stream: {
...
export class AskFollowupQuestionTool extends BaseTool<"ask_followup_question"> {
	...
	async execute(
		...
		const recordMissingParamError = async (paramName: string): Promise<void> => {
			...
			task.tokenUsageTracker.recordToolError("ask_followup_question")
