...
/**
 * Narrow structural view of `Task` used by {@link AttemptCompletionTool}.
 * Avoids importing the concrete `Task` (which would create a module cycle);
 * `Task` satisfies this structurally and method-parameter bivariance lets us
 * override `BaseTool`'s `task: Task` with this wider type.
 */
interface AttemptCompletionHost extends ToolTaskContext, ToolTaskSay {
	...
	readonly tokenUsageTracker: ToolTaskContext["tokenUsageTracker"] & {
		readonly toolUsage: ToolUsage
		getTokenUsage(): TokenUsage
		emitFinal(): void
	}
	emit(event: AgentEventName.TaskCompleted, taskId: string, tokenUsage: TokenUsage, toolUsage: ToolUsage): boolean
}
...
export class AttemptCompletionTool extends BaseTool<"attempt_completion"> {
	...
	async execute(
		...
		if (task.stream.didToolFailInCurrentTurn) {
		...
		if (preventCompletionWithOpenTodos && hasIncompleteTodos) {
			...
			task.tokenUsageTracker.recordToolError("attempt_completion")
	...
	override async handlePartial(task: AttemptCompletionHost, block: ToolUse<"attempt_completion">): Promise<void> {
		const result: string | undefined = block.params.result
		const command: string | undefined = block.params.command

		const lastMessage = task.messageStore.clineMessages.at(-1)

		if (command) {
			// `if (command)` により command は非空 string へ絞り込み済み。
			// `command ?? ""` の右辺は到達不能なので素の command を渡す。
	...
	private emitTaskCompleted(task: AttemptCompletionHost): void {
		// Force final token usage update before emitting TaskCompleted.
		// This ensures the latest stats are captured regardless of throttle timer.
		task.tokenUsageTracker.emitFinal()
		task.emit(
			AgentEventName.TaskCompleted,
			task.taskId,
			task.tokenUsageTracker.getTokenUsage(),
			task.tokenUsageTracker.toolUsage,
		)
	}
}
