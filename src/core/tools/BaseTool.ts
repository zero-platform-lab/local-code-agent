 ...
 *
 * The `task` handed to a tool is typed via the `THost` parameter rather than the
 * concrete `Task`, so this base does not depend on the task layer (which would
 * otherwise create a module cycle: task → tools → BaseTool → task). Each tool
 * supplies a narrow structural host (e.g. `EditFileHost`) that `Task` satisfies;
 * tools that have not been narrowed yet default to `unknown` and keep their own
 * `task: Task` override (method-parameter bivariance permits both).
 *
 * @template TName - The specific tool name, which determines native arg types
 * @template THost - The `task` surface this tool needs (defaults to `unknown`)
 */
export abstract class BaseTool<TName extends ToolName, THost = unknown> {
	/**
	 ...
	 */
	abstract execute(params: ToolParams<TName>, task: THost, callbacks: ToolCallbacks): Promise<void>
	 ...
	 */
	async handlePartial(_task: THost, _block: ToolUse<TName>): Promise<void> {
	 ...
	 */
	async handle(task: THost, block: ToolUse<TName>, callbacks: ToolCallbacks): Promise<void> {
