...
/** Narrow structural view of `Task` required by {@link CodebaseSearchTool}. */
interface CodebaseSearchHost extends ToolTaskContext, ToolTaskSay {
	stream: {
		didToolFailInCurrentTurn: boolean
	}
	readonly cwd: string
	readonly providerRef: WeakRef<{ readonly context: vscode.ExtensionContext }>
}

export class CodebaseSearchTool extends BaseTool<"codebase_search"> {
	readonly name = "codebase_search" as const

	async execute(params: CodebaseSearchParams, task: CodebaseSearchHost, callbacks: ToolCallbacks): Promise<void> {
...
`,
	...
	override async handlePartial(task: CodebaseSearchHost, block: ToolUse<"codebase_search">): Promise<void> {
