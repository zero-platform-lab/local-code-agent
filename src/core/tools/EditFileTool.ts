...
import type { ToolEditContext } from "./toolHost"

/** {@link ToolEditContext} plus the per-turn/edit counters EditFile reads. */
interface EditFileHost extends ToolEditContext {
	mistakeTracker: { count: number; editFileMisses: Map<string, number> }
	stream: {
...
export function buildWhitespaceTolerantRegex(oldLF: string): RegExp {
	...
	// oldLF は非空が保証されるので `/(\s+|\S+)/g` は必ず 1 つ以上マッチする（null にならない）。
...
export class EditFileTool extends BaseTool<"edit_file"> {
	...
	async execute(params: EditFileParams, task: EditFileHost, callbacks: ToolCallbacks): Promise<void> {
		...
		const recordFailureForPathAndMaybeEscalate = async (relPath: string, formattedError: string): Promise<void> => {
			const currentCount = (task.mistakeTracker.editFileMisses.get(relPath) || 0) + 1
			task.mistakeTracker.editFileMisses.set(relPath, currentCount)
		...
		try {
			...
			if (!file_path) {
				...
				task.tokenUsageTracker.recordToolError("edit_file")
			...
			if (fileExists) {
				try {
					...
					task.tokenUsageTracker.recordToolError("edit_file", formattedError)
			...
			// !isNewFile の分岐に入るのは「ファイルが存在し読み込めた」場合だけなので、
			// currentContentLF は必ず文字列（`?? currentContent ?? ""` の fallback は到達不能）。
			const newContent = isNewFile ? new_string : restoreLineEnding(currentContentLF!, originalEol)

			// Check if any changes were made
			if (!isNewFile && newContent === currentContent) {
				if (relPathForErrorHandling) {
					task.mistakeTracker.count = 0
					task.mistakeTracker.editFileMisses.delete(relPathForErrorHandling)
				}
				await finalizePartialToolAskIfNeeded(relPath)
			...
			task.mistakeTracker.count = 0
			task.mistakeTracker.editFileMisses.delete(relPath)
			...
			task.tokenUsageTracker.recordToolUsage("edit_file")
			await task.diffViewProvider.reset()
	...
	override async handlePartial(task: EditFileHost, block: ToolUse<"edit_file">): Promise<void> {
