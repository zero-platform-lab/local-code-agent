...
interface EditProviderLike {
	getState(): Promise<{ diagnosticsEnabled?: boolean; writeDelayMs?: number; experiments?: Experiments }>
}

/** Narrow structural view of `Task` required by {@link EditTool}. */
interface EditHost extends ToolTaskContext, ToolTaskSay, ToolFileContextTracker, ToolDiffViewProvider {
	didEditFile: boolean
	readonly cwd: string
	processQueuedMessages(): void
	readonly providerRef: WeakRef<EditProviderLike>
...
export class EditTool extends BaseTool<"edit"> {
	...
	async execute(params: EditParams, task: EditHost, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			if (!relPath) {
				...
				task.tokenUsageTracker.recordToolError("edit")
			...
			if (matchCount === 0) {
				task.mistakeTracker.count++
				task.tokenUsageTracker.recordToolError("edit", "no_match")
				pushToolResult(
					formatResponse.toolError(
			...
			task.tokenUsageTracker.recordToolUsage("edit")
			await task.diffViewProvider.reset()
	...
	override async handlePartial(task: EditHost, block: ToolUse<"edit">): Promise<void> {
