...
/** Provider surface read by execute_command via `providerRef.deref()`. */
interface ExecuteCommandProvider {
	getState(): Promise<{
		terminalShellIntegrationDisabled?: boolean
		terminalOutputPreviewSize?: TerminalOutputPreviewSize
	}>
	postMessageToWebview(message: unknown): unknown
	readonly context?: { readonly globalStorageUri?: { readonly fsPath?: string } }
}

/**
 * Narrow structural view of `Task` used by {@link ExecuteCommandTool} and the
 * module-level {@link executeCommandInTerminal}. Avoids importing the concrete
 * `Task` (which would create a module cycle); `Task` satisfies this structurally
 * and method-parameter bivariance lets us override `BaseTool`'s `task: Task`.
 */
interface ExecuteCommandHost extends ToolTaskContext, ToolTaskSay {
	readonly cwd: string
	readonly taskId: string
	readonly askState: { lastMessageTs?: number }
	stream: {
...
export class ExecuteCommandTool extends BaseTool<"execute_command"> {
	...
	async execute(params: ExecuteCommandParams, task: ExecuteCommandHost, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			if (!command) {
				...
				task.tokenUsageTracker.recordToolError("execute_command")
			...
			const executionId = task.askState.lastMessageTs?.toString() ?? Date.now().toString()
	...
	override async handlePartial(task: ExecuteCommandHost, block: ToolUse<"execute_command">): Promise<void> {
...
export async function executeCommandInTerminal(
	...
	const onCompletedPromise: Promise<void> = new Promise((resolve) => {
