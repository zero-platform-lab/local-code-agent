...
interface ReadCommandOutputProviderLike {
	readonly context: { readonly globalStorageUri: { readonly fsPath: string } }
}

/** Narrow structural view of `Task` required by {@link ReadCommandOutputTool}. */
interface ReadCommandOutputHost extends ToolTaskContext, ToolTaskSay {
	stream: {
		didToolFailInCurrentTurn: boolean
	}
	readonly taskId: string
	readonly providerRef: WeakRef<ReadCommandOutputProviderLike>
}
...
export class ReadCommandOutputTool extends BaseTool<"read_command_output"> {
	...
	async execute(
		...
		if (!artifact_id) {
			...
			task.tokenUsageTracker.recordToolError("read_command_output")
	...
	private async searchInArtifact(
		...
		try {
			while (bytesRead < totalSize && !hitLimit) {
				...
				/* v8 ignore next -- 到達不能: split("\n") は必ず 1 要素以上を返すため pop() が undefined にならず、?? "" の既定側は踏めない（作者が書いた防御既定・安全のため残す） */
