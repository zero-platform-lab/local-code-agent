...
/** Narrow structural view of `Task` required by {@link RunSlashCommandTool}. */
interface RunSlashCommandHost extends ToolTaskContext {
	stream: {
		didToolFailInCurrentTurn: boolean
	}
	readonly cwd: string
	readonly providerRef: WeakRef<RunSlashCommandProviderLike>
}

export class RunSlashCommandTool extends BaseTool<"run_slash_command"> {
	readonly name = "run_slash_command" as const

	async execute(params: RunSlashCommandParams, task: RunSlashCommandHost, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			if (!commandName) {
				...
				task.tokenUsageTracker.recordToolError("run_slash_command")
	...
	override async handlePartial(task: RunSlashCommandHost, block: ToolUse<"run_slash_command">): Promise<void> {
