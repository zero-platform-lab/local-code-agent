...
export class SearchReplaceTool extends BaseTool<"search_replace"> {
	...
	async execute(params: SearchReplaceParams, task: ToolEditContext, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			if (!file_path) {
				...
				task.tokenUsageTracker.recordToolError("search_replace")
			...
			if (matchCount === 0) {
				task.mistakeTracker.count++
				task.tokenUsageTracker.recordToolError("search_replace", "no_match")
				pushToolResult(
			...
			if (matchCount > 1) {
				task.mistakeTracker.count++
				task.tokenUsageTracker.recordToolError("search_replace", "multiple_matches")
				pushToolResult(
					formatResponse.toolError(
			...
			task.tokenUsageTracker.recordToolUsage("search_replace")
			await task.diffViewProvider.reset()
	...
	override async handlePartial(task: ToolEditContext, block: ToolUse<"search_replace">): Promise<void> {
