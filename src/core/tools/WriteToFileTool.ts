...
/** {@link ToolEditContext} plus the `api.getModel().id` check WriteToFile does. */
interface WriteToFileHost extends ToolEditContext {
...
export class WriteToFileTool extends BaseTool<"write_to_file"> {
	...
	async execute(params: WriteToFileParams, task: WriteToFileHost, callbacks: ToolCallbacks): Promise<void> {
		...
		if (!relPath) {
			...
			task.tokenUsageTracker.recordToolError("write_to_file")
		...
		// relPath は上の !relPath ガードで非空が保証されるので、L66 で計算済みの
		// absolutePath をそのまま使う（`relPath ? … : ""` の else は到達不能なため除去）。
	...
	override async handlePartial(task: WriteToFileHost, block: ToolUse<"write_to_file">): Promise<void> {
