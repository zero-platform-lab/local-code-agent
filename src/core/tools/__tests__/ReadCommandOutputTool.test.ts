...
describe("ReadCommandOutputTool", () => {
	...
	beforeEach(() => {
		...
		// Mock task object — ReadCommandOutputTool awaits providerRef.deref() so it needs
		// mockResolvedValue rather than the factory default mockReturnValue.
		mockTask = makeMockTask() as any
		mockTask.taskId = taskId
		mockTask.sayAndCreateMissingParamError = vi.fn().mockResolvedValue("Missing parameter")
	...
	describe("Error handling", () => {
		...
		it("should reject invalid artifact_id with path traversal attempt", async () => {
			...
			for (const invalidId of invalidIds) {
				...
				expect(mockTask.mistakeTracker!.count).toBeGreaterThan(0)
				expect(mockTask.stream.didToolFailInCurrentTurn).toBe(true)
		...
		it("should accept valid artifact_id format", async () => {
			...
			expect(mockTask.stream.didToolFailInCurrentTurn).toBe(false)
		})
		...
		it("should handle missing artifact_id parameter", async () => {
			...
			expect(mockTask.mistakeTracker!.count).toBeGreaterThan(0)
			expect(mockTask.tokenUsageTracker.recordToolError).toHaveBeenCalledWith("read_command_output")
			expect(mockTask.stream.didToolFailInCurrentTurn).toBe(true)
	...
	// 以下は既存スイートで未到達だったストリーミング境界を埋める。
	// 守るのは「巨大出力を有界メモリで走査する」実装の分岐が正しく閉じること。
	describe("Streaming edge cases", () => {
		const artifactId = "cmd-1706119234567.txt"

		/** position 指定に応じて content の一部を返す汎用ストリーミング read。 */
		const streamRead = (content: string) => {
			const b = Buffer.from(content, "utf8")
			vi.mocked(fs.stat).mockResolvedValue({ size: b.length } as any)
			mockFileHandle.read.mockImplementation((buf: Buffer, _off: number, len: number, pos: number | null) => {
				const p = pos ?? 0
				if (p >= b.length) {
					return Promise.resolve({ bytesRead: 0 })
				}
				const n = Math.min(len, b.length - p)
				b.copy(buf, 0, p, p + n)
				return Promise.resolve({ bytesRead: n })
			})
		}

		it("末尾に改行が無い最終行もマッチ対象になる（partialLine の後処理）", async () => {
			// "beta" は改行で終わらない最終行。split で残る partialLine 経路を通す
			streamRead("alpha\nbeta")

			await tool.execute({ artifact_id: artifactId, search: "beta" }, mockTask, mockCallbacks)

			const result = mockCallbacks.pushToolResult.mock.calls[0][0]
			expect(result).toContain("Total matches: 1")
			expect(result).toMatch(/2 \|.*beta/)
		})

		it("マッチ行が limit を超えたら打ち切る（hitLimit）", async () => {
			// 最初のマッチ行だけで limit を超えるので、その行すら積まずに停止
			streamRead("errorLongMatchingLine\nanother error here\n")

			await tool.execute({ artifact_id: artifactId, search: "error", limit: 5 }, mockTask, mockCallbacks)

			const result = mockCallbacks.pushToolResult.mock.calls[0][0]
			expect(result).toContain("No matches found for the search pattern")
		})

		it("stat より read が早く尽きても（0 バイト）検索は安全に終わる", async () => {
			// stat は 100 バイトと言うが read は即 0 を返す＝途中打ち切り
			vi.mocked(fs.stat).mockResolvedValue({ size: 100 } as any)
			mockFileHandle.read.mockResolvedValue({ bytesRead: 0 })

			await tool.execute({ artifact_id: artifactId, search: "anything" }, mockTask, mockCallbacks)
		...
		it("offset の行番号計算で prefix read が 0 バイトでも落ちない", async () => {
			// offset より前の read が 0 を返す＝行番号は 1 として続行
			vi.mocked(fs.stat).mockResolvedValue({ size: 50 } as any)
			mockFileHandle.read.mockImplementation((buf: Buffer, _off: number, _len: number, pos: number | null) => {
				if (pos === 10) {
					buf.write("CONTENT")
					return Promise.resolve({ bytesRead: 7 })
				}
				// prefix（pos < offset）の読み取りは 0 バイト
				return Promise.resolve({ bytesRead: 0 })
			})
			...
			const result = mockCallbacks.pushToolResult.mock.calls[0][0]
			expect(result).toContain("Showing bytes 10-17")
			expect(result).toMatch(/1 \| CONTENT/)
		})

		it("read が Error 以外で失敗しても String 化して原因を返す", async () => {
			// ネイティブ層由来の非 Error 拒否値でも握りつぶさない
			mockFileHandle.read.mockRejectedValue("EIO-non-error-string")

			await tool.execute({ artifact_id: artifactId }, mockTask, mockCallbacks)

			expect(mockTask.stream.didToolFailInCurrentTurn).toBe(true)
			expect(mockTask.say).toHaveBeenCalledWith("error", expect.stringContaining("EIO-non-error-string"))
			expect(mockCallbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("EIO-non-error-string"))
		})
	})
