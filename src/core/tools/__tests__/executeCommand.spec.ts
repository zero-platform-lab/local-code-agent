...
import type { AgentTerminalCallbacks } from "../../../integrations/terminal/types"
...
describe("executeCommand", () => {
	...
	beforeEach(() => {
		...
		// Create mock task via shared factory; executeCommandInTerminal awaits providerRef.deref()
		// so it needs mockResolvedValue rather than the factory default mockReturnValue.
		mockTask = makeMockTask({ cwd: "/test/project" }) as any
		mockTask.taskId = "test-task-123"
	...
	describe("Working Directory Behavior", () => {
		it("should use terminal.getCurrentWorkingDirectory() in the output message for completed commands", async () => {
			...
			mockTerminal.runCommand.mockImplementation((command: string, callbacks: AgentTerminalCallbacks) => {
		...
		it("should use terminal.getCurrentWorkingDirectory() for VSCode Terminal with shell integration", async () => {
			...
			mockVSCodeTerminal.runCommand = vitest
				.fn()
				.mockImplementation((command: string, callbacks: AgentTerminalCallbacks) => {
					setTimeout(() => {
						callbacks.onCompleted("Command output", mockProcess)
						callbacks.onShellExecutionComplete({ exitCode: 0 }, mockProcess)
					}, 0)
	...
	describe("Terminal Working Directory Updates", () => {
		it("should update working directory when terminal returns different cwd", async () => {
			...
			const mockTerminalInstance = {
				...
				runCommand: vitest.fn().mockImplementation((command: string, callbacks: AgentTerminalCallbacks) => {
					setTimeout(() => {
