import type { ClineAskUseMcpServer, McpResourceResponse } from "@openai-agent/types"
...
interface McpResourceProviderLike {
	getMcpHub(): { readResource(serverName: string, uri: string): Promise<McpResourceResponse> } | undefined
}

/** Narrow structural view of `Task` required by {@link AccessMcpResourceTool}. */
interface AccessMcpResourceHost extends ToolTaskContext, ToolTaskSay {
	readonly providerRef: WeakRef<McpResourceProviderLike>
}
...
export class AccessMcpResourceTool extends BaseTool<"access_mcp_resource"> {
	...
	async execute(
		...
		try {
			if (!server_name) {
				...
				task.tokenUsageTracker.recordToolError("access_mcp_resource")
	...
	override async handlePartial(task: AccessMcpResourceHost, block: ToolUse<"access_mcp_resource">): Promise<void> {
