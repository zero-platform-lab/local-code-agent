...
*** End Patch`)
		...
		it("should reject an unknown hunk header", () => {
			// Add/Delete/Update 以外のヘッダは安全に ParseError で拒否する。
			expect(() =>
				parsePatch(`*** Begin Patch
*** Rename File: x.py
*** End Patch`),
...
*** Rename File: x.py
*** End Patch`),
			).toThrow("is not a valid hunk header")
		})

		it("should reject a non-first chunk that is missing its @@ context marker", () => {
			// 2 番目以降のチャンクは @@ 文脈行が必須。欠けていれば拒否する。
			expect(() =>
				parsePatch(`*** Begin Patch
*** Update File: f.py
 ctx
...
notacontext
*** End Patch`),
			).toThrow("Expected update hunk to start with a @@ context marker")
		})

		it("should reject an update chunk whose @@ context has no following lines", () => {
			// @@ 文脈行だけで本文が無いチャンクは拒否する。
			expect(() =>
				parsePatch(`*** Begin Patch
*** Update File: f.py
@@ ctx
*** End Patch`),
		...
		it("should reject an update chunk that starts with the End of File marker", () => {
			// @@ の直後がいきなり End of File マーカーで本文ゼロのケースを拒否する。
			expect(() =>
				parsePatch(`*** Begin Patch
*** Update File: f.py
@@ ctx
...
*** End Patch`),
		...
		it("should reject an update chunk whose first body line is not context/+/-", () => {
			// 先頭本文が ' '/'+'/'-' のいずれでもない行なら拒否する(未パースの状態)。
			expect(() =>
				parsePatch(`*** Begin Patch
*** Update File: f.py
garbage line
*** End Patch`),
		...
		it("should treat a blank line inside a chunk as a context line", () => {
			// チャンク本文の途中の空行は old/new 双方に空文字の文脈行として取り込む。
			const result = parsePatch(`*** Begin Patch
*** Update File: f.py
@@
 foo
...
*** End Patch`)
		...
		it("should skip a blank line that appears before the first chunk", () => {
			// Update File ヘッダ直後の空行はチャンク間の区切りとして読み飛ばす。
			const result = parsePatch(`*** Begin Patch
*** Update File: f.py
