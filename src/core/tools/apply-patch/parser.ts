...
function checkPatchBoundaries(lines: string[]): void {
	/* v8 ignore next -- 到達不能: parsePatch は split 結果(常に1要素以上)か heredoc スライス(2要素以上)しか渡さず空配列に到達しない防御ガード。分岐と throw を同一行に置き単一 ignore で除外する */
	if (lines.length === 0) throw new ParseError("Empty patch")
...
function parseUpdateFileChunk(
	...
	/* v8 ignore next -- 到達不能: parseOneHunk は while(remainingLines.length>0) 内でのみ呼び出すため空配列に到達しない防御ガード。分岐と throw を同一行に置き単一 ignore で除外する */
	if (lines.length === 0) throw new ParseError("Update hunk does not contain any lines", lineNumber)
