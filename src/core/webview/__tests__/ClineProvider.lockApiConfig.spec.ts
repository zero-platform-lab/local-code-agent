...
// Task モジュールは transitive に vscode API を触る（DiffViewProvider 等）ため、この spec
// の限定的な vscode mock では load できない。ClineProvider は既定 factory 内で `new Task(opts)`
// を評価するが、テストは `taskFactory` 引数（下記 fakeTaskFactory）を注入するのでその経路は
// 通らない。よって Task class 本体は「呼ばれたら明示 error を投げる stub」にしておく。
vi.mock("../../task/Task", () => ({
	Task: class {
		constructor() {
...
// Task 構築の fake は ClineProvider の taskFactory 引数で注入する（下記 fakeTaskFactory）
const fakeTaskFactory = (options: any) => ({
	...
	modeState: { apiConfigName: options.historyItem?.apiConfigName },
})
