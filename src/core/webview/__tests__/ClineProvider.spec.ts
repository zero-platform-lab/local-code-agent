...
// Task モジュールは transitive vscode API に触るため、この spec の vscode mock だけでは load
// 中に落ちる。ClineProvider は既定 factory 内で real Task を new するが、テストは taskFactory
// 引数（fakeTaskFactory）を注入するので実 Task は構築されない。Task class は「呼ばれたら error」
// の stub にし、fake surface は fakeTaskFactory に集約。
vi.mock("../../task/Task", () => ({
	Task: class {
		constructor() {
			throw new Error("real Task should not be constructed in this spec; use fakeTaskFactory instead")
		}
	},
}))

const fakeTaskFactory = (options: any) => {
	const task: any = {
		api: undefined,
		abortTask: vi.fn(),
...
describe("ClineProvider", () => {
	...
	beforeEach(() => {
		...
		// contextProxy は public なので抑制不要（以前は private 前提のコメントだった）。
	...
	describe("clearTask message handler", () => {
		...
		test("calls clearTask (delegation handled via metadata)", async () => {
			...
			const mockCline = fakeTaskFactory(defaultTaskOptions)
		...
		test("calls clearTask even with parent task (delegation via metadata)", async () => {
			...
			const parentTask = fakeTaskFactory(defaultTaskOptions)
			const childTask = fakeTaskFactory(defaultTaskOptions)
	...
	test("addClineToStack adds multiple Cline instances to the stack", async () => {
		// Setup Cline instance with auto-mock from the top of the file
		const mockCline1 = fakeTaskFactory(defaultTaskOptions) // Create a new mocked instance
		const mockCline2 = fakeTaskFactory(defaultTaskOptions) // Create a new mocked instance
	...
	describe("deleteMessage", () => {
		...
		test("handles deletion with confirmation dialog", async () => {
			...
			const mockApiHistory = [
				...
				{ ts: 5000 },
				{ ts: 6000 },
			] as (MessageParam & { ts?: number })[]

			// Setup Task instance with auto-mock from the top of the file
			const mockCline = fakeTaskFactory(defaultTaskOptions) // Create a new mocked instance
			mockCline.messageStore.clineMessages = mockMessages // Set test-specific messages
			mockCline.messageStore.apiConversationHistory = mockApiHistory // Set API history
...
describe("ClineProvider - Comprehensive Edit/Delete Edge Cases", () => {
	...
	describe("Edit Messages with Images and Attachments", () => {
		...
		test("handles editing messages containing images", async () => {
			...
			const mockCline = fakeTaskFactory(defaultTaskOptions)
			mockCline.messageStore.clineMessages = mockMessages
			mockCline.messageStore.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }, { ts: 3000 }] as any[]
			mockCline.overwriteClineMessages = vi.fn()
			mockCline.overwriteApiConversationHistory = vi.fn()
	...
	describe("Concurrent Edit Operations", () => {
		...
		test("handles race conditions with simultaneous edits", async () => {
			...
			mockCline.messageStore.apiConversationHistory = [
				{ ts: 1000 },
				{ ts: 2000 },
				{ ts: 3000 },
				{ ts: 4000 },
	...
	describe("Edit Permissions and Authorization", () => {
		...
		describe("Operations on Deleted or Non-existent Messages", () => {
			...
			test("handles edit operations on deleted messages", async () => {
				...
				mockCline.messageStore.apiConversationHistory = [{ ts: 1000 }] as any[]
				mockCline.overwriteClineMessages = vi.fn()
				mockCline.overwriteApiConversationHistory = vi.fn()
				mockCline.handleWebviewAskResponse = vi.fn()
