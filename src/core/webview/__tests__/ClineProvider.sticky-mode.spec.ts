...
// Task モジュールは transitive vscode API に触るため、この spec の限定的な vscode mock では
// load 中に落ちる。ClineProvider は既定 factory 内で real Task を new するが、テストは
// `taskFactory` 引数（`fakeTaskFactory`）を注入するので実 Task は構築されない。よって Task
// class は「呼ばれたら明示 error」の stub にし、fake surface は fakeTaskFactory に集約する
// （spec 内で直接 fake を作る箇所も全て fakeTaskFactory 経由）。
vi.mock("../../task/Task", () => ({
	Task: class {
		constructor() {
...
const fakeTaskFactory = (options: any): Task =>
	({
		...
		// mode の所有者は Task の modeState collaborator（sticky mode の書き込み先）。
		modeState: { mode: options.historyItem?.mode },
	}) as unknown as Task
...
describe("ClineProvider - Sticky Mode", () => {
	...
	describe("handleModeSwitch", () => {
		...
		it("should save mode to task metadata when switching modes", async () => {
			// Create a mock task
			const mockTask = fakeTaskFactory({
				provider,
		...
		it("should update task's taskMode property when switching modes", async () => {
			...
			const mockTask = {
				taskId: "test-task-id",
				modeState: { mode: "code" }, // Initial mode
				emit: vi.fn(),
				saveClineMessages: vi.fn(),
				messageStore: {
					clineMessages: [],
			...
			// Verify task の mode が modeState 経由で更新されたことを確認
			expect(mockTask.modeState.mode).toBe("architect")
	...
	describe("Integration with new_task tool", () => {
		it("should preserve parent task mode when creating subtasks", async () => {
			...
			// Create parent task
			const parentTask = fakeTaskFactory({
				provider,
				apiConfiguration: { apiProvider: "openai" },
			})
	...
	describe("Concurrent mode switches", () => {
		it("should handle concurrent mode switches on the same task", async () => {
			...
			// mode 切替は sticky provider profile の永続化（mode を含まない更新）も伴うため、
			// 「最後の更新」ではなく「mode を書いた更新のうち最後のもの」を見る。
			const modeHistoryCalls = updateTaskHistorySpy.mock.calls.filter((call) => "mode" in (call[0] ?? {}))
			expect(modeHistoryCalls[modeHistoryCalls.length - 1][0]).toMatchObject({
				id: mockTask.taskId,
				mode: "code",
			})
		})
	...
	describe("Mode switch failure scenarios", () => {
		...
		it("should handle errors during mode switch gracefully", async () => {
			...
			// Since the error is thrown before updating the task mode,
			...
			expect(mockTask.modeState.mode).toBe("code")
	...
	describe("Multiple tasks switching modes simultaneously", () => {
		it("should handle multiple tasks switching modes independently", async () => {
			...
			expect(task1.modeState.mode).toBe("architect")
			expect(task2.modeState.mode).toBe("debug")
			expect(task3.modeState.mode).toBe("code")
