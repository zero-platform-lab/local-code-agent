...
// load 中に落ちる。ClineProvider は既定 factory 内で `new Task(opts)` を評価するが、テストは
// `taskFactory` 引数を注入するので実 Task は構築されない。よって Task class 本体は明示 error
// 投げ stub とし、fake 表面は `fakeTaskFactory` へ移す。
vi.mock("../../task/Task", () => ({
	Task: class {
		constructor() {
			throw new Error("real Task should not be constructed in this spec; taskFactory injection expected")
		}
	},
}))
...
describe("ClineProvider Task History Synchronization", () => {
	...
	describe("updateTaskHistory", () => {
		...
		it("invalidates the recent-tasks cache on updateTaskHistory (UTH-04)", async () => {
			...
			// Cache 無効化の副作用を collaborator 越しに直接観測する
			// （キャッシュの所有は RecentTasksCache、その memoize 挙動は同クラスの spec が担保）。
			const recentTasks = (provider as unknown as { recentTasks: RecentTasksCache }).recentTasks
			const invalidateSpy = vi.spyOn(recentTasks, "invalidate")
