...
describe("checkpointRestoreHandler", () => {
	...
	beforeEach(() => {
		...
		mockProvider = {
			...
			getTaskWithId: vi.fn(() => ({
				historyItem: { id: "test-task-123", messages: mockCline.messageStore.clineMessages },
			})),
	...
	describe("handleCheckpointRestoreOperation", () => {
		...
		it("should save messages after delete operation", async () => {
			...
			mockCline.checkpointRestore.mockImplementation(async () => {
				mockCline.messageStore.clineMessages = mockCline.messageStore.clineMessages.slice(0, 2)
			})
			...
			expect(saveTaskMessages).toHaveBeenCalledWith({
				messages: mockCline.messageStore.clineMessages,
				taskId: "test-task-123",
