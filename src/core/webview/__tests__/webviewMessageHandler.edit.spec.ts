...
describe("webviewMessageHandler - Edit Message with Timestamp Fallback", () => {
	...
	it("should not modify API history when apiConversationHistoryIndex is -1", async () => {
		...
		mockCurrentTask.messageStore.clineMessages = [
			{
				ts: userMessageTs,
				type: "say",
				say: "user_feedback",
		...
		mockCurrentTask.messageStore.apiConversationHistory = [
			{
		...
		mockCurrentTask.messageStore.apiConversationHistory = [
			{
				ts: 1000,
				role: "assistant",
				content: [{ type: "input_text", text: "Old message 1" }],
			},
			{
				ts: 2000,
				role: "assistant",
				content: [{ type: "input_text", text: "Old message 2" }],
			},
		] as ApiMessage[]
