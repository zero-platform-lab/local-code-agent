...
describe("stripLineNumbers", () => {
	...
	it("末尾の改行を保持する", () => {
		// content が改行で終わるケース（trailing newline 保持分岐の入口）。
		const input = "1 | line one\n2 | line two\n"
		expect(stripLineNumbers(input)).toBe("line one\nline two\n")
	})
...
describe("applyRunLengthEncoding", () => {
	...
	it("繰り返しの無い相異なる行はそのまま（末尾に改行なし）", () => {
		// 連続する別行 → else の repeatCount===0 分岐（result += prevLine）と、
		// 末尾改行なし（content.slice の -1 分岐）＋ ループ後の残り 1 行出力を通す。
		const input = "a\nb\nc"
		expect(applyRunLengthEncoding(input)).toBe("a\nb\nc")
	})

	it("途中で終わる長い繰り返しは圧縮する（有益）", () => {
		// 繰り返しの run が別行で終わる分岐。prevLine を十分長くして圧縮が有益になる。
		const long = "X".repeat(50)
		const input = `${long}\n${long}\n${long}\n${long}\n${long}\nend`
		expect(applyRunLengthEncoding(input)).toBe(`${long}\n<previous line repeated 4 additional times>\nend`)
	})

	it("途中で終わる短い繰り返しは展開する（非有益）", () => {
		// run が別行で終わるが、圧縮記述の方が長いので展開したまま出す分岐。
		const input = "a\na\na\nb"
		expect(applyRunLengthEncoding(input)).toBe("a\na\na\nb")
	})
})
...
describe("processCarriageReturns", () => {
	...
	it("上書きテキストの末尾が高サロゲートで切れたら空白で補正する", () => {
		// segment（上書き側）の末尾が high surrogate 単独 → 507 の分岐。
		// "0123456789" を "ab" + 高サロゲート で部分上書きする。
		const input = "0123456789\rab\uD83D"
		expect(processCarriageReturns(input)).toBe("ab\uD83D 456789")
	})

	it("上書き境界が絵文字の先頭（高サロゲート）に当たったら空白で補正する", () => {
		// 元テキスト側の上書き位置が high surrogate（絵文字の先頭）→ 509 の分岐。
		// "ab🚀cd" を "XY" で部分上書きすると境界が 🚀 の先頭に当たる。
		const input = "ab🚀cd\rXY"
		expect(processCarriageReturns(input)).toBe("XY \uDE80cd")
	})
