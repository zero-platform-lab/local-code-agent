...
const rec = (lineNumber: number): LineRecord => ({
	lineNumber,
...
describe("formatWithLineNumbers", () => {
	...
	it("末尾要素が欠けた配列でも幅計算の既定値で落ちない", () => {
		// `lines[last]?.lineNumber || 1` の防御分岐（末尾が undefined / 行番号 falsy）を
		// 直接踏む。スパース配列で length>0 かつ末尾 undefined を作る。
		const sparse: LineRecord[] = [rec(1)]
		sparse.length = 2 // sparse[1] は undefined、length は 2

		const result = formatWithLineNumbers(sparse)

		// map は穴を飛ばし、join で穴が "" になるため末尾に改行が付く。
		// 幅計算は既定値 1（→パディング無し）。フォールバックが 1 でないと桁が変わる。
		expect(result).toBe("1 | line 1\n")
	})
})

// ─── computeIncludedRanges Tests ──────────────────────────────────────────────

describe("computeIncludedRanges", () => {
	it("空配列は空レンジを返す", () => {
		expect(computeIncludedRanges([])).toEqual([])
	})

	it("連続した行番号は 1 つのレンジにまとめる", () => {
		expect(computeIncludedRanges([rec(3), rec(4), rec(5)])).toEqual([[3, 5]])
	})

	it("行番号に飛びがあれば別レンジに分割する", () => {
		// readWithIndentation の出力は常に連続なのでこの gap 分岐は本体経由では踏めない。
		// ヘルパ単体で非連続入力を与え、分割ロジックを固定する。
		expect(computeIncludedRanges([rec(1), rec(2), rec(5), rec(6), rec(9)])).toEqual([
			[1, 2],
			[5, 6],
			[9, 9],
		])
