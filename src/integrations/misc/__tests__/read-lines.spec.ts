...
describe("nthline", () => {
	...
	describe("readLines function", () => {
		...
		it("数値でない startLine は RangeError で拒否する", async () => {
			// 型上は number だが、実行時に文字列が渡っても壊れないことを固定する。
			await expect(readLines(testFile, 2, "oops" as unknown as number)).rejects.toThrow(
				"Invalid startLine: oops. Line numbers must be numbers.",
			)
		})

		it("数値でない endLine は RangeError で拒否する", async () => {
			await expect(readLines(testFile, "oops" as unknown as number)).rejects.toThrow(
				"Invalid endLine: oops. Line numbers must be numbers.",
			)
		})
