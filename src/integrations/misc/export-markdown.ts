...
export type ExtendedContentBlock = ContentBlockParam | ReasoningBlock
...
export function formatContentBlockToMarkdown(block: ExtendedContentBlock): string {
	switch (block.type) {
		...
		case "reasoning":
			return `[Reasoning]\n${block.text}`
		default:
			// 型の上では到達しないが、保存済み履歴に未知のブロックが混ざる場合の保険。
			return `[Unexpected content type: ${(block as { type?: string }).type}]`
	}
}

export function findToolName(toolCallId: string, messages: MessageParam[]): string {
