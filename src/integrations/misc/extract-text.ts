...
// @ts-expect-error - pdf-parse は subpath export の型定義を持たない
...
export async function extractTextFromFileWithMetadata(
	...
	if (!isBinary) {
		...
		return {
			...
			/* v8 ignore next -- 到達不能: readWithSlice は offset=0・非空内容で includedRanges を必ず 1 要素返し undefined 側に入らない */
...
export function stripLineNumbers(content: string, aggressive: boolean = false): string {
	...
	if (content.endsWith(lineEnding)) {
		/* v8 ignore next 3 -- 到達不能: 末尾改行を持つ content は split(/\r?\n/) で必ず末尾空要素を生み、join 後の result も lineEnding で終わるため追加は起きない */
	...
	while (crPos < lineEnd) {
		...
		if (segment !== "") {
			...
			if (segment.length >= curLine.length) {
				...
				/* v8 ignore next -- 到達不能: 直上の if (segment !== "") で segment.length>=1 が保証され :0 側へ入らない */
