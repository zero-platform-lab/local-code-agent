...
// Narrow structural view of the owning Terminal that TerminalProcess needs.
// Declared here (instead of importing the concrete Terminal class) to break
// the Terminal ↔ TerminalProcess cycle. Terminal satisfies this shape
// structurally.
...
export class TerminalProcess extends BaseTerminalProcess {
	private terminalRef: WeakRef<TerminalProcessOwner>

	constructor(terminal: TerminalProcessOwner) {
		super()
	...
	public get terminal(): TerminalProcessOwner {
	...
	public override async run(command: string) {
		...
		const streamAvailable = new Promise<AsyncIterable<string>>((resolve, reject) => {
			const timeoutId = setTimeout(() => {
				...
				this.emit(
					"no_shell_integration",
					`VSCE shell integration stream did not start within ${BaseTerminal.getShellIntegrationTimeout() / 1000} seconds. Terminal problem?`,
				)

				// Reject with descriptive error
				reject(
					new Error(
						`VSCE shell integration stream did not start within ${BaseTerminal.getShellIntegrationTimeout() / 1000} seconds.`,
					),
				)
			}, BaseTerminal.getShellIntegrationTimeout())
		...
		if (isPowerShell) {
			...
			if (BaseTerminal.getCommandDelay() > 0) {
				commandToExecute += ` ; start-sleep -milliseconds ${BaseTerminal.getCommandDelay()}`
			}
