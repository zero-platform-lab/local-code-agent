...
function createSanitizedGit(baseDir: string): SimpleGit {
	...
	for (const [key, value] of Object.entries(process.env)) {
		...
		// or invoke interactive helpers. The shadow checkpoint git only runs
		// scripted operations (add/commit -m/ls-files/etc.) — an inherited
		// GIT_EDITOR / GIT_PAGER / GIT_ASKPASS is never legitimate here, and
		// simple-git >=3.32 refuses to spawn git while those are set unless
		// the corresponding unsafe flag is opted in.
		if (
			key === "GIT_DIR" ||
			key === "GIT_WORK_TREE" ||
			key === "GIT_INDEX_FILE" ||
	...
	const options: Partial<SimpleGitOptions> = {
		baseDir,
		config: [],
		// simple-git >=3.32 blocks operations if inherited env vars would let git
		// spawn an editor/pager/askpass. The env is already sanitized above (the
		// primary defence), but simple-git checks env at spawn time — the shadow
		// git process gets spawned from the parent env before git.env() takes
		// effect on the underlying binary. Opt in to the categories that shadow
		// git operations legitimately never trigger (no interactive prompts, no
		// external programs) so a stray inherited env var doesn't wedge us.
		unsafe: {
			allowUnsafeEditor: true,
			allowUnsafePager: true,
			allowUnsafeAskPass: true,
			// initShadowGit passes `--template=""` on purpose so a user's
			// init.templateDir doesn't leak hooks/config into the shadow repo.
	...
	public static async deleteBranch(git: SimpleGit, branchName: string) {
		...
		if (currentBranch === branchName) {
			...
			try {
				...
				return false
				/* v8 ignore next -- 人工分岐: V8 は try/catch/finally の finally に「例外が伝播中に通過する」経路を合成分岐として数えるが、この catch が全例外を握るため到達不能。ソースに if は無い（コンパイラ生成の分岐）。dual-path 復元で潰すと catch 内 console.error が投げたとき復元されず実テストが 1 件落ちる（実測済み）ため finally を保つ。 */
			} finally {
				// 破壊操作の前に外した core.worktree を、成功・失敗どちらでも必ず本体ワークスペースへ
				// 復元する安全網。catch 内の console.error 自体が投げても復元される。
				if (worktree.value) {
