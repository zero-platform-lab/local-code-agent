...
const initWorkspaceRepo = async ({
	workspaceDir,
	userName = "OpenAI Compatible Agent",
	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#hasNestedGitRepositories`, () => {
			it("throws error when nested git repositories are detected during initialization", async () => {
				...
				await mainGit.init()
				await mainGit.addConfig("user.name", "OpenAI Compatible Agent")
				await mainGit.addConfig("user.email", "support@example.com")
				...
				await nestedGit.addConfig("user.name", "OpenAI Compatible Agent")
				await nestedGit.addConfig("user.email", "support@example.com")
		...
		describe(`${klass.name}#saveCheckpoint with allowEmpty option`, () => {
			...
			it("isolates checkpoint operations from GIT_DIR environment variable", async () => {
				...
				// In the real-world Dev Container scenario, GIT_DIR is set BEFORE Agent starts,
