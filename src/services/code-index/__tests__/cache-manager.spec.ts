...
describe("CacheManager", () => {
	...
	describe("flush", () => {
		it("should persist current hashes to disk immediately", async () => {
			...
			expect(safeWriteJson).toHaveBeenCalledWith(mockCachePath.fsPath, { "a.ts": "hashA" })
		})

		it("should swallow save errors (never throws)", async () => {
			const consoleErrorSpy = vitest.spyOn(console, "error").mockImplementation(() => {})
			;(safeWriteJson as Mock).mockRejectedValue(new Error("disk full"))

			await expect(cacheManager.flush()).resolves.toBeUndefined()
