	...
	public async startIndexing(): Promise<void> {
		...
		try {
			...
			if (hasExistingData && !collectionCreated) {
				...
				// 注記: line247 で (indexed===0 && found>0) は必ず throw 済み。ここに到達する時点で
				// その否定が保証されるため、同一条件 (found>0 && indexed===0) は常に false。防御的な最終チェック。
				/* v8 ignore start -- 到達不能: line247 のガードにより (found>0 && indexed===0) はここでは常に false */
