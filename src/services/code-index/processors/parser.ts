	...
	private _chunkTextByLines(
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (currentChunkLength > 0 && currentChunkLength + lineLength > effectiveMaxChars) {
				...
				// 注記: splitIndex <= i-1 なので finalizeChunk 後の chunkStartLineIndex(=splitIndex+1) は必ず <= i。
				// よって i < chunkStartLineIndex は成立せず、下の巻き戻しは到達不能（安全既定として残す）。
				/* v8 ignore start -- 到達不能: 上記のとおり i < chunkStartLineIndex は成立しない（実測確認） */
