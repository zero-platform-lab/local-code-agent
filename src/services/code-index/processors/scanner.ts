...
export class DirectoryScanner implements IDirectoryScanner {
	 ...
	 * @param rooIgnoreController Optional AgentIgnoreController instance for filtering
	...
	public async scanDirectory(
		...
		// Initialize AgentIgnoreController if not provided
		const ignoreController = new AgentIgnoreController(directoryPath)

		await ignoreController.initialize()

		// Filter paths using .agentignore
							...
							if (trimmedContent) {
								...
								try {
									...
									if (currentBatchBlocks.length >= this.batchSegmentThreshold) {
										...
										// 注記: 直上（バッチ投入前）で signal?.aborted は検査済みで、ここまで await は無い。
										// また Promise.race 後は pendingBatchCount が必ず 20 未満へ下がりループを抜けるため、
										// この待機ループ内で新たに中断が観測されて throw されることはない（実測確認）。
										while (pendingBatchCount >= MAX_PENDING_BATCHES) {
											/* v8 ignore start -- 到達不能: 上記のとおり待機ループ内で signal?.aborted が真になることはない（防御既定として残す） */
											if (signal?.aborted) {
												throw new DOMException("Indexing aborted", "AbortError")
											}
											/* v8 ignore stop */
