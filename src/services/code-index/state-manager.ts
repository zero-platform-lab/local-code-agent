...
export class CodeIndexStateManager {
	...
	public reportBlockIndexingProgress(processedItems: number, totalItems: number): void {
		...
		if (progressChanged || this._systemStatus !== "Indexing") {
			...
			// 注記: oldStatus は _systemStatus を "Indexing" にする前に採取している。
			// このブロックへ入れるのは (progressChanged || _systemStatus !== "Indexing") が真のとき。
			// - progressChanged が真なら第3条件で必ず true。
			// - progressChanged が偽で入る場合は _systemStatus !== "Indexing"、つまり
			//   oldStatus !== "Indexing"（=新 _systemStatus）となり第1条件で必ず true。
			// よって if は常に true で、false 側（発火しない経路）は到達不能。
			/* v8 ignore next -- 到達不能: 上記のとおり本ブロック進入時は 3 条件のいずれかが必ず真 */
