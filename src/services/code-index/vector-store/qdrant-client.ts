...
export class QdrantVectorStore implements IVectorStore {
	...
	private parseQdrantUrl(url: string | undefined): string {
		...
		try {
			...
			// (parsing is only used for validation; an invalid URL throws and falls through to catch)
			new URL(trimmedUrl)
			return trimmedUrl
		} catch {
		...
		try {
			...
			if (directoryPrefix) {
				...
				if (normalizedPrefix === "." || normalizedPrefix === "./") {
					...
					// 注記: 直上の L418 で normalizedPrefix === "." / "./" は除外済み。
					// path.posix.normalize はそれ以外のどの入力でも先頭 "./" を生成しないため（実測確認）、
					// 下の startsWith("./") は常に false（true 側 slice(2) は到達不能）。
					// 安全既定としてブロック自体は残し、到達不能な分岐のみ計測から除外する。
					let prefixToNormalize = normalizedPrefix
					/* v8 ignore start -- 到達不能: 上記のとおり normalizedPrefix が "./" 始まりになることはない */
					if (normalizedPrefix.startsWith("./")) {
						prefixToNormalize = normalizedPrefix.slice(2)
					}
					/* v8 ignore stop */
					const cleanedPrefix = path.posix.normalize(prefixToNormalize)
