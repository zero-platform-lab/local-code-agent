...
import { getGlobalAgentDirectory, getProjectAgentDirectoryForCwd } from "../agent-config"
...
export async function getCommands(cwd: string): Promise<Command[]> {
	...
	const globalDir = path.join(getGlobalAgentDirectory(), "commands")
	...
	const projectDir = path.join(getProjectAgentDirectoryForCwd(cwd), "commands")
...
async function tryLoadCommand(
	...
	try {
		...
		// name はモデル由来。path.join の正規化で `..` を含むと dirPath の外へ抜け、
		// `../../../../etc/passwd` が `/etc/passwd.md` を読む任意ファイル読み取りになる。
		// 正当なサブディレクトリ名（`group/cmd`）は許しつつ、解決後パスが commands
		// ディレクトリ配下から出るものだけを弾く。
