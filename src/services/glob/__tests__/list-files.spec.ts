...
describe("hidden directory exclusion", () => {
	...
	it("should allow explicit targeting of hidden directories", async () => {
		// Mock filesystem structure for explicit .agent-memory targeting
		const mockReaddir = vi.fn()
		vi.mocked(fs.promises).readdir = mockReaddir

		// Mock .agent-memory directory contents
		mockReaddir.mockResolvedValueOnce([
		...
		// Call listFiles explicitly targeting .agent-memory directory
		const [result] = await listFiles("/test/.agent-memory", true, 100)

		// When explicitly targeting a hidden directory, its subdirectories should be included
		const directories = result.filter((item) => item.endsWith("/"))

		const hasTasksDir = directories.some((dir) => dir.includes(".agent-memory/tasks/") || dir.includes("tasks/"))
		const hasContextDir = directories.some(
			(dir) => dir.includes(".agent-memory/context/") || dir.includes("context/"),
		)
	...
	it("should include top-level files when recursively listing a hidden directory that's also in DIRS_TO_IGNORE", async () => {
		// This test specifically addresses the bug where files at the root level of .agent/temp
		...
		const mockProcess = {
			stdout: {
				on: vi.fn((event, callback) => {
					if (event === "data") {
						// Simulate files that should be found in .agent/temp
						// Note: ripgrep returns relative paths
						setTimeout(() => {
							callback("teste1.md\n")
		...
		// Mock directory listing for .agent/temp
		const mockReaddir = vi.fn()
		vi.mocked(fs.promises).readdir = mockReaddir
		mockReaddir.mockResolvedValueOnce([{ name: "22", isDirectory: () => true, isSymbolicLink: () => false }])

		// Call listFiles targeting .agent/temp (which is both hidden and in DIRS_TO_IGNORE)
