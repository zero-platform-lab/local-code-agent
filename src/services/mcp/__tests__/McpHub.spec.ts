...
import { fetchServerTools } from "../mcpClientQueries"
import type { ServerConfig } from "../serverConfigSchema"

/**
 * 変更検知（updateServerConnections）用の「宣言された設定」。
 * この spec 群は toggle/timeout/callTool を検証していて reconcile 経路は通らないため、
 * 型を満たす最小値で足りる。reconcile 自体は McpHub.serverReconciliation.spec.ts が担当。
 */
...
describe("McpHub", () => {
	...
	describe("Discriminated union type handling", () => {
		it("should create connected connections with proper type", async () => {
			...
			// Find the connection
			const connection = mcpHub.connectionStore.find("union-test-server")
		...
		it("should create disconnected connections for disabled servers", async () => {
			...
			// Find the connection
			const connection = mcpHub.connectionStore.find("disabled-union-server")
			expect(connection).toBeDefined()
		...
		it("should handle type narrowing correctly in callTool", async () => {
			...
			mcpHub.connectionStore.replaceAll([connectedConnection])
			...
			mcpHub.connectionStore.replaceAll([disconnectedConnection])
	...
	describe("DisableReason enum usage", () => {
		it("should use MCP_DISABLED reason when MCP is globally disabled", async () => {
			...
			// Find the connection
			const connection = mcpHub.connectionStore.find("mcp-disabled-server")
			expect(connection).toBeDefined()
		...
		it("should use SERVER_DISABLED reason when server is individually disabled", async () => {
			...
			// Find the connection
			const connection = mcpHub.connectionStore.find("server-disabled-server")
			expect(connection).toBeDefined()
			expect(connection?.type).toBe("disconnected")
			expect(connection?.server.status).toBe("disconnected")
			expect(connection?.server.disabled).toBe(true)
		...
		it("should handle both disable reasons correctly", async () => {
			...
			// Find the connection
			const connection = mcpHub.connectionStore.find("both-reasons-server")
			expect(connection).toBeDefined()
			expect(connection?.type).toBe("disconnected")
	...
	describe("Null safety improvements", () => {
		it("should handle null client safely in disconnected connections", async () => {
			...
			const connection = mcpHub.connectionStore.find("null-safety-server")
			expect(connection).toBeDefined()
			expect(connection?.type).toBe("disconnected")
		...
		it("should handle connection type checks safely", async () => {
			...
			// Get the connection
			const connection = mcpHub.connectionStore.find("type-check-server")
			expect(connection).toBeDefined()
		...
		it("should handle connection deletion safely", async () => {
			...
			const connection = mcpHub.connectionStore.find("delete-safety-server")
	...
	describe("toggleToolAlwaysAllow", () => {
		it("should add tool to always allow list when enabling", async () => {
			...
			mcpHub.connectionStore.replaceAll([mockConnection])
		...
		it("should mark all tools as always allowed when wildcard is present", async () => {
			...
			const tools = await fetchServerTools(
				mcpHub.connectionStore.find("test-server", "global"),
				(mcpHub as any).settingsPaths,
			)
	...
	describe("server disabled state", () => {
		...
		it("should filter out disabled servers from getServers", () => {
			...
			mcpHub.connectionStore.replaceAll(mockConnections)
		...
		it("should prevent calling tools on disabled servers", async () => {
			...
			const connection = mcpHub.connectionStore.find("disabled-server")
			expect(connection).toBeDefined()
			expect(connection?.type).toBe("disconnected")
			expect(connection?.server.disabled).toBe(true)
	...
	describe("callTool", () => {
		...
		describe("updateServerTimeout", () => {
			...
			it("should fallback to default timeout when config has invalid timeout", async () => {
				...
				mcpHub.connectionStore.replaceAll([mockConnectionInitial])
				...
				mcpHub.connectionStore.replaceAll([mockConnectionInvalid])
	...
	describe("MCP global enable/disable", () => {
		...
		it("should disconnect all servers when MCP is toggled from enabled to disabled", async () => {
			...
			const connectedServer = mcpHub.connectionStore.find("toggle-test-server")
			...
			const existingConnections = mcpHub.connectionStore.snapshot()
			...
			const disconnectedServer = mcpHub.connectionStore.find("toggle-test-server")
		...
		it("should not connect to servers when MCP is globally disabled", async () => {
			...
			const disabledServer = mcpHub.connectionStore.find("disabled-test-server")
		...
		it("should connect to servers when MCP is globally enabled", async () => {
			...
			const enabledServer = mcpHub.connectionStore.find("enabled-test-server")
		...
		it("should handle refreshAllConnections when MCP is disabled", async () => {
			...
			const server = mcpHub.connectionStore.find("refresh-test-server")
		...
		it("should skip restarting connection when MCP is disabled", async () => {
			...
			const server = mcpHub.connectionStore.find("restart-test-server")
			expect(server).toBeDefined()
			expect(server!.server.status).toBe("disconnected")
			expect(server!.client).toBeNull()
			expect(server!.transport).toBeNull()
