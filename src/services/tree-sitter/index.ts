...
// このリストを通過した拡張子は loadRequiredLanguageParsers の switch に必ず入るため、
// 3つが揃っていないと実行時例外になる:
//   1. ここの拡張子
//   2. languageParser.ts の switch の case
//   3. packages/build の SUPPORTED_TREE_SITTER_LANGUAGES（.vsix に同梱する wasm）
// 言語を足すときは3箇所すべてに入れること。
const extensions = [
	"js",
	"jsx",
	"ts",
	...
	"html",
	"htm",
	// Markdown（tree-sitter を使わず parseMarkdown で処理する）
	"md",
	"markdown",
	// JSON
	"json",
	...
	"ejs",
	"erb",
	// Visual Basic .NET — tree-sitter パーサは無い。code-index の fallback chunking 用に
	// スキャン対象として残す（code-index/shared/supported-extensions.ts の fallbackExtensions）。
	// 定義抽出の経路では下の try/catch により「定義なし」として扱われる。
	"vb",
].map((e) => `.${e}`)
...
export async function parseSourceCodeDefinitionsForFile(
	...
	// パーサを持たない拡張子（.vb など、code-index の fallback chunking 用に extensions へ
	// 入っているもの）や wasm のロード失敗は「定義なし」として扱い、呼び出し元に例外を伝播させない。
	// ここで rethrow すると、対象ファイルを開いただけで実行時エラーになる。
	let languageParsers: LanguageParser
	try {
		languageParsers = await loadRequiredLanguageParsers([filePath])
	} catch (error) {
		console.error(
			`Failed to load a tree-sitter parser for ${filePath}: ${error instanceof Error ? error.message : error}`,
		)
		return undefined
	}
