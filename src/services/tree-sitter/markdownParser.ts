...
export function parseMarkdown(content: string): QueryCapture[] {
	...
	for (let i = 0; i < captures.length; i += 2) {
		if (i + 1 < captures.length) {
			...
			/* v8 ignore start -- 到達不能: parseMarkdown はヘッダ毎に name.definition と definition の2件を必ずペアで push するため captures.length は常に偶数。i は2刻みで進み i+1<length が常に真となり、この else（奇数個の端数処理）へは入らない。防御分岐として残す。 */
		} else {
