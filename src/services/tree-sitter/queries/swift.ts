...
; static / class メソッド専用のパターンは置いていない。
; 上の (function_declaration name: ...) が static メソッドも捕捉し、parseFile は
; 行範囲で重複排除するため出力は同じになる。かつて存在した
;   (function_declaration (modifiers (property_modifier)) name: (simple_identifier) @name)
; は Query の構築だけで 16.3 秒かかっていた（他パターンは全て 210ms 以下）。
