...
describe("Code Action Prompts", () => {
	...
	describe("non-string parameter values", () => {
		it("should stringify array values via String() when substituting", () => {
			// createPrompt の else 側（typeof value !== "string" → String(value)）を踏む
			const prompt = supportPrompt.create("EXPLAIN", { arr: [1, 2, 3] }, { EXPLAIN: "values: ${arr}" })
			expect(prompt).toBe("values: 1,2,3")
		})
	})
