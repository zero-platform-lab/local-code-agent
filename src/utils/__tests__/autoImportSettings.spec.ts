...
describe("autoImportSettings", () => {
	...
	it("should skip auto-import when settings file does not exist", async () => {
		const settingsPath = "~/Documents/agent-config.json"
		vi.mocked(vscode.workspace.getConfiguration).mockReturnValue({
		...
		expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(
			"[AutoImport] Checking for settings file at: /home/user/Documents/agent-config.json",
		)
		expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(
			"[AutoImport] Settings file not found at /home/user/Documents/agent-config.json, skipping auto-import",
		)
		expect(mockProviderSettingsManager.import).not.toHaveBeenCalled()
	})
	...
	it("should stringify non-Error throwables in the catch (String(error) 分岐)", async () => {
		const settingsPath = "~/config.json"
		vi.mocked(vscode.workspace.getConfiguration).mockReturnValue({
			get: vi.fn().mockReturnValue(settingsPath),
		} as any)

		// Error インスタンスではない値を throw させ、`: String(error)` 側を通す。
		vi.mocked(fileExistsAtPath).mockRejectedValue("plain-string-failure")
		...
		expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(
			"[AutoImport] Unexpected error during auto-import: plain-string-failure",
		)
	})
})
