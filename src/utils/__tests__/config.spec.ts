...
describe("injectVariables", () => {
	...
	it("ネスト変数が見つからず propNotFoundValue も無ければ、元の ${key:name} を残す", async () => {
		const consoleWarnSpy = vitest.spyOn(console, "warn").mockImplementation(() => {})
		// 第3引数(propNotFoundValue)を渡さない → `?? match` の match 側に落ちる。
		const result = await injectVariables("value: ${foo:BAR}", { foo: {} })
		expect(result).toBe("value: ${foo:BAR}")
		expect(consoleWarnSpy).toHaveBeenCalledWith(
			`[injectVariables] variable "BAR" referenced but not found in "foo"`,
		)
		consoleWarnSpy.mockRestore()
	})

	it("ネスト変数が文字列でない場合はそのまま埋め込む（toPosix を呼ばない）", async () => {
		// 型契約上は string だが、実行時に数値が来ても toPosix で落ちないことを固定する。
		const result = await injectVariables("count=${obj:num}", { obj: { num: 42 } as unknown as string })
