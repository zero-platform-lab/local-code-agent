...
[remote "origin"]
 url = https://github.com/example-org/local-agent.git
...
`
		...
		expect(result).toEqual({
			repositoryUrl: "https://github.com/example-org/local-agent.git",
			repositoryName: "example-org/local-agent",
			defaultBranch: "main",
		})
...
[core]
	...
	bare = false
[remote "origin"]
	url = git@github.com:example-org/local-agent.git
	fetch = +refs/heads/*:refs/remotes/origin/*
[branch "main"]
	remote = origin
	merge = refs/heads/main
...
describe("convertGitUrlToHttps", () => {
	it("should leave HTTPS URLs unchanged", () => {
		const url = "https://github.com/example-org/local-agent.git"
		const converted = convertGitUrlToHttps(url)

		expect(converted).toBe("https://github.com/example-org/local-agent.git")
	})

	it("should convert SSH URLs to HTTPS format", () => {
		const url = "git@github.com:example-org/local-agent.git"
		const converted = convertGitUrlToHttps(url)
	...
	it("should convert SSH URLs with ssh:// prefix to HTTPS format", () => {
		const url = "ssh://git@github.com/example-org/local-agent.git"
		const converted = convertGitUrlToHttps(url)
	...
	it("should handle URLs without git@ prefix", () => {
		const url = "ssh://github.com/example-org/local-agent.git"
		const converted = convertGitUrlToHttps(url)
...
describe("sanitizeGitUrl", () => {
	it("should sanitize HTTPS URLs with credentials", () => {
		const url = "https://username:password@github.com/example-org/local-agent.git"
		const sanitized = sanitizeGitUrl(url)

		expect(sanitized).toBe("https://github.com/example-org/local-agent.git")
	})
	...
	it("should leave SSH URLs unchanged", () => {
		...
		expect(sanitized).toBe("git@github.com:example-org/local-agent.git")
	})
	...
	it("should leave SSH URLs with ssh:// prefix unchanged", () => {
		...
		expect(sanitized).toBe("ssh://git@github.com/example-org/local-agent.git")
	})

	it("should remove tokens from other URL formats", () => {
		const url = "https://oauth2:ghp_abcdef1234567890abcdef1234567890abcdef@github.com/example-org/local-agent.git"
		const sanitized = sanitizeGitUrl(url)
...
describe("extractRepositoryName", () => {
	it("should extract repository name from HTTPS URL", () => {
		...
		expect(repoName).toBe("example-org/local-agent")
	})

	it("should extract repository name from HTTPS URL without .git suffix", () => {
		const url = "https://github.com/example-org/local-agent"
		const repoName = extractRepositoryName(url)
...
// エラー経路・エッジケース
//
// 各コマンド実行系（searchCommits / getCommitInfo / getWorkingState / getGitStatus）は
// 「git 未インストール / 非リポジトリ」を早期 return で弾いたあと、実コマンドの失敗は
// catch で握って安全な既定値（[] / エラー文字列 / null）に落とす。ここを固定する。
// URL 系ユーティリティは解析に失敗しても元の値を返す（例外を投げない）ことが不変条件。
// ---------------------------------------------------------------------------
describe("git utils - error paths and edge cases", () => {
	const cwd = "/test/path"
	...
	// exec を「--version と rev-parse は成功、指定コマンドは失敗」に仕立てるヘルパ。
	// failCommand に一致するコマンドだけ error を返し、それ以外は既定応答で成功させる。
	function execWithFailingCommand(failCommand: string, errValue: unknown = new Error("boom")) {
		vitest.mocked(exec).mockImplementation((command: string, _options: any, callback: any) => {
			if (command === "git --version") {
				callback(null, { stdout: "git version 2.39.2", stderr: "" })
			} else if (command === "git rev-parse --git-dir") {
				callback(null, { stdout: ".git", stderr: "" })
			} else if (command === failCommand) {
				callback(errValue)
			} else {
				callback(new Error(`Unexpected command: ${command}`))
			}
	...
	function execWithResponses(responses: Map<string, { stdout: string; stderr: string }>) {
		vitest.mocked(exec).mockImplementation((command: string, _options: any, callback: any) => {
			const r = responses.get(command)
			if (r) {
				callback(null, r)
			} else {
				callback(new Error(`Unexpected command: ${command}`))
	...
	describe("getGitRepositoryInfo", () => {
		it("workspaceRoot が文字列でないと path.join が投げ、外側 catch が {} を返す", async () => {
			// path は本物なので、非文字列を渡すと path.join が TypeError を投げる。
			// これが内側の try/catch では拾われず、関数最外の catch に到達する。
			const result = await getGitRepositoryInfo(123 as unknown as string)
			expect(result).toEqual({})
		})
	})

	describe("URL ユーティリティは解析失敗でも元の値を返す", () => {
		it("convertGitUrlToHttps: 文字列以外は例外を握って引数をそのまま返す", () => {
			expect(convertGitUrlToHttps(123 as unknown as string)).toBe(123 as unknown as string)
		})

		it("sanitizeGitUrl: 文字列以外は例外を握って引数をそのまま返す", () => {
			expect(sanitizeGitUrl(123 as unknown as string)).toBe(123 as unknown as string)
		})

		it("extractRepositoryName: 文字列以外は例外を握って空文字を返す", () => {
			expect(extractRepositoryName(123 as unknown as string)).toBe("")
		})
	})

	describe("searchCommits", () => {
		it("grep が空でハッシュ検索も空なら [] を返す", async () => {
			// query がハッシュ様（[a-f0-9]）なので grep 空 → ハッシュ検索へフォールバックする。
			execWithResponses(
				new Map([
					["git --version", { stdout: "git version 2.39.2", stderr: "" }],
					["git rev-parse --git-dir", { stdout: ".git", stderr: "" }],
		...
		it("git log 実行が投げたら catch で [] に落ちる", async () => {
			execWithFailingCommand(
				'git log -n 10 --format="%H%n%h%n%s%n%an%n%ad" --date=short --grep="test" --regexp-ignore-case',
			)
			const result = await searchCommits("test", cwd)
			expect(result).toEqual([])
	...
	describe("getCommitInfo", () => {
		it("本文(body)が無いコミットでも Description 節を出さずに整形できる", async () => {
			const hash = "nobody"
			// 6 番目のフィールド(%b)が空 → trim 後 5 要素になり body は undefined。
			execWithResponses(
				new Map([
					["git --version", { stdout: "git version 2.39.2", stderr: "" }],
					["git rev-parse --git-dir", { stdout: ".git", stderr: "" }],
					...
					[`git show --stat --format="" ${hash}`, { stdout: "1 file changed", stderr: "" }],
					[`git show --format="" ${hash}`, { stdout: "diff-body", stderr: "" }],
				]),
			)

			const result = await getCommitInfo(hash, cwd)
			expect(result).toContain("Commit: short (fullhash)")
			expect(result).toContain("Message: subject line")
			expect(result).not.toContain("Description:")
		})

		it("git show 実行が投げたら失敗メッセージを返す", async () => {
			execWithFailingCommand('git show --format="%H%n%h%n%s%n%an%n%ad%n%b" --no-patch abc123')
			const result = await getCommitInfo("abc123", cwd)
			expect(result).toMatch(/^Failed to get commit info:/)
		})

		it("Error 以外が投げられても String(error) で整形して返す", async () => {
			// error instanceof Error が false の分岐（String(error)）を通す。
			execWithFailingCommand(
				'git show --format="%H%n%h%n%s%n%an%n%ad%n%b" --no-patch abc123',
				"plain string failure",
			)
			const result = await getCommitInfo("abc123", cwd)
			expect(result).toBe("Failed to get commit info: plain string failure")
		})
	})

	describe("getWorkingState", () => {
		it("git status 実行が投げたら失敗メッセージを返す", async () => {
			execWithFailingCommand("git status --short")
			const result = await getWorkingState(cwd)
			expect(result).toMatch(/^Failed to get working state:/)
		})

		it("Error 以外が投げられても String(error) で整形して返す", async () => {
			execWithFailingCommand("git status --short", "plain string failure")
			const result = await getWorkingState(cwd)
			expect(result).toBe("Failed to get working state: plain string failure")
		})
	})

	describe("getGitStatus", () => {
		it("git status 実行が投げたら null を返す", async () => {
			execWithFailingCommand("git status --porcelain=v1 --branch")
			const result = await getGitStatus(cwd)
			expect(result).toBeNull()
		})
	})
