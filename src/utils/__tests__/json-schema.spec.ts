...
import { normalizeToolSchema, ToolInputSchema } from "../json-schema"
...
describe("normalizeToolSchema", () => {
	...
	it("should leave a schema with an empty top-level anyOf untouched", () => {
		// anyOf は truthy だが空配列 → 展開できるバリアントが無いのでそのまま通す。
		const result = normalizeToolSchema({ anyOf: [] })
		expect(result).toEqual({ anyOf: [] })
	})

	it("should leave a schema with a non-array top-level composition keyword untouched", () => {
		// 型契約上は配列だが、壊れた入力(非配列)でも落とさずそのまま返す。
		const result = normalizeToolSchema({ oneOf: "broken" as unknown as Record<string, unknown>[] })
		expect(result).toEqual({ oneOf: "broken" })
	})
...
describe("ToolInputSchema", () => {
	it("再帰的に additionalProperties:false を既定として付与する", () => {
		const parsed = ToolInputSchema.parse({
			type: "object",
			properties: {
				name: { type: "string" },
				nested: {
		...
		// トップと入れ子の両方に false 既定が入る。
		expect(parsed.additionalProperties).toBe(false)
		expect(parsed.properties.nested.additionalProperties).toBe(false)
		// primitive は passthrough だが additionalProperties 既定も付く（union の既定）。
		expect(parsed.properties.name.type).toBe("string")
	})

	it("passthrough により未知キーを保持する", () => {
		const parsed = ToolInputSchema.parse({
			type: "object",
			properties: {},
			title: "My Tool",
	...
	it("不正な type は safeParse で安全に失敗する", () => {
		const result = ToolInputSchema.safeParse({ type: "not-a-valid-type" })
		expect(result.success).toBe(false)
	})

	it("配列 items も再帰的に検証・既定付与する", () => {
		const parsed = ToolInputSchema.parse({
			type: "array",
		...
		expect(parsed.items.additionalProperties).toBe(false)
	})
})
