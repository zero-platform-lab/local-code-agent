...
describe("mcp-name utilities", () => {
	...
	describe("parseMcpToolName", () => {
		...
		it("should return null when the server name segment is empty", () => {
			// "mcp----tool" は server が空文字になる → 空判定で弾く。
			expect(parseMcpToolName("mcp----tool")).toBeNull()
		})

		it("should return null when the tool name segment is empty", () => {
			// "mcp--server--" は tool が空文字になる → 空判定で弾く。
			expect(parseMcpToolName("mcp--server--")).toBeNull()
		})
	})
