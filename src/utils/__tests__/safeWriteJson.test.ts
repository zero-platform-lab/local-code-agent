...
describe("safeWriteJson", () => {
	...
	// _streamDataToFile の分岐：prettyPrint 指定あり／undefined データを null として書く。
	test("prettyPrint 指定かつ undefined データは null をタブ整形で書き出す", async () => {
		const filePath = path.join(tempDir, "pretty-null.json")

		// options を渡すことで options?.prettyPrint の「options あり」側、
		// data === undefined の null 側、prettyPrint の "\t" 側を同時に通す。
		await safeWriteJson(filePath, undefined, { prettyPrint: true })

		const raw = await fs.readFile(filePath, "utf-8")
		expect(raw).toBe("null")
	})

	// onCompromised コールバック本体、および解放関数が失敗した場合の finally の握りつぶし。
	test("onCompromised はログして再 throw、解放失敗は finally でログして resolve する", async () => {
		vi.resetModules()

		const lockFilePath = path.join(tempDir, "compromised.json")
		await fs.writeFile(lockFilePath, JSON.stringify({ initial: true }))

		const consoleErrorSpy = vi.spyOn(console, "error").mockImplementation(() => {})

		vi.doMock("proper-lockfile", () => ({
			lock: vi.fn(async (_p: string, opts: any) => {
				// onCompromised の本体（ログ＋throw）を直接通す。
				expect(() => opts.onCompromised(new Error("compromised"))).toThrow("compromised")
				// 解放関数は reject させ、finally の unlock catch を通す。
				return async () => {
		...
		// 書き込み自体は成功。解放失敗はログのみで、Promise は resolve する。
		await expect(mockedSafeWriteJson(lockFilePath, { ok: 1 })).resolves.toBeUndefined()

		const written = JSON.parse(await fs.readFile(lockFilePath, "utf-8"))
