...
describe("Shell Detection Tests", () => {
	...
	// --------------------------------------------------------------------------
	// VS Code 設定取得の例外・環境変数フォールバック
	// --------------------------------------------------------------------------
	describe("VS Code config / environment error paths", () => {
		it("Windows: getConfiguration が投げても catch して安全な既定に落ちる", () => {
			Object.defineProperty(process, "platform", { value: "win32" })
			// getWindowsTerminalConfig 内の try が投げる → catch で既定値を返す。
			vscode.workspace.getConfiguration = () => {
		...
		it("macOS: getConfiguration が投げても catch して安全な既定に落ちる", () => {
			Object.defineProperty(process, "platform", { value: "darwin" })
			vscode.workspace.getConfiguration = () => {
				throw new Error("config unavailable")
			}
			...
			const result = getShell()
			expect(result).toBe("/bin/zsh") // macOS の環境変数フォールバック既定
		})

		it("Windows: COMSPEC 未設定なら cmd.exe の既定にフォールバックする", () => {
			Object.defineProperty(process, "platform", { value: "win32" })
			// VS Code からは何も取れず、userInfo も null、COMSPEC も未設定。
			vscode.workspace.getConfiguration = () => ({ get: () => undefined }) as any
			vi.mocked(userInfo).mockReturnValue({ shell: null } as any)
			delete process.env.COMSPEC
