...
describe("tiktoken", () => {
	...
	it("should not throw on text content with special tokens", async () => {
		const content: ContentBlockParam[] = [{ type: "text", text: "something<|endoftext|>something" }]
	...
	it("should handle missing text content", async () => {
		...
		const content = [{ type: "text" }] as any as ContentBlockParam[]
	...
	it("should apply a fudge factor to the token count", async () => {
		...
		const content: ContentBlockParam[] = [{ type: "text", text: "Test" }]
	...
	describe("serialization edge cases", () => {
		it("tool_use の input が JSON 化できなくても落ちず、フォールバック文字列で数える", async () => {
			// BigInt は JSON.stringify で TypeError になる → serializeToolUse の catch を通す。
			const content = [
				{
					type: "tool_use",
					id: "tool_bigint",
			...
			const result = await tiktoken(content)
			// "[serialization error]" を含む文字列としてトークン化されるので正の値になる。
			expect(result).toBeGreaterThan(0)
		})

		it("tool_result 配列内の空 text 要素でも落ちない（item.text || '' 分岐）", async () => {
			const content = [
				{
					type: "tool_result",
					tool_use_id: "tool_empty",
					content: [
						{ type: "text", text: "" }, // 空文字 → falsy
						{ type: "text" }, // text 欠落 → undefined
					],
				},
			] as unknown as ContentBlockParam[]
		...
		it("tool_result の未知コンテンツブロックはプレースホルダとして安全に数える", async () => {
			const content = [
				{
					type: "tool_result",
					tool_use_id: "tool_unknown",
					content: [
						{ type: "text", text: "ok" },
