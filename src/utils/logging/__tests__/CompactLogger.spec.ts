...
describe("CompactLogger", () => {
	...
	describe("Child Logger Edge Cases", () => {
		...
		test("parentMeta を持たない logger の child は meta をそのまま親メタにする", () => {
			// logger は beforeEach で parentMeta 無しで生成される（child の `: meta` 側を通す）。
	...
	describe("parentMeta + 明示 meta の結合（combineMeta 本体）", () => {
		it("parentMeta を持つ logger に meta 付きでログすると両者を結合する", () => {
			// parentMeta と meta の両方が存在する経路（combineMeta の最終 return）を通す。
			const withParent = new CompactLogger(transport, { ctx: "parent", traceId: "t1" })

			withParent.info("message", { userId: "u1" })

			// meta.ctx が無いので parentMeta.ctx が使われる。
			expect(transport.entries[0]).toMatchObject({
				m: "message",
				c: "parent",
				d: { traceId: "t1", userId: "u1" },
			})
		})

		it("meta.ctx があればそちらが親の ctx を上書きする", () => {
			const withParent = new CompactLogger(transport, { ctx: "parent", traceId: "t1" })

			withParent.info("message", { ctx: "call-site", userId: "u1" })
