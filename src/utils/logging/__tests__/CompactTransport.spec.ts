...
describe("CompactTransport", () => {
	...
	describe("Initialization Failure", () => {
		it("ログファイル初期化に失敗したら明示的な Error を投げる", () => {
			// testDir 配下に「ファイル」を作り、その配下をログパスにする。
			// mkdirSync(recursive) は祖先がファイルなので ENOTDIR で失敗する（モック不要）。
			const blocker = path.join(testDir, "blocker-file")
			fs.writeFileSync(blocker, "x")

			const failing = new CompactTransport({
				level: "fatal",
				fileOutput: { enabled: true, path: path.join(blocker, "sub", "fail.log") },
			})

			expect(() => failing.write({ t: Date.now(), l: "info", m: "x" })).toThrow(/Failed to initialize log file/)
		})
	})
