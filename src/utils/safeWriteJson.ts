...
async function safeWriteJson(filePath: string, data: any, options?: SafeWriteJsonOptions): Promise<void> {
	...
	// ロック取得に失敗した場合は下の finally に入る前に throw するため、releaseLock は
	// 実際のロック解放関数が代入された状態でしか finally から呼ばれない（definite assignment）。
	...
	try {
		...
		// If lock acquisition fails, we throw immediately. この throw は下の
		// try/catch/finally に入る前に脱出するため、releaseLock が未代入のまま finally から
		// 呼ばれることはない（未取得のロックを解放しに行かない）。
	...
	try {
		...
		try {
			// ここに到達するのはロック取得に成功した場合だけなので、releaseLock は実際の解放関数。
			await releaseLock()
		} catch (unlockError) {
