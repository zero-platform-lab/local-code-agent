import type { ContentBlockParam, ToolResultBlockParam, ToolUseBlockParam } from "@openai-agent/types"
 ...
 */
function serializeToolUse(block: ToolUseBlockParam): string {
 ...
 */
function serializeToolResult(block: ToolResultBlockParam): string {
...
export async function tiktoken(content: ContentBlockParam[]): Promise<number> {
	if (content.length === 0) {
		return 0
	}
	...
	for (const block of content) {
		if (block.type === "text") {
			...
			const serialized = serializeToolUse(block as ToolUseBlockParam)
			...
			const serialized = serializeToolResult(block as ToolResultBlockParam)
			if (serialized.length > 0) {
				const tokens = encoder.encode(serialized, undefined, [])
				totalTokens += tokens.length
			}
