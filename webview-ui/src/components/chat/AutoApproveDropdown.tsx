...
import { useAgentPortal } from "@/components/ui/hooks/useAgentPortal"
...
export const AutoApproveDropdown = ({ disabled = false, triggerClassName = "" }: AutoApproveDropdownProps) => {
	...
	const onAutoApproveToggle = React.useCallback(
		(key: AutoApproveSetting, value: boolean) => {
			...
			/* v8 ignore next 4 -- 到達不能: 個々の許可ボタンは autoApprovalEnabled が偽のとき disabled になるため、ここへは常に真の状態でしか来ない。将来 disabled を外したときの保険として残す */
	...
	const handleSelectAll = React.useCallback(() => {
		...
		/* v8 ignore next 4 -- 到達不能: 「すべて選択」ボタンも autoApprovalEnabled が偽のときは disabled。同上の保険 */
