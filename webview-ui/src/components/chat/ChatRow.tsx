...
export const ChatRowContent = ({
	...
	const [icon, title] = useMemo(() => {
		switch (type) {
			...
			default:
				return [null, null]
			/* v8 ignore next -- 人工分岐: どの case も return するため、switch を抜ける合成経路は存在しない */
		}
	}, [
		type,
		isCommandExecuting,
			...
			case "webFetch": {
				return (
					<div style={headerStyle}>
						...
						<span style={{ fontWeight: "bold" }}>
							<Trans
								i18nKey="chat:webFetch.wantsToFetch"
				...
				if (isBatchRequest) {
					return (
						<>
							...
							<BatchFilePermission
								/* v8 ignore next -- 到達不能: この枝は tool.batchFiles が配列のときだけ描画される。型の都合で既定は残す */
		...
		case "say":
			switch (message.say) {
					...
					return (
						<>
							...
							{(((cost === null || cost === undefined) && apiRequestFailedMessage) ||
								...
								<ErrorRow
									type="api_failure"
									/* v8 ignore next -- 到達不能: 直上の条件でどちらかは必ず入っているため、空文字の既定は踏まない */
				...
				case "too_many_tools_warning": {
					...
					/* v8 ignore next -- 到達不能: 既定の "{}" を渡しているので解析結果が undefined になることはない */
				...
				/* v8 ignore next -- 人工分岐: どの case も return するため、say の switch を抜けて ask へ落ちる合成経路は存在しない */
			}
		case "ask":
			switch (message.ask) {
				case "mistake_limit_reached":
				...
				case "use_mcp_server":
					...
					/* v8 ignore next 3 -- 到達不能: 直前でオブジェクトリテラルから組み立てているので falsy にならない */
					if (!useMcpServer) {
						return null
					}
				...
				default:
					return null
				/* v8 ignore next 3 -- 人工分岐: どの case も return するため、switch を抜ける合成経路は存在しない */
			}
	}
}
