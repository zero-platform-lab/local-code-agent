...
import { mentionRegexGlobal, commandRegexGlobal, unescapeSpaces } from "@agent/context-mentions"
import { WebviewMessage } from "@agent/WebviewMessage"
import { Mode, getAllModes } from "@agent/modes"
...
import { moveMenuSelection, selectableOptionAt } from "./contextMenuKeyboard"
...
export const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		// 補完メニューの 5 state と、その一貫性を保つ規則（外側クリック・種別クリア・blur）は
		// useContextMenuState が所有する。cursorPosition などは寿命が違うのでここに残す。
		const {
			showContextMenu,
			searchQuery,
			selectedMenuIndex,
			...
			closeOnBlur,
		} = useContextMenuState(contextMenuContainerRef)
		...
		const queryItems = useMemo(
			() => buildContextMenuQueryItems({ gitCommits, openedTabs, filePaths }),
		...
		const handleKeyDown = useCallback(
			(event: React.KeyboardEvent<HTMLTextAreaElement>) => {
				if (showContextMenu) {
					...
					if ((event.key === "Enter" || event.key === "Tab") && selectedMenuIndex !== -1) {
						event.preventDefault()
						const selectedOption = selectableOptionAt(
							getContextMenuOptions(
								searchQuery,
								selectedType,
								queryItems,
				...
				if (event.key === "Backspace" && !isComposing) {
					// 何をするかの判断は純関数側。ここは決まった動作を適用するだけ。
					const action = resolveMentionBackspace({ inputValue, cursorPosition, justDeletedSpaceAfterMention })

					if (action.kind === "collapse-space-after-mention") {
						if (action.moveCaret) {
							event.preventDefault()
							textAreaRef.current?.setSelectionRange(action.newCursorPosition, action.newCursorPosition)
						}

						setCursorPosition(action.newCursorPosition)
						setJustDeletedSpaceAfterMention(true)
					} else if (action.kind === "remove-mention") {
						if (action.removed) {
							event.preventDefault()
							setInputValue(action.removed.newText)
							// Store the new cursor position in state
							setIntendedCursorPosition(action.removed.newCursorPosition)
						}
			...
			[
				...
				commands,
				enterBehavior,
				// useState 由来なので識別子は不変。依存に足しても再生成の頻度は変わらない
				// （hook 越しなので eslint からは stable と判定できないだけ）。
				setShowContextMenu,
				setSelectedMenuIndex,
				setSelectedType,
			],
		...
		const handleBlur = useCallback(() => {
			// メニュー自身のクリックで閉じない規則は useContextMenuState が持つ。
			closeOnBlur()
			setIsFocused(false)
		}, [closeOnBlur])
		...
		const handlePaste = useCallback(
			...
			[shouldDisableImages, setSelectedImages, cursorPosition, setInputValue, inputValue, t, setShowContextMenu],
		)

		const updateHighlights = useCallback(() => {
			/* v8 ignore next -- 到達不能: どちらの ref も同じ描画ツリーの要素に必ず付き、この関数は描画後にしか呼ばれない。将来の描画変更に備えた防御は残す */
		...
		const handleDrop = useCallback(
			async (e: React.DragEvent<HTMLDivElement>) => {
				...
				if (text) {
					const insertion = buildMentionInsertion({ text, inputValue, cursorPosition, cwd })

					if (insertion) {
						setInputValue(insertion.newValue)
						setCursorPosition(insertion.newCursorPosition)
						setIntendedCursorPosition(insertion.newCursorPosition)
					}
			...
			<div
				...
				<div className="flex items-center gap-2">
					...
					<div className={cn("flex flex-shrink-0 items-center gap-0.5 h-5 leading-none", "pr-2")}>
						{!isEditMode ? <SecretModeToggle /> : null}
						{!isEditMode ? <FileMappingToggle /> : null}
