...
export const CodeIndexPopover: React.FC<CodeIndexPopoverProps> = ({
		...
		const handleMessage = (event: MessageEvent<any>) => {
			if (event.data.type === "indexingStatusUpdate") {
				...
				if (event.data.success) {
					...
					// 5 秒後に消す。以前はここで同期的に idle へ戻していたため、
					// 保存に失敗してもエラー表示が一度も描画されなかった。
	...
	// 保存エラーの自動消去タイマーは、アンマウント時にだけ捨てる。
	// （メッセージ購読の effect 側で捨てると、再描画のたびにタイマーが消える）
	useEffect(
		() => () => {
			if (errorResetTimerRef.current !== null) {
				clearTimeout(errorResetTimerRef.current)
			...
			<Popover
				...
				<PopoverContent
					...
					<div className="p-4">
									...
									<div className="space-y-2">
										...
										<div className="flex items-center gap-2">
											<Slider
												...
												value={[currentSettings.codebaseIndexSearchMinScore]}
											...
											<span className="w-12 text-center">
												{currentSettings.codebaseIndexSearchMinScore.toFixed(2)}
											</span>
											<VSCodeButton
												appearance="icon"
									...
									<div className="space-y-2">
										...
										<div className="flex items-center gap-2">
											<Slider
												...
												value={[currentSettings.codebaseIndexSearchMaxResults]}
												onValueChange={(values) =>
						...
						{saveStatus === "error" && (
							<div className="mt-2">
								<span className="text-sm text-vscode-errorForeground block">{saveError}</span>
							</div>
						)}
					</div>
				</PopoverContent>
