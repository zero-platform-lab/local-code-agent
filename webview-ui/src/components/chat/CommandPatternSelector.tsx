								...
								<div className="flex-1">
									{editState.isEditing ? (
										<input
											...
											// Leaving the field or pressing Enter keeps what was typed; only Escape
											// puts the original pattern back.
											onBlur={() => setEditState(item.pattern, false, editState.value)}
											onKeyDown={(e) => {
												if (e.key === "Enter") {
													setEditState(item.pattern, false, editState.value)
												}
												if (e.key === "Escape") {
