...
const ContextMenu: React.FC<ContextMenuProps> = ({
		...
		switch (option.type) {
			...
			case ContextMenuOptionType.Folder:
				if (option.value) {
					...
					/* v8 ignore next -- 到達不能: 直前の if (option.value) が空文字を弾く */
	...
	const getIconForOption = (option: ContextMenuQueryItem): string => {
		switch (option.type) {
			/* v8 ignore next 2 -- 到達不能: Mode は codicon の描画条件で除外されるため呼ばれない */
			case ContextMenuOptionType.Mode:
				return "symbol-misc"
			/* v8 ignore next 2 -- 到達不能: Command は codicon の描画条件で除外されるため呼ばれない */
			case ContextMenuOptionType.Command:
				return "play"
			/* v8 ignore next 2 -- 到達不能: OpenedFile は material icon 側で描画されるため呼ばれない */
			case ContextMenuOptionType.OpenedFile:
				return "window"
			/* v8 ignore next 2 -- 到達不能: File は material icon 側で描画されるため呼ばれない */
			case ContextMenuOptionType.File:
				return "file"
			/* v8 ignore next 2 -- 到達不能: Folder は material icon 側で描画されるため呼ばれない */
			case ContextMenuOptionType.Folder:
				return "folder"
			case ContextMenuOptionType.Problems:
				return "warning"
			...
			case ContextMenuOptionType.NoResults:
				return "info"
			/* v8 ignore next 2 -- 到達不能: 残る enum 値は上の case か、呼び出し側の除外条件（SectionHeader）で尽きている */
			default:
				return "file"
		}
	}
