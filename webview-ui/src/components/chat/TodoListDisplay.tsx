...
export function TodoListDisplay({ todos }: { todos: any[] }) {
	// 毎レンダー新しい配列になると下の useMemo/useEffect が毎回走るので memo 化する。
	const safeTodos = useMemo(() => (Array.isArray(todos) ? todos : []), [todos])
	const [isCollapsed, setIsCollapsed] = useState(true)
	const ulRef = useRef<HTMLUListElement>(null)
	const itemRefs = useRef<(HTMLLIElement | null)[]>([])
	const scrollIndex = useMemo(() => {
		const inProgressIdx = safeTodos.findIndex((todo: any) => todo.status === "in_progress")
		if (inProgressIdx !== -1) return inProgressIdx
		return safeTodos.findIndex((todo: any) => todo.status !== "completed")
	}, [safeTodos])

	const mostImportantTodo = useMemo(() => {
		const inProgress = safeTodos.find((todo: any) => todo.status === "in_progress")
		if (inProgress) return inProgress
		return safeTodos.find((todo: any) => todo.status !== "completed")
	}, [safeTodos])
	useEffect(() => {
		if (isCollapsed) return
		/* v8 ignore next -- 到達不能: 展開時は ul を必ず描画してから effect が走る */
	...
	}, [safeTodos, isCollapsed, scrollIndex])
	if (safeTodos.length === 0) return null

	const totalCount = safeTodos.length
	const completedCount = safeTodos.filter((todo: any) => todo.status === "completed").length
		...
		<div data-todo-list className="mt-1 -mx-2.5 border-t border-vscode-sideBar-background overflow-hidden">
			...
			{!isCollapsed && (
				<ul ref={ulRef} className="list-none max-h-[300px] overflow-y-auto mt-2 -mb-1 pb-0 px-2 cursor-default">
					{safeTodos.map((todo: any, idx: number) => {
