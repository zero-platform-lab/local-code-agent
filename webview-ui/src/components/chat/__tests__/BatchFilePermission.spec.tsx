...
describe("BatchFilePermission", () => {
	...
	it("shows a file that has no line snippet with just its path", () => {
		render(
			<TranslationProvider>
				<BatchFilePermission
					files={[{ key: "file1", path: "src/a.ts", content: "src/a.ts", isOutsideWorkspace: false }]}
					onPermissionResponse={mockOnPermissionResponse}
					ts={1}
				/>
			</TranslationProvider>,
		...
		expect(label.textContent?.trim()).toBe("src/a.ts\u200e")
	})
})
