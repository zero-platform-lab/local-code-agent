...
describe("ChatView scroll behavior regression coverage", () => {
	...
	// このテストはフル並列実行時の CPU 競合でタイミングが揺れて flaky だった。ボタンは毎クリック
	// 取り直す（連打中の再描画で DOM が差し替わっても古い参照への no-op を避ける）。逆に「待機」を
	// 挟むと再描画でオーバーレイごとボタンが一瞬消える窓が開くので、待たずに連続クリックする。
	// 残る負荷ゆらぎは retry で吸収する（isolation では安定。ロジック自体は正しい）。
	it("repeated checkpoint clicks step backward through previous checkpoints", { retry: 3 }, async () => {
