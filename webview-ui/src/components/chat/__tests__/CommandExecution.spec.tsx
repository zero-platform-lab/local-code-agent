...
// Mock dependencies. The message handler is captured so tests can deliver extension messages.
...
Output:
	...
	const sendStatus = (status: Record<string, unknown>) => {
		for (const handler of messageHandlers.current) {
			fireEvent.click(document.body) // flush any pending state before delivering
			handler({ data: { type: "commandExecutionStatus", text: JSON.stringify(status) } } as MessageEvent)
		}
	}

	it("shows the process id and an abort control while the command runs", () => {
		render(
			<ExtensionStateWrapper>
				<CommandExecution executionId="test-1" text="npm install" />
			</ExtensionStateWrapper>,
		...
		act(() => sendStatus({ executionId: "test-1", status: "started", pid: 4242, command: "npm install" }))

		expect(screen.getByText("(PID: 4242)")).toBeInTheDocument()

		fireEvent.click(document.querySelector(".lucide-octagon-x")!.closest("button")!)
		expect(vscode.postMessage).toHaveBeenCalledWith({ type: "terminalOperation", terminalOperation: "abort" })
	})

	it("shows a running command that has no process id yet", () => {
		render(
			<ExtensionStateWrapper>
				<CommandExecution executionId="test-1" text="npm install" />
			</ExtensionStateWrapper>,
		...
		act(() => sendStatus({ executionId: "test-1", status: "started", command: "npm install" }))

		expect(screen.queryByText(/PID/)).not.toBeInTheDocument()
	})

	it("streams output as it arrives", () => {
		render(
			<ExtensionStateWrapper>
				<CommandExecution executionId="test-1" text="npm install" />
			</ExtensionStateWrapper>,
		...
		act(() => sendStatus({ executionId: "test-1", status: "output", output: "installing..." }))

		expect(screen.getByTestId("terminal-output")).toHaveTextContent("installing...")
	})

	it("opens the output when the terminal falls back to a plain shell", () => {
		render(
			<ExtensionStateWrapper>
				<CommandExecution executionId="test-1" text={"npm install\nOutput:\nsome output"} />
			</ExtensionStateWrapper>,
		...
		act(() => sendStatus({ executionId: "test-1", status: "fallback" }))

		expect(screen.getByTestId("terminal-output")).toBeInTheDocument()
	})

	it("marks a successful exit green and a failed one red", () => {
		...
		act(() => sendStatus({ executionId: "test-1", status: "exited", exitCode: 0 }))
		...
		act(() => sendStatus({ executionId: "test-1", status: "exited", exitCode: 1 }))
		expect(second.container.querySelector(".bg-red-600")).toBeInTheDocument()
	})

	it("ignores status updates that belong to another command", () => {
		render(
			<ExtensionStateWrapper>
				<CommandExecution executionId="test-1" text="npm install" />
			</ExtensionStateWrapper>,
		...
		act(() => sendStatus({ executionId: "other", status: "started", pid: 4242, command: "npm install" }))
	...
	it("ignores a status payload it cannot understand", () => {
		...
		act(() => {
			for (const handler of messageHandlers.current) {
				handler({ data: { type: "commandExecutionStatus", text: "not json" } } as MessageEvent)
			}
		})
	...
	it("ignores messages that are not command status updates", () => {
		render(
			<ExtensionStateWrapper>
				<CommandExecution executionId="test-1" text="npm install" />
			</ExtensionStateWrapper>,
	...
	it("folds the output away and back with the chevron", () => {
		render(
			<ExtensionStateWrapper>
				<CommandExecution executionId="test-1" text={"npm install\nOutput:\nsome output"} />
			</ExtensionStateWrapper>,
		...
		// The output is rendered but collapsed to zero height until the chevron is used.
		expect(document.querySelector(".max-h-0")).toBeInTheDocument()

		fireEvent.click(document.querySelector(".lucide-chevron-down")!.closest("button")!)

		expect(document.querySelector(".max-h-0")).not.toBeInTheDocument()
		expect(screen.getByTestId("terminal-output")).toBeInTheDocument()
	})
})
