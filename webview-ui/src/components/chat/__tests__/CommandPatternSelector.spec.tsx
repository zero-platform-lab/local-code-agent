...
describe("CommandPatternSelector", () => {
	...
	it("lists a repeated pattern only once", () => {
		render(
			<TestWrapper>
				<CommandPatternSelector
					...
					patterns={[
						{ pattern: "npm install", description: "Install npm packages" },
						{ pattern: "npm install", description: "The very same pattern again" },
						{ pattern: "npm *", description: "Any npm command" },
					]}
				/>
			</TestWrapper>,
		...
		const patternsContainer = manageCommandsButton?.nextElementSibling as HTMLElement
		expect(within(patternsContainer).getAllByText("npm install")).toHaveLength(1)
	})

	it("keeps what was typed when the input loses focus", () => {
		render(
			<TestWrapper>
				<CommandPatternSelector {...defaultProps} />
			</TestWrapper>,
		...
		fireEvent.click(getByText("npm install express").closest("div")!)
		const input = getByDisplayValue("npm install express") as HTMLInputElement
		fireEvent.change(input, { target: { value: "npm install react" } })
	...
	it("keeps what was typed when Enter is pressed", () => {
		render(
			<TestWrapper>
				<CommandPatternSelector {...defaultProps} />
			</TestWrapper>,
