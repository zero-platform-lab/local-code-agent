...
describe("FollowUpSuggest", () => {
	...
	describe("choosing a suggestion", () => {
		const state: TestExtensionState = {
			autoApprovalEnabled: true,
			alwaysAllowFollowupQuestions: true,
			followupAutoApproveTimeoutMs: 3000,
		}

		it("hands the chosen suggestion to the parent and stops the countdown", () => {
			renderWithTestProviders(
				<FollowUpSuggest
					suggestions={mockSuggestions}
					onSuggestionClick={mockOnSuggestionClick}
			...
			expect(mockOnCancelAutoApproval).toHaveBeenCalled()
			expect(screen.queryByText(/timerPrefix/)).not.toBeInTheDocument()
		})

		it("keeps the countdown running for a shift-click, which only copies to the input", () => {
			renderWithTestProviders(
				<FollowUpSuggest
					suggestions={mockSuggestions}
					onSuggestionClick={mockOnSuggestionClick}
		...
		it("treats the copy-to-input control as a shift-click and stops auto-approval", () => {
			const { container } = renderWithTestProviders(
				<FollowUpSuggest
					suggestions={mockSuggestions}
					onSuggestionClick={mockOnSuggestionClick}
			...
			fireEvent.click(container.querySelector("svg.lucide-clipboard-copy")!.parentElement!)
		...
		it("shows which mode a suggestion would switch to", () => {
			renderWithTestProviders(
				<FollowUpSuggest
					suggestions={[{ answer: "Do it", mode: "architect" }]}
					onSuggestionClick={mockOnSuggestionClick}
					ts={1}
				/>,
				state,
		...
		it("falls back to the default timeout when the configured one is not a number", () => {
			renderWithTestProviders(
				<FollowUpSuggest suggestions={mockSuggestions} onSuggestionClick={mockOnSuggestionClick} ts={1} />,
				{ ...state, followupAutoApproveTimeoutMs: undefined as unknown as number },
			)

			// The default is 60s, so the countdown starts at 60 rather than at 3.
