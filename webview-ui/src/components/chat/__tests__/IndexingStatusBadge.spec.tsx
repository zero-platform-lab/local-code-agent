...
describe("IndexingStatusBadge", () => {
	...
	const sendStatus = (values: Record<string, unknown>) => {
		act(() => {
			window.dispatchEvent(new MessageEvent("message", { data: { type: "indexingStatusUpdate", values } }))
		})
	}

	it("ignores an indexing update that belongs to another workspace", async () => {
		renderComponent()

		sendStatus({ systemStatus: "Error", workspacePath: "/some/other/workspace" })

		await waitFor(() => expect(screen.getByRole("button")).toHaveAttribute("aria-label", "Index ready"))
	})

	it("shows the stopping state while indexing is being cancelled", async () => {
		renderComponent()

		sendStatus({ systemStatus: "Stopping" })

		await waitFor(() => expect(screen.getByRole("button")).toHaveAttribute("aria-label", "indexingStatus.stopping"))
		expect(screen.getByRole("button").querySelector("span.rounded-full")?.className).toContain("bg-amber-500")
	})

	it("falls back to a neutral badge for a status it does not know", async () => {
		const { container } = renderComponent()

		sendStatus({ systemStatus: "SomethingNew" })

		await waitFor(() => expect(screen.getByRole("button")).toHaveAttribute("aria-label", "indexingStatus.status"))
		expect(container.querySelector("span.rounded-full")?.className).toContain("bg-vscode-descriptionForeground/60")
	})
})
