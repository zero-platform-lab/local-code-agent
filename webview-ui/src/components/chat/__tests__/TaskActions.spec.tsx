...
describe("TaskActions", () => {
	...
	it("asks for confirmation before deleting a task", () => {
		...
		fireEvent.click(screen.getByLabelText("Delete Task (Shift + Click to skip confirmation)"))

		// Nothing is deleted yet: the dialog has to be confirmed first.
		expect(mockPostMessage).not.toHaveBeenCalledWith({ type: "deleteTaskWithId", text: "test-task-id" })
		expect(screen.getByRole("alertdialog")).toBeInTheDocument()
	})

	it("deletes straight away on a shift-click", () => {
		render(<TaskActions item={mockItem} buttonsDisabled={false} />)
		...
		expect(mockPostMessage).toHaveBeenCalledWith({ type: "deleteTaskWithId", text: "test-task-id" })
		expect(screen.queryByRole("alertdialog")).not.toBeInTheDocument()
	})

	it("closes the confirmation dialog again when it is dismissed", async () => {
		render(<TaskActions item={mockItem} buttonsDisabled={false} />)
	...
	it("does not offer a copy button for a task without a prompt", () => {
		render(<TaskActions item={{ ...mockItem, task: "" }} buttonsDisabled={false} />)

		expect(screen.queryByLabelText("Copy")).not.toBeInTheDocument()
	})

	it("does not offer any task actions when there is no task at all", () => {
	...
	it("opens the API and UI history from the debug buttons", () => {
		mockUseExtensionState.mockReturnValue({ debug: true } as any)
		render(<TaskActions item={mockItem} buttonsDisabled={false} />)
	...
	it("acknowledges a copied prompt with a check mark", () => {
		mockUseCopyToClipboard.mockReturnValue({ copyWithFeedback: vi.fn(), showCopyFeedback: true })

		const { container } = render(<TaskActions item={mockItem} buttonsDisabled={false} />)
