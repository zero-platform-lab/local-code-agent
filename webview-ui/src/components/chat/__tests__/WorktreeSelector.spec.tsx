...
vi.mock("../../worktrees/CreateWorktreeModal", () => ({
	CreateWorktreeModal: ({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) => (
		<div data-testid="create-worktree-modal">
			<button data-testid="create-worktree-close" onClick={onClose} />
			<button data-testid="create-worktree-success" onClick={onSuccess} />
		</div>
	),
}))
...
describe("WorktreeSelector", () => {
	...
	test("treats a worktree list message with no list as an empty list", () => {
		render(<WorktreeSelector />)
		simulateWorktreeListMessage(mockWorktrees)
		expect(screen.getByTestId("worktree-selector-trigger")).toBeInTheDocument()

		act(() => {
			window.dispatchEvent(new MessageEvent("message", { data: { type: "worktreeList", isGitRepo: true } }))
		})

		// Nothing to choose between, so the selector gets out of the way.
		expect(screen.queryByTestId("worktree-selector-trigger")).not.toBeInTheDocument()
	})

	test("names a detached current worktree as having no branch", () => {
		render(<WorktreeSelector />)

		simulateWorktreeListMessage([
			{
				path: "/path/to/detached",
				branch: "",
				commitHash: "abc123",
		...
		expect(screen.getByTestId("worktree-selector-trigger")).toHaveTextContent("worktrees:noBranch")
	})

	test("opens the create dialog from the list and closes the list with it", async () => {
		render(<WorktreeSelector />)
		simulateWorktreeListMessage(mockWorktrees)

		fireEvent.click(screen.getByTestId("worktree-selector-trigger"))
		fireEvent.click(await screen.findByText("worktrees:newWorktree"))

		expect(screen.queryByText("worktrees:newWorktree")).not.toBeInTheDocument()
		expect(await screen.findByTestId("create-worktree-modal")).toBeInTheDocument()
	})

	test("closes the create dialog when it is dismissed", async () => {
		render(<WorktreeSelector />)
		simulateWorktreeListMessage(mockWorktrees)

		fireEvent.click(screen.getByTestId("worktree-selector-trigger"))
		fireEvent.click(await screen.findByText("worktrees:newWorktree"))
		fireEvent.click(screen.getByTestId("create-worktree-close"))

		expect(screen.queryByTestId("create-worktree-modal")).not.toBeInTheDocument()
	})

	test("refreshes the list after a worktree was created", async () => {
		render(<WorktreeSelector />)
		simulateWorktreeListMessage(mockWorktrees)
		...
		fireEvent.click(await screen.findByText("worktrees:newWorktree"))
		mockPostMessage.mockClear()
		fireEvent.click(screen.getByTestId("create-worktree-success"))
