...
describe("CheckpointSaved popover visibility", () => {
	...
	it("renders nothing when the checkpoint does not match the expected shape", () => {
		const { container } = render(
			<CheckpointSaved {...baseProps} checkpoint={{ from: 1, to: false } as Record<string, unknown>} />,
		)

		expect(container).toBeEmptyDOMElement()
	})

	it("drops its pending close timer when it unmounts", () => {
		vi.useFakeTimers()

		try {
			const { unmount } = render(<CheckpointSaved {...baseProps} />)
			expect(lastOnOpenChange).toBeTruthy()

			// Closing the popover schedules the "briefly keep the menu visible" timer.
			lastOnOpenChange!(true)
			lastOnOpenChange!(false)
			unmount()
