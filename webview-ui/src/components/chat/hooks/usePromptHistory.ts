...
export const usePromptHistory = ({
	...
	const setCursorPosition = useCallback(
		(textarea: HTMLTextAreaElement, position: "start" | "end", length: number) => {
			setTimeout(() => {
				const offset = position === "start" ? 0 : length
	...
	const navigateToHistory = useCallback(
		(newIndex: number, textarea: HTMLTextAreaElement, cursorPos: "start" | "end" = "start"): boolean => {
			...
			/* v8 ignore next -- 到達不能: promptHistory は空文字を除外して作られ、直前で範囲も確認済みのため空要素は存在しない。防御は残す */
