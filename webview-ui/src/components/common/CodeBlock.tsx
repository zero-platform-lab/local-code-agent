	...
	({
		...
		useEffect(() => {
			...
			const fallback = (
				<pre style={{ padding: 0, margin: 0 }}>
					<code className={`hljs language-${currentLanguage}`}>{source || ""}</code>
				</pre>
			)
		...
		useEffect(() => {
			...
			const handleWheel = (e: WheelEvent) => {
				...
				/* v8 ignore next -- 到達不能: このリスナは preRef.current 自身に張られ、アンマウント時に外されるため、発火時に ref が空になることはない。防御は残す */
				if (!preRef.current) return
		...
		useEffect(() => {
			...
			// The listener below is bound to the pre element itself, so any press that reaches it
			// (directly or from the highlighted code inside) starts a selection.
