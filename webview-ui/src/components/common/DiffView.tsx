...
const DiffView = memo(({ source, filePath }: DiffViewProps) => {
	...
	const isLightTheme = useMemo(() => {
		/* v8 ignore next -- 到達不能: webview は常に DOM 上で動くため document が無い実行環境は存在しない。SSR 向けの防御は残す */
	...
	const renderContent = (line: DiffLine, hunk: Hunk, lineIndexInHunk: number): React.ReactNode => {
		...
		if (line.type === "deletion") {
			...
			const deletionIndex = hunkLinesBeforeThis.filter(
				(l) => l.type === "deletion" || l.type === "context",
			).length
			return hunk.highlightedOldLines[deletionIndex] || line.content
		}

		if (line.type === "addition") {
			// Count additions and context lines before this line
			const additionIndex = hunkLinesBeforeThis.filter(
				(l) => l.type === "addition" || l.type === "context",
			).length
			return hunk.highlightedNewLines[additionIndex] || line.content
		}

		// Whatever is left is a context line: gap lines never get here because the table renders
		// them with their own row. Prefer new-side highlighting, fall back to old-side.
