...
describe("DismissibleUpsell", () => {
	...
	it("dismisses on container click when dismissOnClick is true and no onClick is provided", async () => {
		const onDismiss = vi.fn()
		const { container } = render(
	...
	it("renders the icon it was given alongside the content", async () => {
		render(
			<DismissibleUpsell upsellId="test-upsell" icon={<span data-testid="upsell-icon" />}>
				<div>Test content</div>
			</DismissibleUpsell>,
		)
		...
		await waitFor(() => expect(screen.getByTestId("upsell-icon")).toBeInTheDocument())
	})

	it("renders nothing in the icon slot when no icon was given", async () => {
		const { container } = render(
			<DismissibleUpsell upsellId="test-upsell">
				<div>Test content</div>
			</DismissibleUpsell>,
		...
		expect(container.querySelector("[data-testid='upsell-icon']")).not.toBeInTheDocument()
	})
})
