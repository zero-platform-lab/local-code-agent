...
describe("SubtaskRow", () => {
	...
	describe("keyboard use", () => {
		it("opens the task with Enter and with Space", () => {
			const node = createMockNode({ id: "leaf-1", task: "Leaf task content" })
			render(<SubtaskRow node={node} depth={1} onToggleExpand={vi.fn()} />)
			const row = screen.getByText("Leaf task content").closest("[role='button']")!

			fireEvent.keyDown(row, { key: "Enter" })
			fireEvent.keyDown(row, { key: " " })

			expect(vscode.postMessage).toHaveBeenCalledTimes(2)
			expect(vscode.postMessage).toHaveBeenCalledWith({ type: "showTaskWithId", text: "leaf-1" })
		})
		...
		it("ignores other keys", () => {
			...
			fireEvent.keyDown(screen.getByText("Leaf task content").closest("[role='button']")!, { key: "a" })
