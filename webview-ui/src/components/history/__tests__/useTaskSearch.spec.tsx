...
describe("useTaskSearch", () => {
	...
	describe("tasks with missing numbers", () => {
		const sparseHistory = [
			{
				id: "sparse-1",
				number: 1,
				task: "No numbers at all",
				ts: new Date("2022-02-10T12:00:00").getTime(),
				tokensIn: 0,
				tokensOut: 0,
				totalCost: 0,
				workspace: "/workspace/project1",
			...
			},
			{
				id: "sparse-3",
				number: 3,
				task: "Also nothing but zeroes",
				ts: new Date("2022-02-11T12:00:00").getTime(),
				tokensIn: 0,
				tokensOut: 0,
				totalCost: 0,
				workspace: "/workspace/project1",
		...
		beforeEach(() => {
			mockUseExtensionState.mockReturnValue({
				taskHistory: sparseHistory,
				cwd: "/workspace/project1",
			} as any)
		})

		it("orders by timestamp in both directions", () => {
			const { result } = renderHook(() => useTaskSearch())

			expect(result.current.tasks[0].id).toBe("sparse-2")

			act(() => {
				result.current.setSortOption("oldest")
			})

			expect(result.current.tasks[0].id).toBe("sparse-1")
		})

		it("treats a task without a cost as the cheapest", () => {
			const { result } = renderHook(() => useTaskSearch())
		...
		it("treats a task without token counts as the smallest", () => {
			const { result } = renderHook(() => useTaskSearch())
		...
		it("falls back to newest first for the relevance sort when nothing is being searched", () => {
			const { result } = renderHook(() => useTaskSearch())

			act(() => {
				// Without a sort to fall back to, the hook leaves "mostRelevant" in place.
				result.current.setLastNonRelevantSort(null)
			})
			act(() => {
				result.current.setSortOption("mostRelevant")
		...
		it("keeps the search ranking for the relevance sort while searching", () => {
			const { result } = renderHook(() => useTaskSearch())
			...
			act(() => {
				// Matches both "No numbers at all" and "Also nothing but zeroes".
				result.current.setSearchQuery("no")
			})

			// The fuzzy search decides the order; the sort must not reshuffle it by date.
			expect(result.current.sortOption).toBe("mostRelevant")
			expect(result.current.tasks).toHaveLength(2)
			expect(result.current.tasks[0].id).toBe("sparse-1")
		})
