...
export const useTaskSearch = () => {
	...
	const tasks = useMemo(() => {
		...
		return [...results].sort((a, b) => {
			switch (sortOption) {
				// presentableTasks only keeps items that have a timestamp, so ts is always set here.
				case "oldest":
					return a.ts - b.ts
				case "mostExpensive":
				...
				// "newest", and anything unexpected, falls back to newest first.
				default:
					return b.ts - a.ts
				/* v8 ignore next -- 人工分岐: どの case も return するため、switch を抜けて関数末尾へ落ちる合成経路は存在しない */
			}
		})
