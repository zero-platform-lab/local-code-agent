...
const McpToolRow = ({ tool, serverName, serverSource, alwaysAllowMcp, isInChatContext = false }: McpToolRowProps) => {
	...
	const handleAlwaysAllowChange = () => {
		/* v8 ignore next -- 到達不能: このハンドラは serverName が真のときだけ描画される Always-allow チェックボックス（L70 の `serverName &&` ガード下）にのみ束縛されるため、UI 経由で serverName 偽の呼び出しは発生しない。防御ガードは残す */
	...
	const handleEnabledForPromptChange = () => {
		/* v8 ignore next -- 到達不能: このハンドラは serverName が真のときだけ描画されるトグルスイッチ（L70 の `serverName &&` ガード下）にのみ束縛されるため、UI 経由で serverName 偽の呼び出しは発生しない。防御ガードは残す */
		if (!serverName) return
		vscode.postMessage({
