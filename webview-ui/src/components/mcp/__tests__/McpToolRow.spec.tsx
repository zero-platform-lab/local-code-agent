...
describe("McpToolRow", () => {
	...
	// enabledForPrompt が未定義なら「有効」既定にフォールバックする（L19 の `?? true`）。
	it("treats a tool with no enabledForPrompt flag as enabled", () => {
		const toolWithoutFlag = {
			name: "no-flag-tool",
			description: "desc",
			// enabledForPrompt をあえて省く
		}
		render(<McpToolRow tool={toolWithoutFlag} serverName="test-server" />)

		const toggleSwitch = screen.getByRole("switch", { name: "Toggle prompt inclusion" })
		// 既定 true なので checked 表示になる
		expect(toggleSwitch).toHaveAttribute("aria-checked", "true")
	})

	// serverSource を渡すと "global" 既定ではなくその値が使われる（L26/L37 の `serverSource || "global"` 左辺）。
	it("uses the provided serverSource instead of defaulting to global", () => {
		render(<McpToolRow tool={mockTool} serverName="test-server" serverSource="project" alwaysAllowMcp={true} />)
		...
		fireEvent.click(screen.getByRole("switch", { name: "Toggle prompt inclusion" }))
		expect(vscode.postMessage).toHaveBeenCalledWith({
			type: "toggleToolEnabledForPrompt",
			serverName: "test-server",
			source: "project",
	...
	// スキーマの説明が無いパラメータは "No description" にフォールバックする（L131 の `|| noDescription`）。
	it("falls back to noDescription when a parameter has no schema description", () => {
		const toolWithBlankParam = {
			...mockTool,
			inputSchema: {
				type: "object",
				properties: {
					bareParam: { type: "string" },
				},
			},
		}
		render(<McpToolRow tool={toolWithBlankParam} serverName="test-server" />)

		expect(screen.getByText("bareParam")).toBeInTheDocument()
