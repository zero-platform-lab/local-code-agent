...
import type { ProviderSettingsEntry, OrganizationAllowList } from "@openai-agent/types"
...
const ApiConfigManager = ({
	...
	const validateName = (name: string, isNewProfile: boolean): string | null => {
		...
		/* v8 ignore next -- 到達不能: 作成・保存ボタンはどちらも空欄では disabled のため、空名でここへは来ない。防御は残す */
	...
	const handleStartRename = () => {
		setIsRenaming(true)
		/* v8 ignore next -- 到達不能: 名前変更ボタンは currentApiConfigName があるときだけ描かれる。防御既定は残す */
	...
	const handleDelete = () => {
		/* v8 ignore next -- 到達不能: 削除ボタンは currentApiConfigName があるときだけ描かれる。残りの防御は残す */
