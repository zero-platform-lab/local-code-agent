						...
						<div className="flex flex-wrap gap-2">
							{(allowedCommands ?? []).map((cmd, index) => (
								<Button
									...
									onClick={() => {
										/* v8 ignore next -- 到達不能: このボタンは allowedCommands を map して描くため、その時点で配列は必ず存在する。防御既定は残す */
						...
						<div className="flex flex-wrap gap-2">
							{(deniedCommands ?? []).map((cmd, index) => (
								<Button
									...
									onClick={() => {
										/* v8 ignore next -- 到達不能: このボタンは deniedCommands を map して描くため、その時点で配列は必ず存在する。防御既定は残す */
