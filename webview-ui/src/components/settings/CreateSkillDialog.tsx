...
const validateDescription = (description: string): string | null => {
	/* v8 ignore next -- 到達不能: 説明が空の間は作成ボタンが disabled のため、空文字ではここへ来ない。防御は残す */
...
export const CreateSkillDialog: React.FC<CreateSkillDialogProps> = ({
	...
	// 入力途中でも規則違反を知らせる。文字種フィルタを厳しくすると `foo-bar` を打つ途中の
	// `foo-` で末尾ハイフンが消えてハイフンが打てなくなるため、削らずに指摘する方を採る。
	const liveNameError = useMemo(() => describeSkillNameProblem(name), [name])
	...
	// （詳細はそれぞれのコメント）。
	//
	// 注意: このフィルタは正規表現より緩い。`-foo` / `foo--bar` / `--` は入力できてしまい、
	// 送信時の validateSkillName で初めて弾かれる。
	...
	const handleCreate = useCallback(() => {
		...
		/* v8 ignore next 4 -- 到達不能: 名前が不正な間は作成ボタンが liveNameError で disabled になるため、ここには正しい名前でしか来ない。防御は残す */
			...
			<DialogContent className="sm:max-w-md">
				...
				<DialogFooter>
					...
					<Button
						...
						disabled={!name || !description || !!liveNameError}>
