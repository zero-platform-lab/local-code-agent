...
const validateCommandName = (name: string): string | null => {
	/* v8 ignore next -- 到達不能: 入力フィルタが空白を落とし、空名では作成ボタンが disabled になるため空文字ではここへ来ない。防御は残す */
	...
	/* v8 ignore next -- 到達不能: 入力フィルタが同じ文字集合しか通さないため、ここで弾かれる名前は入力できない。防御は残す */
...
export const CreateSlashCommandDialog: React.FC<CreateSlashCommandDialogProps> = ({
	...
	const handleCreate = useCallback(() => {
		...
		// The input filter above drops dots, so the name never carries an extension yet.
