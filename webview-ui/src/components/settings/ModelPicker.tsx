...
import { type ProviderSettings, type ModelInfo, type OrganizationAllowList } from "@openai-agent/types"
...
export const ModelPicker = ({
	...
	const onSelect = useCallback(
		(modelId: string) => {
			/* v8 ignore next 3 -- 到達不能: 選択肢と「このモデルを使う」項目はどちらも非空の value を渡すため空の modelId では呼ばれない。防御は残す */
			if (!modelId) {
				return
			}
	...
	useEffect(() => {
		if (!selectedModelId && !isInitialized.current) {
			// Nothing is selected yet, so the default is the only candidate.
