...
import { supportPrompt, SupportPromptType } from "@agent/support-prompt"
...
const PromptsSettings = ({
	...
	const updateSupportPrompt = (type: SupportPromptType, value: string | undefined) => {
		// Don't trim during editing: intentional whitespace and empty strings are preserved.
		const finalValue = value

		const updatedPrompts = { ...customSupportPrompts }
		/* v8 ignore next 2 -- 到達不能: 入力ハンドラは常に文字列を渡し、削除はリセットボタン（handleSupportReset）が担う。防御は残す */
