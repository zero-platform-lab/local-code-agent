...
export function SettingsSearch({ index, onNavigate, sections }: SettingsSearchProps) {
	...
	const moveHighlight = useCallback(
		(direction: 1 | -1) => {
			/* v8 ignore next -- 到達不能: 呼び出し元 handleSearchKeyDown が results.length を先に確認してから呼ぶため空にはならない防御既定 */
			if (!results.length) return
			const flatIds = results.map((r) => r.settingId)
			/* v8 ignore next -- 到達不能: dropdown が開いて results があるときは効果で highlightedResultId が必ず設定されるため : -1 は踏めない防御既定 */
