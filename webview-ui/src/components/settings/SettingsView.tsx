...
import { shouldMarkDirty } from "./settingsChangeDetection"
...
const SettingsView = forwardRef<SettingsViewRef, SettingsViewProps>(({ onDone, targetSection }, ref) => {
	...
	const setApiConfigurationField = useCallback(
		<K extends keyof ProviderSettings>(field: K, value: ProviderSettings[K], isUserAction: boolean = true) => {
			setCachedState((prevState) => {
				...
				if (shouldMarkDirty(field, previousValue, value, isUserAction)) {
					setChangeDetected(true)
				}
		...
		if (isSettingValid) {
			vscode.postMessage({
				...
				updatedSettings: {
					...
					mcpEnabled,
					maxOpenTabsContext: Math.min(Math.max(0, maxOpenTabsContext ?? DEFAULT_MAX_OPEN_TABS_CONTEXT), 500),
					maxWorkspaceFiles: Math.min(Math.max(0, maxWorkspaceFiles ?? DEFAULT_MAX_WORKSPACE_FILES), 500),
	...
	useLayoutEffect(() => {
		if (contentRef.current) {
			// scrollPositions は全セクション分を 0 で初期化済みなので未設定にはならない。
	...
	useEffect(() => {
		/* v8 ignore next -- 到達不能: containerRef は同じ描画ツリーの div に必ず紐づくため、この effect の実行時点で null にならない。将来の描画変更に備えた防御は残す */
						...
						{renderTab === "general" && (
							<div>
								...
								<PiiSettings
									...
									setPiiMasking={(value) => setCachedStateField("piiMasking", value)}
								/>
