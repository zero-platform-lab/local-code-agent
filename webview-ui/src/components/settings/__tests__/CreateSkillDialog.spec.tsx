...
describe("CreateSkillDialog", () => {
	...
	describe("modes, source and validation", () => {
		const renderDialog = (props: Partial<React.ComponentProps<typeof CreateSkillDialog>> = {}) =>
			render(
				<CreateSkillDialog
					open
					onOpenChange={mockOnOpenChange}
		...
		const fillIn = (name = "my-skill", description = "My skill description") => {
			fireEvent.change(screen.getByPlaceholderText("settings:skills.createDialog.namePlaceholder"), {
				target: { value: name },
			})
			fireEvent.change(screen.getByPlaceholderText("settings:skills.createDialog.descriptionPlaceholder"), {
				target: { value: description },
			})
		}

		const create = () =>
			fireEvent.click(screen.getAllByTestId("button").find((b) => b.getAttribute("data-variant") === "primary")!)
		...
		it("stores no restriction when nothing is ticked at all", async () => {
			renderDialog()
			fillIn()
			fireEvent.click(screen.getByTestId("checkbox-create-mode-any"))
			...
			await waitFor(() =>
				expect(vscode.postMessage).toHaveBeenCalledWith(expect.objectContaining({ skillModeSlugs: undefined })),
			)
		})

		it("writes globally when there is no workspace", async () => {
			...
			await waitFor(() =>
				expect(vscode.postMessage).toHaveBeenCalledWith(expect.objectContaining({ source: "global" })),
			)
		})

		it("writes where the user chose to", async () => {
			renderDialog()
			fillIn()
		...
		it("refuses a description that is too long", () => {
			renderDialog()
			fillIn("my-skill", "x".repeat(1025))

			create()

			expect(screen.getByText("settings:skills.validation.descriptionTooLong")).toBeInTheDocument()
			expect(vscode.postMessage).not.toHaveBeenCalled()
		})

		it("cannot be submitted while the name is invalid", () => {
			renderDialog()
			fillIn("-bad-name-", "fine")
		...
		it("forgets everything that was typed when it is cancelled", () => {
			renderDialog()
			fillIn()

			fireEvent.click(
				screen.getAllByTestId("button").find((b) => b.getAttribute("data-variant") === "secondary")!,
			)
