...
describe("SkillsSettings", () => {
	...
	describe("choosing which modes a skill applies to", () => {
		const openModeDialog = (skillName: string) => {
			const row = screen.getByText(skillName).closest("div.flex")!.parentElement!.parentElement!
			fireEvent.click(row.querySelectorAll("button")[0])
		}

		it("starts on 'any mode' for a skill that names none", () => {
			renderSkillsSettings()

			openModeDialog("project-skill")

			expect(screen.getByTestId("checkbox-mode-any")).toBeChecked()
			expect(screen.getByTestId("checkbox-mode-architect")).not.toBeChecked()
		})

		it("starts with the modes the skill already names", () => {
			renderSkillsSettings()

			openModeDialog("project-mode-skill")

			expect(screen.getByTestId("checkbox-mode-any")).not.toBeChecked()
			expect(screen.getByTestId("checkbox-mode-architect")).toBeChecked()
		})

		it("clears the specific modes when 'any mode' is picked", () => {
			renderSkillsSettings()
			openModeDialog("project-mode-skill")
		...
		it("keeps 'any mode' on when it is unticked on its own", () => {
			renderSkillsSettings()
			openModeDialog("project-skill")
		...
		it("drops 'any mode' as soon as a specific mode is picked", () => {
			renderSkillsSettings()
			openModeDialog("project-skill")
			...
			expect(screen.getByTestId("checkbox-mode-any")).not.toBeChecked()
			expect(screen.getByTestId("checkbox-mode-code")).toBeChecked()
		})

		it("falls back to 'any mode' when the last specific mode is removed", () => {
			renderSkillsSettings()
			openModeDialog("project-mode-skill")

			fireEvent.click(screen.getByTestId("checkbox-mode-architect"))

			expect(screen.getByTestId("checkbox-mode-any")).toBeChecked()
		})

		it("keeps the remaining modes when one of several is removed", () => {
			renderSkillsSettings()
			openModeDialog("project-skill")
			fireEvent.click(screen.getByTestId("checkbox-mode-code"))
			fireEvent.click(screen.getByTestId("checkbox-mode-architect"))
		...
		it("saves 'any mode' as no restriction at all", () => {
			renderSkillsSettings()
			openModeDialog("project-mode-skill")
			fireEvent.click(screen.getByTestId("checkbox-mode-any"))
			...
			expect(vscode.postMessage).toHaveBeenCalledWith(expect.objectContaining({ newSkillModeSlugs: undefined }))
		})
	...
	it("saves no restriction when neither 'any mode' nor a specific mode is ticked", () => {
		renderSkillsSettings()
		...
		expect(vscode.postMessage).toHaveBeenCalledWith(
			expect.objectContaining({ type: "updateSkillModes", newSkillModeSlugs: undefined }),
		)
	})
})
