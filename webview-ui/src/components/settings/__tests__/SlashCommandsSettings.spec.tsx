...
describe("SlashCommandsSettings", () => {
	...
	it("opens the command file that a command came from", () => {
		renderSlashCommandsSettings()

		fireEvent.click(screen.getAllByTestId("button").find((button) => button.textContent === "")!)

		expect(vscode.postMessage).toHaveBeenCalledWith({ type: "openFile", text: expect.any(String) })
	})

	it("asks the extension to find the file when a command has no path", () => {
		renderSlashCommandsSettings([{ name: "no-path", description: "d", source: "global" } as Command])
	...
	it("refreshes the list shortly after a command was created", () => {
		vi.useFakeTimers()

		try {
			renderSlashCommandsSettings()
			fireEvent.click(screen.getByText("settings:slashCommands.addCommand").closest("button")!)
			fireEvent.click(screen.getByTestId("create-command-button"))
	...
	it("treats a missing command list as an empty one", () => {
		mockExtensionState = { commands: undefined, cwd: "/workspace" }
