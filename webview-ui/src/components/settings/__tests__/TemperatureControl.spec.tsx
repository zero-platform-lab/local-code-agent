...
vi.mock("@/components/ui", () => ({
	...
	Slider: ({ value, onValueChange, "data-testid": dataTestId }: any) => (
		<>
			<input type="range" value={value[0]} readOnly data-testid={dataTestId} role="slider" />
			<button data-testid="slider-set" onClick={() => onValueChange([0.5])}>
				set
			</button>
		</>
	),
...
describe("TemperatureControl", () => {
	...
	it("treats a null value as no custom temperature", () => {
		// Initial render sees value=null: isCustomTemperature starts true and the slider
		// evaluates inputValue ?? 0, then the effect corrects it to unchecked.
		const onChange = vi.fn()
		render(<TemperatureControl value={null} onChange={onChange} />)
		expect(screen.getByRole("checkbox")).not.toBeChecked()
	})

	it("falls back to defaultValue when enabling with no current value", async () => {
		const onChange = vi.fn()
		render(<TemperatureControl value={undefined} onChange={onChange} defaultValue={0.3} />)
	...
	it("falls back to 0 when enabling with neither value nor defaultValue", async () => {
		const onChange = vi.fn()
		render(<TemperatureControl value={undefined} onChange={onChange} />)
		fireEvent.click(screen.getByRole("checkbox"))
		await waitFor(() => {
	...
	it("propagates slider changes through the debounced onChange", async () => {
		const onChange = vi.fn()
		render(<TemperatureControl value={0.7} onChange={onChange} />)
		fireEvent.click(screen.getByTestId("slider-set"))
		await waitFor(() => {
