...
describe("OpenAICompatible Component - includeMaxTokens checkbox", () => {
	...
	describe("Initial State", () => {
		...
		// backend は includeMaxTokens === true のときだけ max_completion_tokens を送る。
		// UI もそれに合わせ、未設定(undefined/null)は unchecked にする（旧実装は ?? true で
		// checked に見えるのに何も送らない、という UI と挙動の食い違いがあった）。
		it("should default to UNCHECKED when includeMaxTokens is undefined (matches backend === true)", () => {
			const apiConfiguration: Partial<ProviderSettings> = {
		...
		it("should default to UNCHECKED when includeMaxTokens is null (matches backend === true)", () => {
			const apiConfiguration: Partial<ProviderSettings> = {
