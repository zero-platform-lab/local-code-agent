		...
		(
			...
			const handleSelect = React.useCallback(
				(optionValue: string) => {
					...
					/* v8 ignore next -- 到達不能: 呼び出し元は options から描いた項目の value をそのまま渡すため必ず見つかる。防御は残す */
					if (!option) return
					...
					/* v8 ignore next -- 到達不能: 項目の onClick が `!option.disabled` を先に確認するため、無効な項目ではここへ来ない。防御は残す */
