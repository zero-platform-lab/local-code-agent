...
export const DeleteWorktreeModal = ({ open, onClose, worktree, onSuccess }: DeleteWorktreeModalProps) => {
	...
	const handleDelete = useCallback(() => {
		...
		// force は必ずユーザーが明示的に選んだときだけ。
		// 以前は locked 以外を無条件に force にしていたので、通常の削除経路が
		// `git worktree remove --force` になり、未コミットの変更と未追跡ファイルが
		// 確認なしに消えていた（復元手段なし）。force を付けなければ git が拒否するので、
		// 失われるものがある場合は利用者にエラーとして見える。
		vscode.postMessage({
			type: "deleteWorktree",
		...
		<Dialog open={open} onOpenChange={(isOpen: boolean) => !isOpen && onClose()}>
			<DialogContent>
				...
				<div className="flex flex-col gap-3 overflow-hidden">
					...
					{/* 強制削除は常に明示的なオプトイン。未コミットの変更がある worktree や
					    locked な worktree は、これを入れない限り git が削除を拒否する。 */}
