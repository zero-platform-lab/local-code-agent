...
import { Mode, defaultModeSlug } from "@agent/modes"
...
export interface ExtensionStateContextType extends ExtensionState {
	...
	setShowAgentIgnoredFiles: (value: boolean) => void
...
export const ExtensionStateContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	const [state, setState] = useState<ExtensionState>({
		...
		terminalShellIntegrationTimeout: DEFAULT_SHELL_INTEGRATION_TIMEOUT_MS,
		mcpEnabled: true,
		currentApiConfigName: "default",
		listApiConfigMeta: [],
		mode: defaultModeSlug,
		// 組み込み既定を事前投入しないこと。このフィールドは「組み込みプロンプトの上書き」を
		// 保持するもので、`{}` が「上書きなし」の正しい表現（拡張側も `{}` を送る）。
		//
		// 既定を入れておくと、下の mergeExtensionState が浅いマージなので拡張から届く `{}`
		// では消えず、設定画面の保存経路が「組み込み既定をユーザーの上書きとして」永続化して
		// しまう。将来 既定を変更しても古い文面が残り続ける。
		customModePrompts: {},
		customSupportPrompts: {},
		...
		cwd: "",
		showAgentIgnoredFiles: DEFAULT_SHOW_AGENT_IGNORED_FILES,
		...
		maxImageFileSize: DEFAULT_MAX_IMAGE_FILE_SIZE_MB, // Default max image file size in MB
		maxTotalImageSize: DEFAULT_MAX_TOTAL_IMAGE_SIZE_MB, // Default max total image size in MB
		...
		codebaseIndexModels: { "openai-compatible": {} },
