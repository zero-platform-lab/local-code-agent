...
const TestComponent = () => {
	const { allowedCommands, setAllowedCommands, showAgentIgnoredFiles, setShowAgentIgnoredFiles } = useExtensionState()
...
describe("ExtensionStateContext", () => {
	...
	it("initializes showAgentIgnoredFiles from the shared default", () => {
		render(
			<ExtensionStateContextProvider>
				<TestComponent />
			</ExtensionStateContextProvider>,
		...
		// 値をリテラルで書き直すとまた拡張ホスト側とずれるので、共有定数を参照する。
		expect(JSON.parse(screen.getByTestId("show-agentignored-files").textContent!)).toBe(
			DEFAULT_SHOW_AGENT_IGNORED_FILES,
		)
	})
...
describe("mergeExtensionState", () => {
	it("should correctly merge extension states", () => {
		...
		const prevState: ExtensionState = {
			...baseState,
			apiConfiguration: { modelTemperature: 0.1 },
			experiments: {} as Record<ExperimentId, boolean>,
		...
		const newState: ExtensionState = {
			...baseState,
			apiConfiguration: { modelTemperature: 0.3 },
			experiments: {
