...
describe("insertMention — non-latin text after the cursor", () => {
	it("keeps text that is not a latin word intact instead of eating it", () => {
		// Japanese/Chinese/Korean text has no spaces between words: stripping "the rest of the
		// word" would delete the sentence the user already typed.
		const { newValue, mentionIndex } = insertMention("@f世界", 2, "src/a.ts")

		expect(newValue).toBe("@src/a.ts 世界")
		expect(mentionIndex).toBe(0)
	})

	it("still strips the partially typed latin word it replaces", () => {
		const { newValue } = insertMention("@fi rest", 3, "src/a.ts")

		expect(newValue).toBe("@src/a.ts  rest")
	})
})

describe("getContextMenuOptions — empty query with a selected type", () => {
	const files: ContextMenuQueryItem[] = [{ type: ContextMenuOptionType.File, value: "/src/a.ts" }]
	const folders: ContextMenuQueryItem[] = [{ type: ContextMenuOptionType.Folder, value: "/src" }]
	const commits: ContextMenuQueryItem[] = [{ type: ContextMenuOptionType.Git, value: "abc1234" }]

	it("lists folders for the folder type", () => {
		const result = getContextMenuOptions("", ContextMenuOptionType.Folder, [...folders, ...files])

		expect(result).toEqual([{ type: ContextMenuOptionType.Folder, value: "/src" }])
	})

	it("reports no results when nothing of the selected type exists", () => {
		expect(getContextMenuOptions("", ContextMenuOptionType.Folder, files)).toEqual([
			{ type: ContextMenuOptionType.NoResults },
		])
		expect(getContextMenuOptions("", ContextMenuOptionType.File, folders)).toEqual([
			{ type: ContextMenuOptionType.NoResults },
		])
	})

	it("always offers the working changes entry for the git type", () => {
		const withCommits = getContextMenuOptions("", ContextMenuOptionType.Git, commits)
		const withoutCommits = getContextMenuOptions("", ContextMenuOptionType.Git, files)

		expect(withCommits[0]).toMatchObject({ type: ContextMenuOptionType.Git, value: "git-changes" })
		expect(withCommits).toHaveLength(2)
		expect(withoutCommits).toEqual([expect.objectContaining({ value: "git-changes" })])
	})
})

describe("getContextMenuOptions — top level suggestions", () => {
	const items: ContextMenuQueryItem[] = [
		{ type: ContextMenuOptionType.Git, value: "abc1234", label: "Initial commit" },
	]

	it("offers the working changes entry once the query passes 'git'", () => {
		const result = getContextMenuOptions("git-c", null, items)

		expect(result[0]).toMatchObject({ type: ContextMenuOptionType.Git, value: "git-changes" })
	})

	it("offers problems and terminal for their prefixes", () => {
		expect(getContextMenuOptions("prob", null, items)[0]).toMatchObject({
			type: ContextMenuOptionType.Problems,
		})
		expect(getContextMenuOptions("term", null, items)[0]).toMatchObject({
			type: ContextMenuOptionType.Terminal,
		})
	})

	it("prefers a known commit over a synthesised one for an exact SHA", () => {
		const result = getContextMenuOptions("abc1234", null, items)

		expect(result[0]).toMatchObject({ value: "abc1234", label: "Initial commit" })
		expect(result.filter((item) => item.value === "abc1234")).toHaveLength(1)
	})
})

describe("getContextMenuOptions — dynamic search results", () => {
	it("falls back to the raw path when a result has neither label nor basename", () => {
		const result = getContextMenuOptions("src", null, [], [{ path: "src/", type: "folder" }])
	...
	it("uses the basename when the result has no label", () => {
		const result = getContextMenuOptions("a", null, [], [{ path: "/src/a.ts", type: "file" }])

		expect(result[0]).toMatchObject({ type: ContextMenuOptionType.File, value: "/src/a.ts", label: "a.ts" })
	})
})

describe("getContextMenuOptions — URL and unknown commit queries", () => {
	const items: ContextMenuQueryItem[] = [{ type: ContextMenuOptionType.Git, value: "abc1234" }]

	it("offers the typed URL as a mention", () => {
		const result = getContextMenuOptions("https://example.com/x", null, items)

		expect(result[0]).toMatchObject({ type: ContextMenuOptionType.URL, value: "https://example.com/x" })
	})

	it("offers a SHA-shaped query that matches no known commit", () => {
		// The commit may exist in the repo even when it is not in the cached list.
		const result = getContextMenuOptions("deadbee", null, [])

		expect(result[0]).toMatchObject({
			type: ContextMenuOptionType.Git,
			value: "deadbee",
			label: "Commit deadbee",
		})
	})

	it("lower-cases the SHA it offers", () => {
		const result = getContextMenuOptions("DEADBEE", null, [])

		expect(result[0]).toMatchObject({ value: "deadbee" })
	})
})

describe("shouldShowContextMenu — slash commands", () => {
	it("shows the menu while a slash command is being typed", () => {
		expect(shouldShowContextMenu("/mode", 5)).toBe(true)
		expect(shouldShowContextMenu("/", 1)).toBe(true)
	})

	it("stops showing it once the slash command has an argument", () => {
		// After the space the user is writing arguments, not picking a command.
		expect(shouldShowContextMenu("/mode x", 7)).toBe(false)
	})

	it("does not treat a cursor beyond the text as a slash command context", () => {
		expect(shouldShowContextMenu("/mode", 99)).toBe(false)
	})

	it("does not treat a slash later in the text as a slash command", () => {
		expect(shouldShowContextMenu("run /mode", 9)).toBe(false)
	})
})
