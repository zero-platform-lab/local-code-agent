...
describe("formatDate — locale fallback", () => {
	it("falls back to English when i18next has no language set", () => {
		const timestamp = Date.UTC(2024, 0, 15, 10, 30)
		const original = i18next.language
		;(i18next as { language: string }).language = ""
		...
		try {
			...
			;(i18next as { language: string }).language = original
		}
	})

	it("uses the configured language when one is set", () => {
		const timestamp = Date.UTC(2024, 0, 15, 10, 30)
		const original = i18next.language
		;(i18next as { language: string }).language = "ja"

		try {
			expect(formatDate(timestamp)).toBe(
				new Date(timestamp).toLocaleString("ja", {
					month: "long",
					day: "numeric",
					hour: "numeric",
					minute: "2-digit",
