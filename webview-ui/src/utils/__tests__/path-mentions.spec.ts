...
describe("Path Mentions Utilities", () => {
	...
	describe("protocol stripping", () => {
		...
		it("strips the file:// protocol before matching the cwd", () => {
			expect(convertToMentionPath("file:///Users/test/project/src/a.ts", CWD)).toBe("@/src/a.ts")
		})

		it("strips the vscode-remote:// protocol and its authority but keeps the absolute path", () => {
			// The authority (host) must go; everything from the first slash on is the path on
			// the remote host, so it still has to match the cwd.
			expect(convertToMentionPath("vscode-remote://ssh-remote+myhost/Users/test/project/src/a.ts", CWD)).toBe(
				"@/src/a.ts",
			)
		})

		it("never leaves a protocol or authority in the produced mention", () => {
			const result = convertToMentionPath("vscode-remote://ssh-remote+myhost/Users/test/project/a.ts", CWD)
			expect(result).not.toContain("vscode-remote")
			expect(result).not.toContain("myhost")
		})

		it("falls back to an empty path when a vscode-remote URI has no path part", () => {
			expect(convertToMentionPath("vscode-remote://ssh-remote+myhost", CWD)).toBe("")
			expect(convertToMentionPath("vscode-remote://ssh-remote+myhost", undefined)).toBe("")
		})

		it("decodes percent-encoded characters in the path", () => {
			expect(convertToMentionPath("file:///Users/test/project/src/my%20file.ts", CWD)).toBe("@/src/my\\ file.ts")
		})

		it("drops the leading slash of Windows drive paths coming from file:// URIs", () => {
			expect(convertToMentionPath("file:///d:/project/src/a.ts", "d:/project")).toBe("@/src/a.ts")
		})

		it("does not throw when the path cannot be URI-decoded and keeps the raw path", () => {
			const spy = vi.spyOn(console, "error").mockImplementation(() => {})

			try {
				// A lone "%" is not a valid escape sequence: decodeURIComponent throws URIError.
				expect(convertToMentionPath("/Users/test/project/100%/a.ts", CWD)).toBe("@/100%/a.ts")
