...
export function convertToMentionPath(path: string, cwd?: string): string {
	...
	if (path.startsWith("file://")) {
		...
		if (firstSlashIndex !== -1) {
			// Keep the leading slash: what follows the authority is the absolute path on the
			// remote host, and dropping the slash would make it never match the cwd below.
